...
Native.alias=function(E,B,A,F){
	for(var D=0,C=E.length;D<C;D++){E[D].alias(B,A,F);
}};
...
(function(B,A){for(var C=A.length;C--;C){Native.genericize(B,A[C],true);
...
["Quad","Cubic","Quart","Quint"].each(
	function(B,A){
		Fx.Transitions[B]=new Fx.Transition(function(C){return Math.pow(C,[A+2]);
});});
