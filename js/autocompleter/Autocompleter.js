...
Autocompleter.Base = new Class({
	...
	initialize: function(el, el2, el3, el4, options) { // RAFK 09-10-2009
		this.setOptions(options);
		this.element = $(el);
		this.element2 = $(el2); // RAFK 09-09-2009
		this.element3 = $(el3); // RAFK 09-10-2009
		this.element4 = $(el4); // RAFK 06-11-2009
		this.build();
	...
	setSelection: function() {
		...
		if (document.getSelection) {
			...
			if (insert.length > 0) // RAFK 09-09-2009
				sel.findText(insert);
	...
	updateChoices: function(choices) {
		...
		choices.each(this.options.injectChoice || function(choice, i){
			var el = new Element('li').setHTML(this.markQueryValue(choice[0], choice[1], choice[2], choice[3])); // RAFK 09-09-2009
			el.inputValue = choice[0]; // RAFK 09-09-2009
	 ...
	 * @return		{String} Text
	 */
	markQueryValue: function(txt, cod, desc, src) { // RAFK 09-10-2009
		return (this.options.markQuery && this.queryValue) ? "<table style='font-size:9px'><tr><td rowspan='3'><img src='" + src + "' width='50'></td><td style='font-size:11px'>" + txt.replace(new RegExp('^(' + this.queryValue.escapeRegExp() + ')', 'i'), "<span class='autocompleter-queried'>$1</span>") + "</td></tr><tr><td class='textoNegrita_11px'>"+cod+"</td></tr><tr><td>" + desc + "</td></tr></table>" : "<table><tr><td rowspan='3'><img src='" + src + "' width='50'></td><td style='font-size:11px'>" + txt + "</td></tr></tr><tr><td class='textoNegrita_11px'>"+cod+"</td></tr><tr><td style='font-size:11px'>" + desc + "</td></tr></table>"; // RAFK 09-09-2009
		
		/*return (this.options.markQuery && this.queryValue) ? "<img src='" + src + "' width='60'> " + txt.replace(new RegExp('^(' + this.queryValue.escapeRegExp() + ')', 'i'), '<span class="autocompleter-queried">$1</span>') : txt;*/
	},
...
Autocompleter.Ajax.Base = Autocompleter.Base.extend({
	...
	options: {
		postVar: 'value',
		postVar2: 'value', // RAFK 09-09-2009
		postVar3: 'value', // RAFK 09-10-2009
		postVar4: 'value', // RAFK 06-11-2009
		postData: {},
		ajaxOptions: {},
	...
	initialize: function(el, el2, el3, el4, url, options) { // RAFK 06-11-2009
		this.parent(el, el2, el3, el4, options); // RAFK 06-11-2009
	...
	query: function(){
		...
		data[this.options.postVar2] = this.element2.value; // RAFK 09-09-2009
		data[this.options.postVar3] = this.element3.value; // RAFK 09-10-2009
		data[this.options.postVar4] = this.element4.value; // RAFK 06-11-2009
