...
function listaPedidoCompra($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 20, $totalRows = NULL) {
	...
	$sqlBusq .= $cond.sprintf("estatus_pedido_compra = 1");
	...
	if ($valCadBusq[1] != "-1" && $valCadBusq[1] != "") {
		$cond = (strlen($sqlBusq) > 0) ? " AND " : " WHERE ";
		$sqlBusq .= $cond.sprintf("(ped_comp.id_pedido_compra_propio LIKE %s
		OR ped_comp.id_pedido_compra_referencia LIKE %s
		OR CONCAT_WS('-', prov.lrif, prov.rif) LIKE %s
		OR prov.id_proveedor LIKE %s)",
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			...
			$htmlTb .= "<td align=\"right\">".htmlentities($row['id_pedido_compra_referencia'])."</td>";
			...
			$htmlTb .= "<td>";
				$htmlTb .= sprintf("<img class=\"puntero\" onclick=\"window.open('iv_orden_compra_form.php?id=%s','_self');\" src=\"../img/iconos/book_next.png\" title=\"Aprobar / Desaprobar Pedido\"/>",
