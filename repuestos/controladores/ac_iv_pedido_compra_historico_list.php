...
function listaPedidoCompra($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 20, $totalRows = NULL) {
	...
	$query = sprintf("SELECT *,
		...
		WHERE (ped_det.id_pedido_compra = vw_iv_pedidos_compra.id_pedido_compra)) AS items,
		
		(SELECT SUM(ped_det.cantidad) AS pedidos
		FROM iv_pedido_compra_detalle ped_det
		WHERE (ped_det.id_pedido_compra = vw_iv_pedidos_compra.id_pedido_compra)) AS pedidos,
		
		(SELECT SUM(ped_det.pendiente) AS pendientes
		FROM iv_pedido_compra_detalle ped_det
		WHERE (ped_det.id_pedido_compra = vw_iv_pedidos_compra.id_pedido_compra)) AS pendientes,
		
		(IFNULL(vw_iv_pedidos_compra.subtotal, 0)
			- IFNULL(vw_iv_pedidos_compra.subtotal_descuento, 0)
			+ IFNULL((SELECT SUM(ped_gasto.monto) AS total_gasto
					FROM iv_pedido_compra_gasto ped_gasto
					WHERE (ped_gasto.id_pedido_compra = vw_iv_pedidos_compra.id_pedido_compra)), 0)
			+ IFNULL((SELECT SUM(ped_iva.subtotal_iva) AS total_iva
					FROM iv_pedido_compra_iva ped_iva
					WHERE (ped_iva.id_pedido_compra = vw_iv_pedidos_compra.id_pedido_compra)), 0)) AS total
	FROM vw_iv_pedidos_compra %s", $sqlBusq);
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			...
			$htmlTb .= "<td align=\"right\">".utf8_encode($row['id_pedido_compra_propio'])."</td>";
			$htmlTb .= "<td>".utf8_encode($row['tipo_pedido_compra'])."</td>";
			$htmlTb .= "<td align=\"right\">".utf8_encode($row['id_pedido_compra_referencia'])."</td>";
			$htmlTb .= "<td>".utf8_encode($row['nombre']." ".$row['apellido_cliente'])."</td>";
