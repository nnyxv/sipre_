...
// elimina del servidor la imagen anterior solo si existe
if (file_exists("../".$_POST['hddUrlArchivo']) && $_POST['hddIdArticulo'] == "")
...
if (window.parent.document.getElementById('hddUrlArchivo').value == '' && window.parent.document.getElementById('fleUrlArchivo').value.length > 0) {
	...
	if (eval((typeof('window.parent.xajax_vistaPreviaImportar') != "undefined"))
	&& typeof('window.parent.xajax_vistaPreviaImportar') != "function"
	&& !(window.parent.xajax_vistaPreviaImportar)) {
	} else {
		window.parent.xajax_vistaPreviaImportar(window.parent.xajax.getFormValues('frmImportarArchivo'));
	}
	
	if (!(window.parent.document.getElementById('btnGuardarImportarArticulo') == undefined)) {
		setTimeout(function(){
			window.parent.document.getElementById('btnGuardarImportarArticulo').disabled = false;
			window.parent.document.getElementById('btnCancelarImportarArticulo').disabled = false;
		},5000);
	}
}
</script>