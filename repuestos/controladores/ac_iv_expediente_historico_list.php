...
function listaFacturaCompra($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 10, $totalRows = NULL) {
	...
	FROM cp_factura fact_comp
		...
		INNER JOIN iv_expediente_detalle_factura expediente_det_fact ON (fact_comp.id_factura = expediente_det_fact.id_factura) %s
	...
	SELECT 
		...
		FROM cp_factura_detalle fact_comp_det
			INNER JOIN iv_pedido_compra ped_comp ON (fact_comp_det.id_pedido_compra = ped_comp.id_pedido_compra)
		WHERE fact_comp_det.id_factura = fact_comp.id_factura_compra
		LIMIT 1) AS estatus_pedido_compra
	FROM iv_factura_compra fact_comp
		...
		INNER JOIN iv_expediente_detalle_factura expediente_det_fact ON (fact_comp.id_factura_compra = expediente_det_fact.id_factura_compra) %s", $sqlBusq, $sqlBusq);
	...
	$htmlTh .= "<tr align=\"center\" class=\"tituloColumna\">";
		...
		$htmlTh .= "<td width=\"8%\">Nro. Referencia";
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		while ($rowFactDet = mysql_fetch_assoc($rsFactDet)) {
			...
			$arrayDet[0] = $rowPedComp['estatus_pedido_compra'];
			$arrayDet[1] = $rowPedComp['tipo_pedido_compra'];
			$arrayDet[2] = $rowPedComp['id_pedido_compra_propio'];
			$arrayDet[3] = $rowPedComp['id_pedido_compra_referencia'];
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			...
			$htmlTb .= "<td>";
			if (count($arrayFactDet) > 0) {
				$htmlTb .= sprintf("<img class=\"puntero\" onclick=\"verVentana('reportes/iv_registro_compra_pdf.php?valBusq=%s', 960, 550);\" src=\"../img/iconos/ico_view.png\" title=\"Ver\"/>",
					$row['id_factura']);
			}
			$htmlTb .= "</td>";
			$htmlTb .= "<td>";
