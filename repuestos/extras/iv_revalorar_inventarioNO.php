...
if ($porcAjuste) {
	echo "<h2>REVALORALIZACION INVENTARIO DISPONIBLE POR LOTE (".number_format($porcAjuste, 3, ".", ",")."%)</h2>";
} else {
...
while ($row = mysql_fetch_assoc($rs)) {
	...
	if ($idValeSalida > 0 && $idValeEntrada > 0 && $idArticulo > 0) {
		...
		$idArticuloCostoNuevo = mysql_insert_id();
					...
					/////////////////////////////////////////////////////////////////
		// articulos actualizar los 0.00
		$update3 = sprintf("UPDATE iv_articulos_costos set costo = 0.01 , costo_promedio = 0.01 where costo =  0.00 and id_articulo_costo = %s", valTpDato($idArticuloCostoNuevo, "int"));




		$resultado4= mysql_query($update3);
			if (!$resultado4) die(mysql_error()."<br>Error Nro: ".mysql_errno()."<br>Line: ".__LINE__."<br>SQL: ".$update3);

		
		

	/////////////////////////////////////////////////////////////////
	/*	// articulos actualizar los 0.5 a 1
		$update2 = sprintf("UPDATE iv_articulos_costos set costo = 1 , costo_promedio = 1 where costo < 1 and costo >= 0.55  and id_articulo_costo = %s", valTpDato($idArticuloCostoNuevo, "int"));
		$resultado3 = mysql_query($update2);
		if (!$resultado3) die(mysql_error()."<br>Error Nro: ".mysql_errno()."<br>Line: ".__LINE__."<br>SQL: ".$update2);
...
<form method="POST" id="frmInventario" name="frmInventario" onsubmit="return false;">
    ...
    <tr>
    	...
    	<td>
        	<input type="text" id="txtPorcentajeAjuste" name="txtPorcentajeAjuste" onblur="setFormatoRafk(this,3);" onkeypress="return validarSoloNumerosReales(event);" value="" class="inputHabilitado"/>
        </td>    
    </tr>
    <tr>
    	<td colspan="4"><hr>
