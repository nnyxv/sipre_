...
while ($row = mysql_fetch_assoc($rs)) {
	...
	FROM cj_cc_encabezadofactura fact_vent
		...
		INNER JOIN iv_articulos art ON (fact_vent_det.id_articulo = art.id_articulo) %s %s
	...
	while ($rowFacturaVenta = mysql_fetch_assoc($rsFacturaVenta)) {
		...
		$objPHPExcel->getActiveSheet()->setCellValue("B".$contFila, utf8_encode($rowFacturaVenta['descripcion']));
		...
		$objPHPExcel->getActiveSheet()->setCellValue("I".$contFila, utf8_encode($rowFacturaVenta['numero_pedido']));
		$objPHPExcel->getActiveSheet()->setCellValue("J".$contFila, utf8_encode($rowFacturaVenta['id_pedido_venta_referencia']));
...
$tituloDcto = "Ventas por Cliente (Artículo)";
