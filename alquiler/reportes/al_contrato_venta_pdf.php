...
if (mysql_num_rows($rsDoc) == 0) { die("No se encontró el contrato por id"); }
...
// Cabecera de página
	...
	function Header(){
		...
		$aux = getBarcode($rowDoc['numero_contrato_venta'],'tmp/img_codigo');
		...
		$this->SetY(16);
		$this->Cell(0,3,"Contrato de Arrendamiento",0,0,'C'); // Título
	}
}
...
$pdf->Cell(175,3,utf8_decode("N° Contrato:"),0,0,'R'); // Título
$pdf->SetFont('Arial','',8); 
$pdf->Cell(15,3,$rowDoc['numero_contrato_venta'],0,0,'L'); // Título 
$pdf->Ln();
...
$sqlPrecio = sprintf("SELECT 
		al_contrato_venta_precio.id_contrato_venta_precio,
		al_contrato_venta_precio.id_contrato_venta,
		al_contrato_venta_precio.id_precio,
		nombre_precio,
		al_precios_detalle.descripcion,
		al_contrato_venta_precio.id_precio_detalle,
		al_contrato_venta_precio.id_tipo_precio,
		al_contrato_venta_precio.dias_calculado,
		total_precio,
		al_contrato_venta_precio.precio
	FROM al_contrato_venta_precio
		...
		INNER JOIN al_precios ON al_contrato_venta_precio.id_precio = al_precios.id_precio
		INNER JOIN al_tipo_precio ON al_contrato_venta_precio.id_tipo_precio = al_tipo_precio.id_tipo_precio
	WHERE al_contrato_venta_precio.id_contrato_venta = %s",
valTpDato($idDocumento, "int"));
...
$pdf->SetFont('Arial','B',8);
headerTable($ancho,$posiscion,$header);
$sqlAcc = sprintf("SELECT al_contrato_venta_accesorio.id_contrato_venta_accesorio, nom_accesorio, des_accesorio, 
	 (CASE acc.id_tipo_accesorio
		...
		THEN 'Accesorio'
			WHEN 3
		THEN 'Contrato'
	END) AS descripcion_tipo_accesorio, al_contrato_venta_accesorio.id_contrato_venta,
	al_contrato_venta_accesorio.id_accesorio, al_contrato_venta_accesorio.id_tipo_accesorio, cantidad, precio
FROM al_contrato_venta_accesorio
	INNER JOIN an_accesorio acc ON al_contrato_venta_accesorio.id_accesorio = acc.id_accesorio
WHERE id_contrato_venta = %s AND acc.id_tipo_accesorio IN (%s) AND id_modulo IN (%s)",
valTpDato($idDocumento, "int"),
...
$sqlAdc = sprintf("SELECT al_contrato_venta_accesorio.id_contrato_venta_accesorio, nom_accesorio,des_accesorio, 
	 (CASE acc.id_tipo_accesorio
			WHEN 1
		THEN 'Adicional'
			WHEN 2
...
$sql = sprintf("SELECT 
	...
	vw_iv_modelo.nom_modelo,
	vw_iv_modelo.nom_marca
FROM al_contrato_venta_cambio_unidad cambio_unidad
INNER JOIN an_unidad_fisica uni_fis ON (uni_fis.id_unidad_fisica = cambio_unidad.id_unidad_fisica)
INNER JOIN vw_iv_modelos vw_iv_modelo ON (uni_fis.id_uni_bas = vw_iv_modelo.id_uni_bas)			
INNER JOIN an_uni_bas uni_bas ON (uni_fis.id_uni_bas = uni_bas.id_uni_bas)
INNER JOIN an_clase clase ON (uni_bas.cla_uni_bas = clase.id_clase)
...
$tieneCambio = mysql_num_rows($rs);

if($tieneCambio){
	$pdf->Ln();
	$pdf->Ln();
	$pdf->SetFont('Arial','B',8);
	$pdf->Cell(33,3,utf8_decode("Cambios de vehículos:"),0,0,'L'); // Título
	$pdf->Ln();
	
	$ancho = array("18","45","30","100");
	$posiscion = array("C","C","C","C");
	$header = array(utf8_decode($spanPlaca),utf8_decode("Unidad"), utf8_decode("Fecha"), "Motivo");
	$pdf->SetFont('Arial','B',8);
	headerTable($ancho,$posiscion,$header,0);
	
	while($row = mysql_fetch_assoc($rs)){
		$pdf->SetFont('Arial','',8);
		
		$ancho = array("18","45","30","100");
		$posiscion = array("C","L","C","L");
		$header = array($row["placa"],
						substr($row["nom_modelo"]." ".$row["nom_uni_bas"],0,25), 
						date(spanDateFormat." h:i a", strtotime($row["fecha_creacion"])), 
						ucfirst(strtolower(substr($row["motivo"],0,72))));
		$pdf->SetFont('Arial','',8);
		bodyTable($ancho,$posiscion,$header,0);
	}
}
...
$sqlIvaContrato = sprintf("
	SELECT id_contrato_venta_iva, id_contrato_venta, observacion, 
		al_contrato_venta_iva.id_iva, al_contrato_venta_iva.iva,
		al_contrato_venta_iva.base_imponible, subtotal_iva
	FROM al_contrato_venta_iva 
		INNER JOIN pg_iva ON al_contrato_venta_iva.id_iva = pg_iva.idIva
	WHERE id_contrato_venta = %s",
valTpDato($rowDoc['id_contrato_venta'], "int"));
