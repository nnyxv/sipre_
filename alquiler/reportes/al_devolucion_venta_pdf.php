...
$queryEncabezado = sprintf("SELECT
	...
	cxc_nc.montoNetoNotaCredito AS total_nota_credito,
	cxc_nc.subtotalNotaCredito AS subtotal_nota_credito,
	cxc_nc.porcentaje_descuento,
	cxc_nc.subtotal_descuento,
	cxc_nc.montoExentoCredito AS monto_exento,
	cxc_nc.montoExoneradoCredito AS monto_exonerado
...
WHERE cxc_nc.idNotaCredito = %s
	AND cxc_nc.idDepartamentoNotaCredito IN (4)",
...
if(in_array($rowConfig403['valor'],array(1,2,3))){
	...
	$posY = 9;
	imagestring($img,1,300,$posY,str_pad(utf8_decode("NOTA DE CRÉDITO SERIE - AL"), 34, " ", STR_PAD_BOTH),$textColor);

	$posY += 9;
	$posY += 9;
	imagestring($img,1,300,$posY,utf8_decode("NOTA CRÉD. NRO."),$textColor);
	imagestring($img,2,374,$posY-3,": ".$rowEncabezado['numeracion_nota_credito'],$textColor);
	...
	imagestring($img,1,375,$posY,": ".$rowEncabezado['numeroFactura'],$textColor);
	...
	$queryDet = sprintf("SELECT
		...
		al_nc_det_precio.precio,
		al_nc_det_precio.dias_calculado,
		al_nc_det_precio.total_precio,
				
		IF ((SELECT COUNT(det_precio_impuesto.id_nota_credito_detalle_precio_impuesto)
				FROM al_nota_credito_detalle_precio_impuesto det_precio_impuesto
				WHERE det_precio_impuesto.id_nota_credito_detalle_precio = al_nc_det_precio.id_nota_credito_detalle_precio) = 0,
			CONCAT('(E) ',precio.nombre_precio), 
			precio.nombre_precio) AS nombre_precio
		
	FROM al_nota_credito_detalle_precio al_nc_det_precio
		INNER JOIN al_precios_detalle det_precio ON (al_nc_det_precio.id_precio_detalle = det_precio.id_precio_detalle)
		INNER JOIN al_precios precio ON (al_nc_det_precio.id_precio = precio.id_precio)
	WHERE al_nc_det_precio.id_nota_credito = %s",
		valTpDato($idDocumento, "int"));
	...
	$queryDet = sprintf("SELECT
		...
		IF ((SELECT COUNT(cxc_nc_det_acc_impuesto.id_nota_credito_detalle_accesorios)
				FROM cj_cc_nota_credito_detalle_accesorios_impuesto cxc_nc_det_acc_impuesto
				WHERE cxc_nc_det_acc_impuesto.id_nota_credito_detalle_accesorios = cxc_nc_det_acc.id_nota_credito_detalle_accesorios) = 0,
	...
	$arrayObservacionDcto = str_split(strtoupper($rowEncabezado['observacionesNotaCredito']), 45);
	...
	imagestring($img,1,380,$posY,str_pad(formatoNumero($rowEncabezado['subtotal_nota_credito']), 18, " ", STR_PAD_LEFT),$textColor); // <----
	...
	imagestring($img,2,362,$posY,str_pad(formatoNumero($rowEncabezado['total_nota_credito']), 18, " ", STR_PAD_LEFT),$textColor);

	$arrayImg[] = "tmp/"."nota_credito_alquiler".$pageNum.".png";
...
if($rowConfig403['valor'] == "2"){//PANAMA
    ...
    $rutaEncabezado = "tmp/nota_credito_encabezado.png";
