...
$queryEncabezado = sprintf("SELECT
	...
	vale_salida.total_vale_salida,
	vale_salida.subtotal,
	vale_salida.porcentaje_descuento,
	vale_salida.subtotal_descuento,
	vale_salida.monto_exento,
	vale_salida.monto_exonerado
FROM al_vale_salida vale_salida
...
WHERE vale_salida.id_vale_salida = %s",
...
if(in_array($rowConfig403['valor'],array(1,2,3))){
	...
	$posY = 9;
	imagestring($img,1,300,$posY,str_pad("VALE DE SALIDA", 34, " ", STR_PAD_BOTH),$textColor);

	$posY += 9;
	$posY += 9;
	imagestring($img,1,300,$posY,utf8_decode("VALE SAL. NRO."),$textColor);
	imagestring($img,2,374,$posY-3,": ".$rowEncabezado['numero_vale_salida'],$textColor);

	$posY += 9;
	imagestring($img,1,300,$posY,utf8_decode("FECHA VALE"),$textColor);
	imagestring($img,1,375,$posY,": ".date(spanDateFormat, strtotime($rowEncabezado['fecha_vale_salida'])),$textColor);
	...
	imagestring($img,2,362,$posY,str_pad(formatoNumero($rowEncabezado['total_vale_salida']), 18, " ", STR_PAD_LEFT),$textColor);

	$arrayImg[] = "tmp/"."vale_salida_alquiler".$pageNum.".png";
