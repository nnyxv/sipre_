...
function buscar($frmBuscar) {
	...
	$objResponse->loadCommands(listaContratoVenta(0, "numero_vale_entrada", "DESC", $valBusq));
...
function listaContratoVenta($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 20, $totalRows = NULL) {
	...
	if ($valCadBusq[1] != "" && $valCadBusq[2] != "") {
		$cond = (strlen($sqlBusq) > 0) ? " AND " : " WHERE ";
		$sqlBusq .= $cond.sprintf("DATE(vale_entrada.fecha_vale_entrada) BETWEEN %s AND %s",
	...
	if ($valCadBusq[5] != "-1" && $valCadBusq[5] != "") {
		...
		OR vale_entrada.numero_vale_entrada LIKE %s
	...
	$query = sprintf("SELECT 
		...
		(vale_entrada.subtotal - vale_entrada.subtotal_descuento) AS total_neto,
		vale_entrada.total_vale_entrada AS total,
	...
	FROM al_contrato_venta contrato
		...
		INNER JOIN al_vale_entrada vale_entrada ON (vale_salida.id_vale_salida = vale_entrada.id_vale_salida)
	...
	$htmlTh .= "<tr align=\"center\" class=\"tituloColumna\">";
		...
		$htmlTh .= ordenarCampo("xajax_listaContratoVenta", "5%", $pageNum, "fecha_vale_entrada", $campOrd, $tpOrd, $valBusq, $maxRows, "Fecha Vale Entrada");
		$htmlTh .= ordenarCampo("xajax_listaContratoVenta", "5%", $pageNum, "numero_vale_entrada", $campOrd, $tpOrd, $valBusq, $maxRows, "Nro. Vale Entrada");
		$htmlTh .= ordenarCampo("xajax_listaContratoVenta", "5%", $pageNum, "numero_vale_salida", $campOrd, $tpOrd, $valBusq, $maxRows, "Nro. Vale Salida");
		...
		$htmlTh .= ordenarCampo("xajax_listaContratoVenta", "5%", $pageNum, "total", $campOrd, $tpOrd, $valBusq, $maxRows, "Total Vale Entrada");
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			...
			$htmlTb .= "<td align=\"center\">".date(spanDateFormat, strtotime($row['fecha_vale_entrada']))."</td>";
			$htmlTb .= "<td align=\"center\">".$row['numero_vale_entrada']."</td>";
			...
			$htmlTb .= "</td>";
			$htmlTb .= "<td>";
				$htmlTb .= sprintf("<img class=\"puntero\" onclick=\"verVentana('reportes/al_devolucion_vale_salida_pdf.php?valBusq=%s', 960, 550);\" src=\"../img/iconos/ico_print.png\" title=\"Vale de Entrada PDF\"/>",
					$row['id_vale_entrada']);
			$htmlTb .= "</td>";
		$htmlTb .= "</tr>";
	}
