...
function calcularDcto($frmDcto, $frmListaPrecio, $frmListaAccesorio, $frmListaAdicional, $frmTotalDcto) {
	...
	if ($frmTotalDcto['hddConfig500'] == 1 && $txtDescuento > $ResultConfig500) {
		...
		$objResponse->alert("El porcentaje de descuento supera al máximo permitido.");
	}
...
function cargarDcto($idDocumento) {
	...
	if ($idDocumento > 0) {
		$objResponse->assign("tituloPaginaAlquiler","innerHTML","Edici&oacute;n Presupuesto de Alquiler");
		...
		$sql = sprintf("SELECT
			...
			AND presupuesto.estatus_presupuesto_venta = 0;",
		...
		if(mysql_num_rows($rs) == 0){
			$objResponse->alert("El Presupuesto no puede ser cargado debido a que su estado no es válido");
			return $objResponse->script("byId('btnCancelar').click();");
		}
		
		// CARGA EL PRECIO DEL CONTRATO
		$sqlPrecioDet = sprintf("SELECT 
				al_presupuesto_venta_precio.*,
				al_precios.nombre_precio,
				al_precios_detalle.descripcion
		...
		while ($rowPrecioDet = mysql_fetch_assoc($rsPrecioDet)) {
			$arrayIva = array();
			$sqlIva = sprintf("SELECT id_impuesto, impuesto FROM al_presupuesto_venta_precio_impuesto WHERE id_presupuesto_venta_precio = %s",
				valTpDato($rowPrecioDet["id_presupuesto_venta_precio"], "int"));
			...
			$objResponse->loadCommands(insertarPrecio($rowPrecioDet["id_presupuesto_venta_precio"], $rowPrecioDet["id_precio"], $rowPrecioDet["nombre_precio"], $rowPrecioDet["id_precio_detalle"], $rowPrecioDet["descripcion"], $rowPrecioDet["precio"], $rowPrecioDet["dias"], $rowPrecioDet["id_tipo_precio"], $rowPrecioDet['dias_calculado'], $rowPrecioDet['total_precio'], $arrayIva));
		}
		
		// CARGA LOS ACCESORIOS Y ADICIONALES
		$sqlAccesorioDet = sprintf("SELECT 
				al_presupuesto_venta_accesorio.*,
				nom_accesorio,
				des_accesorio 
			FROM al_presupuesto_venta_accesorio 
			INNER JOIN an_accesorio ON al_presupuesto_venta_accesorio.id_accesorio = an_accesorio.id_accesorio
			WHERE id_presupuesto_venta = %s",
			valTpDato($idDocumento, "int"));
		...
		while ($rowAccesorioDet = mysql_fetch_assoc($rsAccesorioDet)) {
			$arrayIva = array();
			$sqlIva = sprintf("SELECT id_impuesto, impuesto FROM al_presupuesto_venta_accesorio_impuesto WHERE id_presupuesto_venta_accesorio = %s",
				valTpDato($rowAccesorioDet["id_presupuesto_venta_accesorio"], "int"));
			$rsIva = mysql_query($sqlIva);			
			if (!$rsIva) return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
			...
			$objResponse->loadCommands(insertarAccesorio($rowAccesorioDet["id_presupuesto_venta_accesorio"], $rowAccesorioDet["id_accesorio"], $rowAccesorioDet["id_tipo_accesorio"], $rowAccesorioDet["nom_accesorio"], $rowAccesorioDet["des_accesorio"], $rowAccesorioDet["cantidad"], $rowAccesorioDet["precio"], $rowAccesorioDet["costo"], $arrayIva));
		}
					
		// DATOS DEL CONTRATO
		$objResponse->assign("txtIdEmpresa","value",$row['id_empresa']);
		$objResponse->assign("txtEmpresa","value",utf8_encode($row['nombre_empresa']));
		$objResponse->assign("hddIdContrato","value",$row['id_presupuesto_venta']);
		$objResponse->assign("txtNumeroContrato","value",$row['numero_presupuesto_venta']);
		...
		$objResponse->assign("tituloPaginaAlquiler","innerHTML","Nuevo Presupuesto de Alquiler");
	...
	if(esFinalizar()){
		$objResponse->assign("tituloPaginaAlquiler","innerHTML","Aprobaci&oacute;n de Presupuesto de Alquiler");
...
function guardarDcto($frmDcto, $frmListaPrecio, $frmListaAccesorio, $frmListaAdicional, $frmTotalDcto, $frmListaCambioUnidades){
	...
	// VERIFICA VALORES DE CONFIGURACION (Dias de vencimiento del presupuesto)
	$diasVencimientoPresupuesto = 0;
	$arrayConfig = valorConfiguracion(501, $idEmpresa);
	if ($arrayConfig[0] != true && strlen($arrayConfig[1]) > 0) {
		return $objResponse->alert($arrayConfig[1]);
	} else if ($arrayConfig[0] == true) {
		if($arrayConfig[1] == ""){
			return $objResponse->alert("No se ha configurado dias de vencimiento por presupuesto");
		} 
		$diasVencimientoPresupuesto = $arrayConfig[1];
	}
	
	$fechaVencimientoPresupuesto = date("Y-m-d", strtotime("+ ".$diasVencimientoPresupuesto." DAYS"));
	
	mysql_query("START TRANSACTION;");
	
	if ($idDocumentoVenta > 0) {
		if (!xvalidaAcceso($objResponse,"al_presupuesto_venta_list","editar")) { return $objResponse; }
		
		//comprobar estado del contrato
		$sql = sprintf("SELECT * FROM al_presupuesto_venta WHERE id_presupuesto_venta = %s AND estatus_presupuesto_venta = 0;",
			valTpDato($idDocumentoVenta, "int"));
		$rs = mysql_query($sql);
		if (!$rs) { return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__); }		
		
		if(mysql_num_rows($rs) == 0){
			$objResponse->alert(utf8_encode("El Presupuesto no puede ser actualizado debido a que su estado no es válido"));
			return $objResponse->script("byId('btnCancelar').click();");
		}
		
		$rowContrato = mysql_fetch_assoc($rs);
					
		$updateSQL = sprintf("UPDATE al_presupuesto_venta SET
			id_cliente = %s,
			id_cliente_pago = %s,
			id_unidad_fisica = %s,
			id_tipo_contrato = %s,
		...
		if (!xvalidaAcceso($objResponse,"al_presupuesto_venta_list","insertar")) { return $objResponse; }
		...
		ORDER BY aplica_sucursales DESC LIMIT 1;",
			valTpDato(49, "int"), // 49 = Presupuesto Alquiler
			valTpDato($idEmpresa, "int"),
			valTpDato($idEmpresa, "int"));
		...
		if($rowNumeracion['numero_actual'] == ""){ return $objResponse->alert("No se ha configurado la numeracion de presupuestos"); }
		...
		$insertSQL = sprintf("INSERT INTO al_presupuesto_venta (numero_presupuesto_venta, id_empresa, id_tipo_contrato, id_cliente, id_cliente_pago, id_unidad_fisica, id_estado_adicional_salida, id_moneda, condicion_pago, estatus_presupuesto_venta, id_empleado_creador, observacion, subtotal, porcentaje_descuento, subtotal_descuento, monto_exento, total_contrato, fecha_vencimiento, fecha_vencimiento_presupuesto, kilometraje_salida, nivel_combustible_salida, fecha_salida, fecha_entrada, dias_contrato)
		VALUE (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s);",
			...
			valTpDato($frmDcto['lstMoneda'], "int"),
			valTpDato($frmDcto['lstTipoPago'], "int"),
			valTpDato(0, "int"), // 0 = Presupuesto Sin Aprobar, 1 = Aprobado, 2 = Anulado
			valTpDato($frmDcto['txtIdEmpleado'], "int"),
			valTpDato($frmTotalDcto['txtObservacion'], "text"),
			valTpDato($frmTotalDcto['txtSubTotal'], "real_inglesa"),
			valTpDato($frmTotalDcto['txtDescuento'], "real_inglesa"),
			...
			valTpDato($frmTotalDcto['txtTotalOrden'], "real_inglesa"),
			valTpDato($fechaVencimientoContrato, "date"),
			valTpDato($fechaVencimientoPresupuesto, "date"),
			valTpDato($frmDcto['txtKilometrajeSalida'], "int"),
			valTpDato($frmDcto['lstCombustibleSalida'], "real_inglesa"),
			valTpDato($fechaSalida, "date"),
			valTpDato($fechaEntrada, "date"),
	...
	if(strlen($frmDcto["hddIdDetPrecioEliminar"]) > 0){		
		$deleteSQL = sprintf("DELETE FROM al_presupuesto_venta_precio WHERE id_presupuesto_venta_precio IN (%s);",
	...
	if(strlen($frmDcto["hddIdDetAccesorioEliminar"]) > 0){
		$deleteSQL = sprintf("DELETE FROM al_presupuesto_venta_accesorio WHERE id_presupuesto_venta_accesorio IN (%s);",
	...
	foreach($frmListaPrecio['hddIdDetallePrecioContrato'] as $indice => $idDetallePrecioContrato){
		if($idDetallePrecioContrato == ""){
			$insertSQL = sprintf("INSERT INTO al_presupuesto_venta_precio (id_presupuesto_venta, id_precio, id_precio_detalle, id_tipo_precio, dias, precio, dias_calculado, total_precio)
			...
			foreach($arrayIdPrecioImpuesto as $indiceImpuesto => $idImpuesto){
				$insertSQL = sprintf("INSERT INTO al_presupuesto_venta_precio_impuesto (id_presupuesto_venta_precio, id_impuesto, impuesto)
				VALUE (%s, %s, %s);",
	...
	foreach($frmListaAccesorio['hddIdDetalleAccesorioContrato'] as $indice => $idDetalleAccesorioContrato){
		if($idDetalleAccesorioContrato == ""){
			$insertSQL = sprintf("INSERT INTO al_presupuesto_venta_accesorio (id_presupuesto_venta, id_accesorio, id_tipo_accesorio, cantidad, precio, costo)
			VALUE (%s, %s, %s, %s, %s, %s);",
				valTpDato($idDocumentoVenta, "int"),
			...
			foreach($arrayIdAccesorioImpuesto as $indiceImpuesto => $idImpuesto){
				$insertSQL = sprintf("INSERT INTO al_presupuesto_venta_accesorio_impuesto (id_presupuesto_venta_accesorio, id_impuesto, impuesto)
				VALUE (%s, %s, %s);",
	...
	$deleteSQL = sprintf("DELETE FROM al_presupuesto_venta_iva WHERE id_presupuesto_venta = %s;",
		valTpDato($idDocumentoVenta, "int"));
	$Result1 = mysql_query($deleteSQL);
	if (!$Result1) { return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__); }
	...
	foreach ($frmTotalDcto['hddIdIva'] as $indice => $idIva) {
		$insertSQL = sprintf("INSERT INTO al_presupuesto_venta_iva (id_presupuesto_venta, base_imponible, subtotal_iva, id_iva, iva)
		VALUE (%s, %s, %s, %s, %s);",
			valTpDato($idDocumentoVenta, "int"),
	...
	if(esFinalizar()){	
		...
		$sql = sprintf("SELECT * 
						...
						WHERE estado_venta = 'ACTIVO FIJO' 
						AND id_estado_adicional = 1 
		...
		$Result1 = aprobarPresupuesto($idDocumentoVenta);
	...
	if($frmDcto['hddIdContrato'] == ""){//si es nuevo
		$objResponse->script("verVentana('reportes/al_presupuesto_venta_pdf.php?valBusq=".$idDocumentoVenta."', 960, 550)");
	}
...
function aprobarPresupuesto($idPresupuestoVenta){
	
	$sql = sprintf("SELECT 
				presupuesto.*, 
				tipo_contrato.id_clave_movimiento_salida
			FROM al_presupuesto_venta presupuesto
			INNER JOIN al_tipo_contrato tipo_contrato ON (presupuesto.id_tipo_contrato = tipo_contrato.id_tipo_contrato)
			WHERE presupuesto.id_presupuesto_venta = %s",
	...
	if($rowNumeracion['numero_actual'] == ""){ return array(false, "No se ha configurado la numeracion de contratos"); }
	...
	$sql = sprintf("INSERT INTO al_contrato_venta (numero_contrato_venta, id_empresa, id_tipo_contrato, id_cliente, id_cliente_pago, id_presupuesto_venta, id_unidad_fisica, id_estado_adicional_salida, id_moneda, condicion_pago, estatus_contrato_venta, id_empleado_creador, observacion, subtotal, porcentaje_descuento, subtotal_descuento, monto_exento, total_contrato, fecha_vencimiento, kilometraje_salida, nivel_combustible_salida, fecha_salida, fecha_entrada, dias_contrato)
			SELECT 
				%s,
				presupuesto.id_empresa,
				presupuesto.id_tipo_contrato,
				presupuesto.id_cliente,
				presupuesto.id_cliente_pago,
				%s,
				presupuesto.id_unidad_fisica,
				presupuesto.id_estado_adicional_salida,
				presupuesto.id_moneda,
				presupuesto.condicion_pago,
				%s,
				%s,
				presupuesto.observacion,
				presupuesto.subtotal,
				presupuesto.porcentaje_descuento,
				presupuesto.subtotal_descuento,
				presupuesto.monto_exento,
				presupuesto.total_contrato,
				presupuesto.fecha_vencimiento,
				presupuesto.kilometraje_salida,
				presupuesto.nivel_combustible_salida,
	...
	$rs = mysql_query($sql);
	if (!$rs) { return array(false, mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__); }
	$idContratoVenta = mysql_insert_id();	
	
	$sql = sprintf("INSERT INTO al_contrato_venta_iva (id_contrato_venta, base_imponible, subtotal_iva, id_iva, iva)	
			SELECT %s, base_imponible, subtotal_iva, id_iva, iva 
			FROM al_presupuesto_venta_iva 
			WHERE id_presupuesto_venta = %s",
		valTpDato($idContratoVenta, "int"),
		valTpDato($idPresupuestoVenta, "int"));
	$rs = mysql_query($sql);
	if (!$rs) { return array(false, mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__); }
	
	$sql = sprintf("SELECT id_presupuesto_venta_precio, id_precio, id_precio_detalle, id_tipo_precio, dias, precio, dias_calculado, total_precio
			FROM al_presupuesto_venta_precio 
			WHERE id_presupuesto_venta = %s",
		valTpDato($idPresupuestoVenta, "int"));
	$rs = mysql_query($sql);
	...
	while($row = mysql_fetch_assoc($rs)){		
		...
		VALUE (%s, %s, %s, %s, %s, %s, %s, %s);",
			...
			valTpDato($row['id_precio_detalle'], "int"),
			valTpDato($row['id_tipo_precio'], "int"),
			valTpDato($row['dias'], "int"),
			valTpDato($row['precio'], "real_inglesa"),
			valTpDato($row['dias_calculado'], "int"),
			valTpDato($row['total_precio'], "real_inglesa"));
		$Result1 = mysql_query($insertSQL);
		if (!$Result1) { return array(false, mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__); }
		$idContratoVentaPrecio = mysql_insert_id();
		
		$sql = sprintf("SELECT id_impuesto, impuesto
				FROM al_presupuesto_venta_precio_impuesto 
				WHERE id_presupuesto_venta_precio = %s",
			valTpDato($row['id_presupuesto_venta_precio'], "int"));
		$rs2 = mysql_query($sql);
		if (!$rs2) { return array(false, mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__); }
		
		while($rowImpuesto = mysql_fetch_assoc($rs2)){
			$insertSQL = sprintf("INSERT INTO al_contrato_venta_precio_impuesto (id_contrato_venta_precio, id_impuesto, impuesto)
			VALUE (%s, %s, %s);",
				valTpDato($idContratoVentaPrecio, "int"),
				valTpDato($rowImpuesto['id_impuesto'], "int"),
				valTpDato($rowImpuesto['impuesto'], "real_inglesa"));
			$Result1 = mysql_query($insertSQL);
			if (!$Result1) { return array(false, mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__); }
		}
	}
	
	$sql = sprintf("SELECT id_presupuesto_venta_accesorio, id_accesorio, id_tipo_accesorio, cantidad, precio, costo
			FROM al_presupuesto_venta_accesorio 
			WHERE id_presupuesto_venta = %s",
		valTpDato($idPresupuestoVenta, "int"));
	$rs = mysql_query($sql);
	...
	//INSERTA ACCESORIOS Y ADICIONALES
	while($row = mysql_fetch_assoc($rs)){
		...
		VALUE (%s, %s, %s, %s, %s, %s);",
			valTpDato($idContratoVenta, "int"),
			valTpDato($row['id_accesorio'], "int"),
			valTpDato($row['id_tipo_accesorio'], "int"),
		...
		$sql = sprintf("SELECT id_impuesto, impuesto
				FROM al_presupuesto_venta_accesorio_impuesto 
				WHERE id_presupuesto_venta_accesorio = %s",
			valTpDato($row['id_presupuesto_venta_accesorio'], "int"));
		$rs2 = mysql_query($sql);
		if (!$rs2) { return array(false, mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__); }
	...
	$sql = sprintf("UPDATE al_presupuesto_venta SET 
					...
					id_empleado_aprobacion = %s,
					fecha_aprobacion = %s
					WHERE id_presupuesto_venta = %s",
			valTpDato(1,"int"),// 0 = Presupuesto Sin Aprobar, 1 = Aprobado, 2 = Anulado
			valTpDato($_SESSION['idEmpleadoSysGts'],"int"),
			"NOW()",
			valTpDato($idPresupuestoVenta,"int"));
	$rs = mysql_query($sql);
	...
	$sql = sprintf("SELECT * 
					...
					AND id_estado_adicional = 1 
					AND id_unidad_fisica = %s;",
		valTpDato($rowPresupuesto['id_unidad_fisica'], "int"));
	$rs = mysql_query($sql);
	if (!$rs) { return array(false, mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__); }
	$rowUnidadFisica = mysql_fetch_assoc($rs);
	
	if(mysql_num_rows($rs) == 0){
		return array(false, utf8_encode("El vehículo posee un estado que no es válido."));
	}
	...
	$updateSQL = sprintf("UPDATE an_unidad_fisica SET 
		...
		valTpDato($rowPresupuesto['kilometraje_salida'], "int"),
		valTpDato($rowPresupuesto['id_estado_adicional_salida'], "int"),
		valTpDato($rowPresupuesto['id_unidad_fisica'], "int"));
	$Result1 = mysql_query($updateSQL);
	if (!$Result1) { return array(false, mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__); }	
	...
	VALUE (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s);",
		...
		valTpDato($rowPresupuesto["id_clave_movimiento_salida"], "int"),
	...
	return array(true, "Presupuesto aprobado, nro de contrato: ".$numeroActual);
}
