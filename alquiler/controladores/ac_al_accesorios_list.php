...
function editarAccesorio($nomObjeto, $idAccesorio) {
	...
	if (xvalidaAcceso($objResponse,"al_accesorios_list","editar")) {
...
function eliminarAccesorio($idAccesorio, $valFormListaAccesorio) {
	...
	if (xvalidaAcceso($objResponse,"al_accesorios_list","eliminar")) {
		...
		$objResponse->alert("Accesorio eliminado exitosamente.");
	...
	if ($valForm['hddIdAccesorio'] > 0) {
		if (xvalidaAcceso($objResponse,"al_accesorios_list","editar")) {
			...
			while($rowComboDetalle = mysql_fetch_assoc($rsComboDetalle)) {
				...
				WHERE tipo = 6 AND activo = 1 AND estado = 1");
				...
				$updateSQL = sprintf("UPDATE an_presupuesto_accesorio_detalle, an_presupuesto_accesorio presto_acc, an_presupuesto presto SET
					...
					iva_unitario = (((CASE (SELECT acc.iva_accesorio FROM an_accesorio acc
										...
										WHEN 1 THEN
											(SELECT iva FROM pg_iva WHERE tipo = 6 AND activo = 1 AND estado = 1 ORDER BY iva)
										ELSE
											0
		...
		if (xvalidaAcceso($objResponse,"al_accesorios_list","insertar")) {
...
function listadoAccesorio($pageNum = 0, $campOrd = "des_accesorio", $tpOrd = "ASC", $valBusq = "", $maxRows = 15, $totalRows = NULL) {
	...
	$htmlTh .= "<tr class=\"tituloColumna\">";
		...
		$htmlTh .= ordenarCampo("xajax_listadoAccesorio", "60%", $pageNum, "des_accesorio", $campOrd, $tpOrd, $valBusq, $maxRows, "Descripción");
		$htmlTh .= ordenarCampo("xajax_listaAccesorio", "10%", $pageNum, "iva_accesorio", $campOrd, $tpOrd, $valBusq, $maxRows, "Impuesto");
		$htmlTh .= ordenarCampo("xajax_listadoAccesorio", "10%", $pageNum, "precio_accesorio", $campOrd, $tpOrd, $valBusq, $maxRows, "Precio");
...
function nuevoAccesorio($nomObjeto) {
	...
	if (xvalidaAcceso($objResponse,"al_accesorios_list","insertar")) {
		...
		$objResponse->assign("tdFlotanteTitulo","innerHTML","Agregar Accesorio");
