...
function buscar($frmBuscar) {
	...
	$objResponse->loadCommands(listaContratoVenta(0, "numeracion_nota_credito", "DESC", $valBusq));
...
function listaContratoVenta($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 20, $totalRows = NULL) {
	...
	if ($valCadBusq[1] != "" && $valCadBusq[2] != "") {
		$cond = (strlen($sqlBusq) > 0) ? " AND " : " WHERE ";
		$sqlBusq .= $cond.sprintf("DATE(notacredito.fechaNotaCredito) BETWEEN %s AND %s",
	...
	if ($valCadBusq[5] != "-1" && $valCadBusq[5] != "") {
		...
		OR notacredito.numeracion_nota_credito LIKE %s
		OR notacredito.numeroControl LIKE %s
	...
	$query = sprintf("SELECT 
		...
		notacredito.numeracion_nota_credito,
		notacredito.numeroControl AS numero_control_nota,
		...
		(notacredito.subtotalNotaCredito - notacredito.subtotal_descuento) AS total_neto,
		notacredito.montoNetoNotaCredito AS total,
		(SELECT SUM(cj_cc_nota_credito_iva.subtotal_iva) FROM cj_cc_nota_credito_iva
		WHERE cj_cc_nota_credito_iva.id_nota_credito = notacredito.idNotaCredito) AS total_iva,
	...
	FROM al_contrato_venta contrato
		...
		INNER JOIN cj_cc_notacredito notacredito ON (notacredito.idDocumento = encabezadofactura.idFactura)
		INNER JOIN cj_cc_cliente cliente ON (notacredito.idCliente = cliente.id)
		%s", $sqlBusq);
	...
	$htmlTh .= "<tr align=\"center\" class=\"tituloColumna\">";
		...
		$htmlTh .= ordenarCampo("xajax_listaContratoVenta", "5%", $pageNum, "fechaNotaCredito", $campOrd, $tpOrd, $valBusq, $maxRows, "Fecha NC");
		$htmlTh .= ordenarCampo("xajax_listaContratoVenta", "5%", $pageNum, "numeracion_nota_credito", $campOrd, $tpOrd, $valBusq, $maxRows, "Nro. NC");
		$htmlTh .= ordenarCampo("xajax_listaContratoVenta", "5%", $pageNum, "numero_control_nota", $campOrd, $tpOrd, $valBusq, $maxRows, "Nro. Control NC");
		$htmlTh .= ordenarCampo("xajax_listaContratoVenta", "5%", $pageNum, "numeroFactura", $campOrd, $tpOrd, $valBusq, $maxRows, "Nro. Factura");
		$htmlTh .= ordenarCampo("xajax_listaContratoVenta", "5%", $pageNum, "numeroControl", $campOrd, $tpOrd, $valBusq, $maxRows, "Nro. Control Factura");
		...
		$htmlTh .= ordenarCampo("xajax_listaContratoVenta", "5%", $pageNum, "total", $campOrd, $tpOrd, $valBusq, $maxRows, "Total NC");
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$imgEstatusNota = "<img src=\"../img/iconos/ico_gris.gif\" title=\"Nota de Crédito\"/>";		
		
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			$htmlTb .= "<td>".$imgEstatusNota."</td>";
			$htmlTb .= "<td>".utf8_encode($row['nombre_empresa'])."</td>";
			$htmlTb .= "<td align=\"center\">".date(spanDateFormat, strtotime($row['fechaNotaCredito']))."</td>";
			$htmlTb .= "<td align=\"center\">".$row['numeracion_nota_credito']."</td>";
			$htmlTb .= "<td align=\"center\">".$row['numero_control_nota']."</td>";
			...
			$htmlTb .= "</td>";
			$htmlTb .= "<td>";
				$htmlTb .= sprintf("<img class=\"puntero\" onclick=\"verVentana('reportes/al_devolucion_venta_pdf.php?valBusq=%s', 960, 550);\" src=\"../img/iconos/ico_print.png\" title=\"Nota de Credito PDF\"/>",
					$row['idNotaCredito']);
			$htmlTb .= "</td>";
		$htmlTb .= "</tr>";
	}
