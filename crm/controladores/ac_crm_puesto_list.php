...
function buscarPuesto($valForm) {
	$objResponse = new xajaxResponse();
	
	$valBusq = sprintf("%s",
		$valForm['lstEmpresa']);
	
	$objResponse->loadCommands(listadoPuesto(0, "", "", $valBusq));
	
	return $objResponse;
}

function cargarPuesto($nomObjeto, $idConfiguracionEmpresa) {
	$objResponse = new xajaxResponse();
	
	if (xvalidaAcceso($objResponse,"crm_puesto_list","editar")) {
		$objResponse->script("
		openImg(byId('".$nomObjeto."'));");
		
		$objResponse->script("
		document.forms['frmPuesto'].reset();
		byId('hddIdPuesto').value = '';
		
		byId('txtIdEmpresa').className = 'inputInicial';
		byId('txtNombre').className = 'inputInicial';
		byId('lstEstatus').className = 'inputInicial';");
	
		$query = sprintf("SELECT * FROM crm_puesto WHERE id_puesto = %s;",
			valTpDato($idConfiguracionEmpresa, "int"));
		$rs = mysql_query($query);
		if (!$rs) return $objResponse->alert(mysql_error()."\n\nLine: ".__LINE__);
		$row = mysql_fetch_assoc($rs);
		
		$objResponse->assign("hddIdPuesto","value",$row['id_puesto']);
		$objResponse->loadCommands(asignarEmpresa($row['id_empresa']));
		$objResponse->assign("txtNombre","value",utf8_encode($row['nombre_puesto']));
		$objResponse->call("selectedOption","lstEstatus",$row['activo']);
		
		$objResponse->assign("tdFlotanteTitulo","innerHTML","Editar Puesto");
		$objResponse->script("
...
function eliminarPuesto($idConfiguracionEmpresa, $valFormListaConfiguracion) {
	$objResponse = new xajaxResponse();
	
	if (xvalidaAcceso($objResponse,"crm_puesto_list","eliminar")) {
		mysql_query("START TRANSACTION;");
		
		$deleteSQL = sprintf("UPDATE crm_puesto SET
						activo = 0
						WHERE id_puesto = %s;",
					valTpDato($idConfiguracionEmpresa, "int"));
		...
		$objResponse->loadCommands(listadoPuesto(
...
function formPuesto($nomObjeto) {
	$objResponse = new xajaxResponse();
	
	if (xvalidaAcceso($objResponse,"crm_puesto_list","insertar")) {
		$objResponse->script("
		openImg(byId('".$nomObjeto."'));");
		...
		$objResponse->assign("tdFlotanteTitulo","innerHTML","Agregar Puesto");
		$objResponse->script("
		centrarDiv(byId('divFlotante2'));");
	}
...
function guardarPuesto($valForm, $valFormListaConfiguracion) {
	$objResponse = new xajaxResponse();
	
	mysql_query("START TRANSACTION;");
	
	if ($valForm['hddIdPuesto'] > 0) {
		if (xvalidaAcceso($objResponse,"crm_puesto_list","editar")) {
			$updateSQL = sprintf("UPDATE crm_puesto SET
				id_empresa = %s,
				nombre_puesto = %s,
				activo = %s
			WHERE id_puesto = %s;",
				...
				valTpDato($valForm['hddIdPuesto'], "int"));
		...
		if (xvalidaAcceso($objResponse,"crm_puesto_list","insertar")) {
			$insertSQL = sprintf("INSERT INTO crm_puesto (id_empresa, nombre_puesto, activo)
			VALUE (%s, %s, %s);",
				valTpDato($valForm['txtIdEmpresa'], "int"),
				valTpDato($valForm['txtNombre'], "text"),
				valTpDato($valForm['lstEstatus'], "boolean"));
...
function listadoPuesto($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 15, $totalRows = NULL) {
	$objResponse = new xajaxResponse();
	...
	$query = sprintf("SELECT * FROM crm_puesto fuente %s", $sqlBusq);
	...
	$htmlTh .= "<tr class=\"tituloColumna\">";
		$htmlTh .= "<td></td>";
		$htmlTh .= ordenarCampo("xajax_listadoClavesUsuarios", "100%", $pageNum, "nombre_puesto", $campOrd, $tpOrd, $valBusq, $maxRows, "Nombre");
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			...
			$htmlTb .= "<td align=\"left\">".$row['nombre_puesto']."</td>";
			$htmlTb .= "<td align=\"center\" class=\"noprint\">";
				$htmlTb .= sprintf("<a class=\"modalImg\" id=\"aEditar%s\" rel=\"#divFlotante\" onclick=\"xajax_cargarPuesto(this.id,'%s');\"><img class=\"puntero\" src=\"../img/iconos/ico_edit.png\" title=\"Editar\"/></a>",
					$contFila,
					$row['id_puesto']);
			$htmlTb .= "</td>";
			...
			$htmlTf .= "<tr class=\"tituloCampo\">";
				...
				$htmlTf .= "<td align=\"center\" class=\"tituloColumna\" width=\"210\">";
					...
					$htmlTf .= "<tr align=\"center\">";
						$htmlTf .= "<td width=\"25\">";
						if ($pageNum > 0) {
							$htmlTf .= sprintf("<a class=\"puntero\" onclick=\"xajax_listadoPuesto(%s,'%s','%s','%s',%s);\">%s</a>",
						...
						$htmlTf .= "<td width=\"100\">";
							...
							$htmlTf .= sprintf("<select id=\"pageNum\" name=\"pageNum\" onchange=\"xajax_listadoPuesto(%s,'%s','%s','%s',%s)\">",
...
$xajax->register(XAJAX_FUNCTION,"buscarPuesto");
$xajax->register(XAJAX_FUNCTION,"cargarPuesto");
$xajax->register(XAJAX_FUNCTION,"cargaLstEmpresa");
$xajax->register(XAJAX_FUNCTION,"eliminarPuesto");
$xajax->register(XAJAX_FUNCTION,"formPuesto");
$xajax->register(XAJAX_FUNCTION,"guardarPuesto");
$xajax->register(XAJAX_FUNCTION,"listadoPuesto");
