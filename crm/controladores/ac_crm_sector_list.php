...
function buscarSector($valForm) {
	$objResponse = new xajaxResponse();
	
	$valBusq = sprintf("%s",
		$valForm['lstEmpresa']);
	
	$objResponse->loadCommands(listadoSector(0, "", "", $valBusq));
	
	return $objResponse;
}

function cargarSector($nomObjeto, $idConfiguracionEmpresa) {
	$objResponse = new xajaxResponse();
	
	if (xvalidaAcceso($objResponse,"crm_sector_list","editar")) {
		$objResponse->script("
		openImg(byId('".$nomObjeto."'));");
		
		$objResponse->script("
		document.forms['frmSector'].reset();
		byId('hddIdSector').value = '';
		
		byId('txtIdEmpresa').className = 'inputInicial';
		byId('txtNombre').className = 'inputInicial';
		byId('lstEstatus').className = 'inputInicial';");
	
		$query = sprintf("SELECT * FROM crm_sector WHERE id_sector = %s;",
			valTpDato($idConfiguracionEmpresa, "int"));
		$rs = mysql_query($query);
		if (!$rs) return $objResponse->alert(mysql_error()."\n\nLine: ".__LINE__);
		$row = mysql_fetch_assoc($rs);
		
		$objResponse->assign("hddIdSector","value",$row['id_sector']);
		$objResponse->loadCommands(asignarEmpresa($row['id_empresa']));
		$objResponse->assign("txtNombre","value",utf8_encode($row['nombre_sector']));
		$objResponse->call("selectedOption","lstEstatus",$row['activo']);
		
		$objResponse->assign("tdFlotanteTitulo","innerHTML","Editar Sector");
		$objResponse->script("
...
function eliminarSector($idConfiguracionEmpresa, $valFormListaConfiguracion) {
	$objResponse = new xajaxResponse();
	
	if (xvalidaAcceso($objResponse,"crm_sector_list","eliminar")) {
		mysql_query("START TRANSACTION;");
		
		$deleteSQL = sprintf("UPDATE crm_sector SET
						activo = 0 
						WHERE id_sector = %s;",
					valTpDato($idConfiguracionEmpresa, "int"));
		...
		$objResponse->loadCommands(listadoSector(
...
function formSector($nomObjeto) {
	$objResponse = new xajaxResponse();
	
	if (xvalidaAcceso($objResponse,"crm_sector_list","insertar")) {
		$objResponse->script("
		openImg(byId('".$nomObjeto."'));");
		...
		$objResponse->assign("tdFlotanteTitulo","innerHTML","Agregar Sector");
		$objResponse->script("
		centrarDiv(byId('divFlotante2'));");
	}
...
function guardarSector($valForm, $valFormListaConfiguracion) {
	$objResponse = new xajaxResponse();
	
	mysql_query("START TRANSACTION;");
	
	if ($valForm['hddIdSector'] > 0) {
		if (xvalidaAcceso($objResponse,"crm_sector_list","editar")) {
			$updateSQL = sprintf("UPDATE crm_sector SET
				id_empresa = %s,
				nombre_sector = %s,
				activo = %s
			WHERE id_sector = %s;",
				...
				valTpDato($valForm['hddIdSector'], "int"));
		...
		if (xvalidaAcceso($objResponse,"crm_sector_list","insertar")) {
			$insertSQL = sprintf("INSERT INTO crm_sector (id_empresa, nombre_sector, activo)
			VALUE (%s, %s, %s);",
				valTpDato($valForm['txtIdEmpresa'], "int"),
				valTpDato($valForm['txtNombre'], "text"),
				valTpDato($valForm['lstEstatus'], "boolean"));
...
function listadoSector($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 15, $totalRows = NULL) {
	$objResponse = new xajaxResponse();
	...
	$query = sprintf("SELECT * FROM crm_sector fuente %s", $sqlBusq);
	...
	$htmlTh .= "<tr class=\"tituloColumna\">";
		$htmlTh .= "<td></td>";
		$htmlTh .= ordenarCampo("xajax_listadoClavesUsuarios", "100%", $pageNum, "nombre_sector", $campOrd, $tpOrd, $valBusq, $maxRows, "Nombre");
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			...
			$htmlTb .= "<td align=\"left\">".$row['nombre_sector']."</td>"; //htmlentities()	
			$htmlTb .= "<td align=\"center\" class=\"noprint\">";
				$htmlTb .= sprintf("<a class=\"modalImg\" id=\"aEditar%s\" rel=\"#divFlotante\" onclick=\"xajax_cargarSector(this.id,'%s');\"><img class=\"puntero\" src=\"../img/iconos/ico_edit.png\" title=\"Editar\"/></a>",
					$contFila,
					$row['id_sector']);
			$htmlTb .= "</td>";
			...
			$htmlTf .= "<tr class=\"tituloCampo\">";
				...
				$htmlTf .= "<td align=\"center\" class=\"tituloColumna\" width=\"210\">";
					...
					$htmlTf .= "<tr align=\"center\">";
						$htmlTf .= "<td width=\"25\">";
						if ($pageNum > 0) {
							$htmlTf .= sprintf("<a class=\"puntero\" onclick=\"xajax_listadoSector(%s,'%s','%s','%s',%s);\">%s</a>",
						...
						$htmlTf .= "<td width=\"100\">";
							...
							$htmlTf .= sprintf("<select id=\"pageNum\" name=\"pageNum\" onchange=\"xajax_listadoSector(%s,'%s','%s','%s',%s)\">",
...
$xajax->register(XAJAX_FUNCTION,"buscarSector");
$xajax->register(XAJAX_FUNCTION,"cargarSector");
$xajax->register(XAJAX_FUNCTION,"cargaLstEmpresa");
$xajax->register(XAJAX_FUNCTION,"eliminarSector");
$xajax->register(XAJAX_FUNCTION,"formSector");
$xajax->register(XAJAX_FUNCTION,"guardarSector");
$xajax->register(XAJAX_FUNCTION,"listadoSector");
