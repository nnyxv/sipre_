...
//jafm
function cargaLstEstatus($idEmpresa = ""){
	$objResponse = new xajaxResponse();
	
	//CONSULTA LOS ESTATUS 
	$queryEquipo = sprintf("SELECT id_posibilidad_cierre,nombre_posibilidad_cierre FROM `crm_posibilidad_cierre` WHERE id_empresa = $idEmpresa and activo =1");
	
		$rsEmp = mysql_query($queryEquipo);
		...
		while($rowEmp =  mysql_fetch_array($rsEmp)){
			...
			$htmlOption .= sprintf("<option value=\"%s\">%s.-  %s</option>",
				$rowEmp['id_posibilidad_cierre'],$count,utf8_encode($rowEmp['nombre_posibilidad_cierre']));
		}
			
		$html = "<select id=\"tdLstEstatus\" name=\"tdLstEstatus\" class=\"inputHabilitado\" onchange=\"byId('btnBuscar').click();\">";
...
function listaEmpleado($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 10, $totalRows = NULL) {
	$objResponse = new xajaxResponse();
	/*
	$objResponse->alert("pageNum " . $pageNum,"campOrd " .  $campOrd ,"$tpOrd " . $tpOrd ,"campOrd " . $valBusq ,"maxRows " . $maxRows ,"totalRows " . $totalRows);
	global $spanCI;
	*/
	$valCadBusq = explode("|", $valBusq);
	$startRow = $pageNum * $maxRows;
...
function lstSeguimiento($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 20, $totalRows = NULL) {
	...
	if($valCadBusq[5] != "-1" && $valCadBusq[5]) {
		$cond = (strlen($sqlBusq) > 0) ? " AND " : " WHERE ";
		$sqlBusq .= $cond.sprintf("posibilidad_cierre.id_posibilidad_cierre = %s",
	...
	/*jafm
	$cond = (strlen($sqlBusq) > 0) ? " AND " : " WHERE ";
	$sqlBusq .= $cond.sprintf("perfil_prospecto.estatus = %s AND seguimiento.estatus = %s",
			valTpDato(1, "int"),
			valTpDato(1, "int"));
	*/	
...
$xajax->register(XAJAX_FUNCTION,"cargaLstEstatus");//jafm
