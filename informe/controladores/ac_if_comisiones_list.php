...
//function cargaLstDecimalPDF($frmBuscar){
function cargaLstDecimalPDF($selId = "", $bloquearObj = false){
	$objResponse = new xajaxResponse();
	
	/*$fecha = sprintf("%s", $frmBuscar['txtFecha']);
	...
	if(($mes < '05') && ($anio >= '2018')){	*/
		...
		//$array = array("0" => "Sin Convertir", "1" => "Convertido a MILES");
		
		$array = array("0" => "Sin Convertir", "1" => "Convertido a MILES");
	$totalRows = count($array);
	
	$class = ($bloquearObj == true) ? "" : "class=\"inputHabilitado\"";
	$onChange = ($bloquearObj == true) ? "onchange=\"selectedOption(this.id,'".$selId."');\"" : "onchange=\"xajax_imprimirComisionesMiles(xajax.getFormValues('frmBuscar'));\"";
...
//		return $objResponse;
	...
	/*}else{
		$objResponse->script("xajax_imprimirComisiones(xajax.getFormValues('frmBuscar'))");	
	}*/
...
function formComisionProduccion($frmBuscar) {
	...
	$htmlTh .= "<tr align=\"center\" class=\"tituloColumna\">";
		...
		$htmlTh .= "<td width=\"8%\">Porc. Comisión</td>";
		$htmlTh .= "<td width=\"10%\">% Productividad</td>";
		$htmlTh .= "<td width=\"10%\">UT'S</td>";
		$htmlTh .= "<td width=\"10%\">UT'S Físicas</td>";
		$htmlTh .= "<td width=\"10%\">".cAbrevMoneda."</td>";
	$htmlTh .= "</tr>";
	while ($rowDetalle = mysql_fetch_assoc($rsDetalle)) {
		$clase = (fmod($contFila, 2) == 0) ? "trResaltar4" : "trResaltar5";
		$contFila++;
		
		$className = ($rowDetalle['total_ut_fisica'] > 0) ? "class=\"inputSinFondo\"" : "class=\"inputCompletoHabilitado\"";
		$readOnly = ($rowDetalle['total_ut_fisica'] > 0) ? "readonly=\"readonly\"" : "";
		$style = ($rowDetalle['total_ut_fisica'] > 0) ? "style=\"color:#FFFFFF; text-align:right\"" : " style=\"text-align:right\"";
		
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			$htmlTb .= "<td>".utf8_encode($rowDetalle['nombre_empleado'])."</td>";
			$htmlTb .= "<td>".utf8_encode($rowDetalle['nombre_cargo'])."</td>";
			$htmlTb .= "<td align=\"right\">".number_format($rowDetalle['porcentaje_comision_productividad'], 2, ".", ",")."</td>";
			...
			$htmlTb .= "</td>";
			$htmlTb .= "<td align=\"right\">";
				$htmlTb .= sprintf("<input type=\"text\" id=\"txtUtFacturada%s\" name=\"txtUtFacturada%s\" class=\"inputSinFondo\" readonly=\"readonly\" style=\"color:#FFFFFF\" value=\"%s\"/>",
					$contFila, $contFila, number_format($rowDetalle['total_ut'], 2, ".", ","));
			$htmlTb .= "</td>";
			$htmlTb .= "<td align=\"right\">";
				$htmlTb .= sprintf("<input type=\"text\" id=\"txtUtsFisica%s\" name=\"txtUtsFisica%s\" %s onblur=\"setFormatoRafk(this,2);\" onkeypress=\"return validarSoloNumerosReales(event)\" %s %s value=\"%s\"/>",
					$contFila, $contFila, $className, $readOnly, $style, number_format($rowDetalle['total_ut_fisica'], 2, ".", ","));
			if (!($rowDetalle['total_ut_fisica'] > 0)) {
...
function guardarComisionProduccion($frmComisionProduccion, $frmBuscar) {
	...
	if (isset($arrayObj)) {
		foreach($arrayObj as $indice => $valor) {
			$txtUtFacturada = str_replace(",", "", $frmComisionProduccion['txtUtFacturada'.$valor]);
			$txtUtsFisica = str_replace(",", "", $frmComisionProduccion['txtUtsFisica'.$valor]);
			$porcentajeProductividad = round(($txtUtFacturada / $txtUtsFisica) * 100, 0); // PORCENTAJE DE PRODUCTIVIDAD
			
			if (isset($txtUtsFisica)) {
				$updateSQL = sprintf("UPDATE iv_cierre_mensual_facturacion SET
					total_ut_fisica = %s,
					porcentaje_productividad = %s
				WHERE id_cierre_mensual_facturacion = %s;",
					valTpDato($txtUtsFisica, "real_inglesa"),
...
$valBusq = sprintf("%s|%s|%s|%s|%s|%s",
	...
	//se agrega filtro de fecha, fecha menor a agosto de 2018 debe enviar al pdf el valor 1 para que se mustren los montos en miles en vez de milones. Fecha igual o superior al agosto ya debería mostrar los montos en miles porque ya se reconvirtieron las tablas
	if(($mes < '08') && ($anio >= '2018')){	
		$objResponse->script(sprintf("verVentana('reportes/if_comisiones_pdf.php?valBusq=%s&miles=%s',890,550)", $valBusq)); 	
	}else{
		//$objResponse->script(sprintf("verVentana('reportes/if_comisiones_pdf.php?valBusq=%s',890,550)", $valBusq));
		$objResponse->script(sprintf("verVentana('reportes/if_comisiones_pdf.php?valBusq=%s&miles=%s',890,550)", $valBusq,0)); 	
	}
...
function imprimirComisionesNO($frmBuscar) {
	$objResponse = new xajaxResponse();
...
function imprimirComisionesMiles($frmBuscar) {
	$objResponse = new xajaxResponse();
	...
	$objResponse->alert($frmBuscar['lstDecimalPDF']);
	
	//$objResponse->script(sprintf("verVentana('reportes/if_comisionesmiles_pdf.php?valBusq=%s',890,550)", $valBusq, $frmBuscar['lstDecimalPDF']));
...
function imprimirComisionesResumen($frmBuscar, $frmBuscarCuenta, $idCuentas) {
	$objResponse = new xajaxResponse();
	

$valBusq = sprintf("%s|%s|%s|%s|%s",
		$frmBuscar['lstEmpresa'],
		$frmBuscarCuenta['txtFecha'],
		$frmBuscar['lstCargo'],
		$frmBuscar['lstEmpleado'],
		$frmBuscar['lstModulo']);
...
function listaComision($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 1, $totalRows = NULL) {
	...
	//return $objResponse->alert($queryComision);
	...
	while ($rowComision = mysql_fetch_assoc($rsLimitComision)) {
		$htmlTh .= "<tr align=\"left\" class=\"trResaltar4\" height=\"24\">";
			...
			$htmlTh .= "<td align=\"center\" colspan=\"5\">".$mes[$rowComision['mes_comision']]." ".$rowComision['ano_comision']."</td>";
		$htmlTh .= "</tr>";
		$htmlTh .= "<tr align=\"left\" class=\"trResaltar4\" height=\"24\">";
			$htmlTh .= "<td colspan=\"7\" title=\"Id Empleado: ".$rowComision['id_empleado']."\">".utf8_encode($rowComision['nombre_empleado'])."</td>";
			$htmlTh .= "<td align=\"right\" class=\"tituloCampo\" colspan=\"2\">"."% Productividad:"."</td>";
			$htmlTh .= "<td align=\"right\" colspan=\"5\">".number_format($rowComision['porcentaje_productividad'], 2, ".", ",")."</td>";
		$htmlTh .= "</tr>";
		$htmlTh .= "<tr align=\"left\" class=\"trResaltar4\" height=\"24\">";
		...
		while ($rowDetalle = mysql_fetch_assoc($rsDetalle)) {
			...
			$htmlTb .= "<tr align=\"right\" class=\"".$clase."\" height=\"24\">";
				...
				$htmlTb .= "<td width=\"7%\">";
					$htmlTb .= number_format($signo * $rowDetalle['venta_bruta'], 3, ".", ",");
				$htmlTb .= "</td>";
				$htmlTb .= "<td width=\"7%\">";
					$htmlTb .= number_format($signo * $rowDetalle['monto_descuento'], 3, ".", ",");
				$htmlTb .= "</td>";
				$htmlTb .= "<td width=\"7%\">";
					$htmlTb .= number_format($signo * $rowDetalle['venta_neta'], 3, ".", ",");
				$htmlTb .= "</td>";
				$htmlTb .= "<td width=\"7%\">";
					$htmlTb .= number_format($signo * $rowDetalle['costo_compra'], 3, ".", ",");
				$htmlTb .= "</td>";
				$htmlTb .= "<td width=\"7%\">";
					$htmlTb .= number_format($signo * $rowDetalle['utilidad_bruta'], 3, ".", ",");
				$htmlTb .= "</td>";
				$htmlTb .= "<td width=\"6%\">";
					$htmlTb .= number_format($signo * $rowDetalle['porcentaje_utilidad_venta'], 3, ".", ",");
				$htmlTb .= "</td>";
				$htmlTb .= "<td align=\"center\" width=\"7%\">".date(spanDateFormat, strtotime($rowDetalle['fecha_documento']))."</td>";
				$htmlTb .= "<td align=\"right\" width=\"3%\">";
					$htmlTb .= ($rowDetalle['dias_inventario'] != "") ? $rowDetalle['dias_inventario'] : "-";
				$htmlTb .= "</td>";
				$htmlTb .= "<td width=\"4%\">";
					$htmlTb .= number_format($signo * $rowDetalle['promedio_porcentaje_comision'],3,'.',',');
				$htmlTb .= "</td>";
				$htmlTb .= "<td width=\"6%\">";
					$htmlTb .= number_format($signo * $rowDetalle['monto_comision'], 3, ".", ",");
				$htmlTb .= "</td>";
				$htmlTb .= "<td>";
		...
		if ($arrayTotalRep[1][7] > 0 || $arrayTotalServ[1][7] > 0 || $arrayTotalVehic[1][7] > 0) {
			...
			if ($arrayTotalVehic[1][7] > 0) {
				$htmlTh .= "<tr align=\"right\" class=\"trResaltarTotal2\" height=\"24\">";
					...
					$htmlTh .= "<td>";
						$htmlTh .= "<div style=\"float:left\">"."<img src=\"../img/iconos/ico_vehiculos.gif\" title=\"Vehículos\"/>"."</div>";
						$htmlTh .= "<div style=\"float:right\">".$arrayTotalVehic[1][7]."</div>";
					$htmlTh .= "</td>";
					$htmlTh .= "<td>"."</td>";
					$htmlTh .= "<td>".number_format($arrayTotalVehic[1][0], 3, ".", ",")."</td>";
					$htmlTh .= "<td>".number_format($arrayTotalVehic[1][1], 3, ".", ",")."</td>";
					$htmlTh .= "<td>".number_format($arrayTotalVehic[1][2], 3, ".", ",")."</td>";
					$htmlTh .= "<td>".number_format($arrayTotalVehic[1][3], 3, ".", ",")."</td>";
					$htmlTh .= "<td>".number_format($arrayTotalVehic[1][4], 3, ".", ",")."</td>";
					$htmlTh .= "<td>".number_format($arrayTotalVehic[1][5], 3, ".", ",")."</td>";
					$htmlTh .= "<td colspan=\"3\">&nbsp;</td>";
					$htmlTh .= "<td colspan=\"3\">".number_format($arrayTotalVehic[1][6], 3, ".", ",")."</td>";
				$htmlTh .= "</tr>";
			}
			$htmlTh .= "<tr align=\"right\" class=\"trResaltarTotal\" height=\"24\">";
...
function listaComisionDetalle($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 20, $totalRows = NULL) {
	...
	while ($rowComision = mysql_fetch_assoc($rsLimitComision)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			$htmlTb .= "<td title=\"".$rowComision['id_comision_empleado_detalle']."\">".utf8_encode(substr($rowComision['descripcion_tipo_comision'],0,46))."</td>";
...
function listaCuenta($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 20, $totalRows = NULL){
	...
	while ($rowCuenta = mysql_fetch_assoc($rsLimitCuenta)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\">";
			$htmlTb .= sprintf("<td><button type=\"button\" id=\"btnListCuneta\" onclick=\"ver(xajax.getFormValues('frmBuscar'),xajax.getFormValues('frmBuscarCuenta'),%s);\"><img src=\"../img/iconos/accept.png\"/></button></td>",$rowCuenta['idCuentas']);
