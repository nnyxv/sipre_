...
function listaComision($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 1, $totalRows = NULL) {
	...
	while ($rowComision = mysql_fetch_assoc($rsLimitComision)) {
		...
		$cond = (strlen($sqlBusq2) > 0) ? " AND " : " WHERE ";
		$sqlBusq2 .= $cond.sprintf("DATE_FORMAT(comision_emp.fecha_comision, %s) = %s",
			valTpDato("%m-%Y", "date"),
			valTpDato(date("m-Y", strtotime("01-".$rowComision['mes_comision']."-".$rowComision['ano_comision'])), "date"));
