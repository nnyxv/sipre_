...
while($rowMovDet = mysql_fetch_array($rsTipoMov)){
	...
	if ($rowMovDet['id_tipo_movimiento'] == 1 || $rowMovDet['id_tipo_movimiento'] == 3) {
		...
		switch ($tipoDocumentoMovimiento) { // 1 = Vale Entrada / Salida, 2 = Nota Credito
			case 1 : $totalNetoClaveMovVentasServ += $totalNeto; break;
			case 2 : $totalNetoClaveMovVentasServ -= $totalNeto; break;
		}
	...
	$arrayMovDet[1] = $rowMovDet['clave'].") ".$rowMovDet['descripcion'];
	$arrayMovDet[2] = $totalNeto;
	$arrayMovDet[3] = $tipoDocumentoMovimiento;
...
imagestring($img,1,0,$posY,strtoupper(substr(utf8_decode(cAbrevMoneda." Rptos / OR"),0,32)),$textColor); // <----
...
if (isset($arrayMovVentasServ)) {
	foreach ($arrayMovVentasServ as $indice => $valor) {
		...
		if ($arrayMovVentasServ[$indice][0] == 1 || $arrayMovVentasServ[$indice][0] == 3) {
			...
			switch($arrayMovVentasServ[$indice][3]) { // 1 = Vale Entrada / Salida, 2 = Nota Credito
				case 1 : $arrayFila[] = round($arrayMovVentasServ[$indice][2],2); break;
				case 2 : $arrayFila[] = round((-1) * $arrayMovVentasServ[$indice][2],2); break;
			}
		}
