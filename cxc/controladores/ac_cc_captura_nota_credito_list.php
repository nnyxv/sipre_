...
function editarNotaCredito($idNotaCredito){
	$objResponse = new xajaxResponse();
	
	if (!xvalidaAcceso($objResponse,"cc_captura_nota_credito_list",'editar')) { return $objResponse; }
	
	$objResponse->script("window.location.href='cc_nota_credito_form.php?id=".$idNotaCredito."&acc=4'");
...
function exportarNotaCredito($frmBuscar) {
	...
	$objResponse->script("window.open('reportes/cc_nota_credito_historico_excel.php?valBusq=".$valBusq."','_self');");
...
function imprimirNotaCredito($frmBuscar){
	...
	$objResponse->script(sprintf("verVentana('reportes/cc_nota_credito_historico_pdf.php?valBusq=%s&lstOrientacionPDF=%s',890,550)", $valBusq, $frmBuscar['lstOrientacionPDF']));
...
function listaNotaCredito($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 20, $totalRows = NULL){
	...
	while ($row = mysql_fetch_assoc($rsLimit)){
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			...
			$htmlTb .= "<td ".$rowspan.">";
				$htmlTb .= sprintf("<img class=\"puntero\" onclick=\"xajax_editarNotaCredito(%s);\" src=\"../img/iconos/pencil.png\" title=\"Editar\"/>",
					$row['idNotaCredito']);
			$htmlTb .= "</td>";
			$htmlTb .= "<td ".$rowspan.">";
			// MODIFICADO ERNESTO
...
function nuevaNotaCredito($acc){
	$objResponse = new xajaxResponse();
	
	if (!xvalidaAcceso($objResponse,"cc_captura_nota_credito_list",'insertar')) { return $objResponse; }
	
	$objResponse->script("window.location.href='cc_nota_credito_form.php?acc=".$acc."';");
...
$xajax->register(XAJAX_FUNCTION,"editarNotaCredito");
...
$xajax->register(XAJAX_FUNCTION,"nuevaNotaCredito");
?>