...
function calcularPagos($frmListaDctoPagado, $frmDcto){
    ...
    $txtMontoPago = str_replace(",", "", $frmDcto['txtTotalCheque']); // Viene con formato 0,000.00	
...
function cargaLstTipoCheque($selId = "", $bloquearObj = false){//si es puerto rico, permitir cambio y uso de tipo de cheque suplidor
    $objResponse = new xajaxResponse();
    ...
    if (in_array($rowConfig403['valor'],array(3))) { // 3 = Puerto Rico
		...
		$html .= "<select id=\"lstTipoCheque\" name=\"lstTipoCheque\" ".$class." ".$onChange." style=\"width:99%\">";
        	$html .= "<option value=\"\">[ Seleccione ]</option>";
	...
	$objResponse->assign("tdlstTipoCheque","innerHTML", $html);
...
function cargarDcto($idCheque){
	...
	if ($idCheque > 0) {
		...
		byId('txtObservacionCheque').readOnly = true;
		...
		FROM cj_cc_cheque ch WHERE id_cheque = %s;",
		...
		switch($rowCheque['estado_cheque']) {
		...
		$objResponse->loadCommands(asignarEmpleado($rowCheque['id_empleado_registro'], false));
		
		$objResponse->assign("hddIdCheque","value",$idCheque);
		$objResponse->loadCommands(asignarCliente($rowCheque['id_cliente']));
		$objResponse->loadCommands(cargaLstTipoCheque($rowCheque['tipo_cheque'], true));
		$objResponse->loadCommands(cargaLstModulo($rowCheque['id_departamento'], "", true));
		$objResponse->assign("txtFecha","value",date(spanDateFormat, strtotime($rowCheque['fecha_cheque'])));
		$objResponse->script(sprintf("byId('tdtxtEstatus').className = '%s';", $classEstatus));
		$objResponse->assign("txtEstatus","value",$rowCheque['descripcion_estado_cheque']);
		$objResponse->assign("txtObservacionCheque","value",$rowCheque['observacion_cheque']);
		
		$objResponse->loadCommands(cargaLstBancoCliente("selBancoCliente", $rowCheque['id_banco_cliente'], true));
		$objResponse->assign("txtNumeroCuenta","value",$rowCheque['cuenta_cliente']);
		$objResponse->assign("txtNumeroDctoPago","value",$rowCheque['numero_cheque']);
		$objResponse->assign("txtMontoPago","value",number_format($rowCheque['monto_neto_cheque'], 2, ".", ","));
		...
		if ($totalRowsCheque > 0) {
			...
			$objDcto->idModulo = $rowCheque['id_departamento'];
			$objDcto->idDocumento = $idCheque;
		...
		$objResponse->assign("txtTotalCheque","value",number_format($rowCheque['monto_neto_cheque'], 2, ".", ","));
		$objResponse->assign("txtTotalSaldo","value",number_format($rowCheque['saldo_cheque'], 2, ".", ","));
...
function insertarDctoPagado($frmAnticipoNotaCreditoChequeTransferencia, $frmListaDctoPagado, $frmListaAnticipo){
    ...
    if (str_replace(",", "", $frmListaDctoPagado['txtMontoRestante']) < str_replace(",", "", $frmAnticipoNotaCreditoChequeTransferencia['txtMontoDocumento'])){
        return $objResponse->alert("El monto a pagar no puede ser mayor que el saldo del cheque");
    }
...
$xajax->register(XAJAX_FUNCTION,"cargaLstTipoCheque");
