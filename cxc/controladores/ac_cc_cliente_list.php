...
function exportarCliente($frmBuscar) {
	...
	$objResponse->script("window.open('reportes/cc_cliente_excel.php?valBusq=".$valBusq."','_self');");
...
function formCliente($idCliente, $frmCliente) {
	...
	if ($idCliente > 0) {
		...
		$objResponse->script("selectedOption('lstTipoCliente', '".((strlen($row['tipocliente']) > 0) ? $row['tipocliente'] : "Todos")."');");
		...
		$objResponse->call("selectedOption","lstTipoCliente","Todos");
...
function listaCliente($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 20, $totalRows = NULL) {
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			...
			$htmlTb .= "<td>";
				$htmlTb .= "<div align=\"center\">".utf8_encode($row['telf'])."</div>";
				$htmlTb .= ((strlen($row['correo']) > 0) ? "<div><a class=\"linkAzulUnderline\" href=\"mailto:".utf8_encode($row['correo'])."\">".utf8_encode($row['correo'])."</a></div>" : "");
			$htmlTb .= "</td>";
