...
function editarFactura($idFactura){
	...
	$objResponse->script("window.location.href = 'cc_factura_form.php?id=".$idFactura."&acc=1'");
...
function formImportarFactura($frmImportarArchivo) {
	$objResponse = new xajaxResponse();
	
	if (isset($frmImportarArchivo['cbxFila'])) {
		foreach ($frmImportarArchivo['cbxFila'] as $indice => $valor) {
			$objResponse->script("
...
function importarFactura($frmImportarArchivo, $frmListaFactura) {
	$objResponse = new xajaxResponse();
	
	if (isset($frmImportarArchivo['cbxFila'])) {
		mysql_query("START TRANSACTION;");
		
		foreach ($frmImportarArchivo['cbxFila'] as $indiceFila => $valorFila) {
			$idEmpresa = $frmImportarArchivo['hddIdEmpresa'.$valorFila];
			$idCliente = $frmImportarArchivo['hddIdCliente'.$valorFila];
			$idModulo = $frmImportarArchivo['hddIdModulo'.$valorFila];
			$idEmpleado = $frmImportarArchivo['hddIdEmpleado'.$valorFila];
			$idTipoPago = $frmImportarArchivo['hddTipoPago'.$valorFila];
			$txtDiasCreditoCliente = (strtotime($frmImportarArchivo['hddFechaVencimiento'.$valorFila]) - strtotime($frmImportarArchivo['hddFechaFactura'.$valorFila])) / 86400;
			$estatusFactura = $frmImportarArchivo['hddEstadoFactura'.$valorFila];
			
			$frmImportarArchivo['txtSubTotal'.$valorFila] = $frmImportarArchivo['hddTotalFactura'.$valorFila];
			$frmImportarArchivo['txtDescuento'.$valorFila] = 0;
			$frmImportarArchivo['txtSubTotalDescuento'.$valorFila] = 0;
			...
			$frmImportarArchivo['txtTotalExento'.$valorFila] = $frmImportarArchivo['hddTotalFactura'.$valorFila];
			$frmImportarArchivo['txtTotalExonerado'.$valorFila] = 0;
			
			if ($idEmpresa > 0 && $idCliente > 0 && $idModulo != "" && $idEmpleado > 0) {
				...
				VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
					...
					valTpDato($frmImportarArchivo['hddNumeroFactura'.$valorFila], "text"),
					valTpDato($frmImportarArchivo['hddNumeroControlFactura'.$valorFila], "text"),
					valTpDato(date("Y-m-d", strtotime($frmImportarArchivo['hddFechaFactura'.$valorFila])), "date"),
					valTpDato(date("Y-m-d", strtotime($frmImportarArchivo['hddFechaVencimiento'.$valorFila])), "date"),
					...
					valTpDato($frmImportarArchivo['hddTotalFactura'.$valorFila], "real_inglesa"),
					valTpDato($frmImportarArchivo['hddTotalSaldo'.$valorFila], "real_inglesa"),
					valTpDato($frmImportarArchivo['hddObservacion'.$valorFila], "text"),
					valTpDato($frmImportarArchivo['txtSubTotal'.$valorFila], "real_inglesa"),
					valTpDato(0, "real_inglesa"),
					valTpDato(0, "real_inglesa"),
					valTpDato($frmImportarArchivo['txtDescuento'.$valorFila], "real_inglesa"),
					valTpDato($frmImportarArchivo['txtSubTotalDescuento'.$valorFila], "real_inglesa"),
					...
					valTpDato($frmImportarArchivo['txtTotalExento'.$valorFila], "real_inglesa"),
					valTpDato($frmImportarArchivo['txtTotalExonerado'.$valorFila], "real_inglesa"),
					valTpDato(((in_array(idArrayPais,array(3))) ? 1 : 2), "int"), // Null o 1 = Aprobada, 2 = Aplicada / Cerrada
					valTpDato("NO", "text"),
					valTpDato($frmImportarArchivo['hddAplicaLibro'.$valorFila], "int"), // 0 = No, 1 = Si
					valTpDato($_SESSION['idEmpleadoSysGts'], "int"));
				mysql_query("SET NAMES 'utf8';");
				$Result1 = mysql_query($insertSQL);
				if (!$Result1) {
					if (mysql_errno() == 1062) {
						return $objResponse->alert("Registro Duplicado"."\n\nLine: ".__LINE__." (".mysql_error().")");
					} else {
		...
		$objResponse->alert("Factura(s) Guardada(s) con Éxito");
		
		$objResponse->script("
		byId('btnCancelarImportarArchivo').click();");
		
		$objResponse->loadCommands(listaFactura(
			$frmListaFactura['pageNum'],
			$frmListaFactura['campOrd'],
			$frmListaFactura['tpOrd'],
			$frmListaFactura['valBusq']));
	} else {
...
function listaFactura($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 20, $totalRows = NULL) {
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			...
			$htmlTb .= "<td ".$rowspan.">";
				$htmlTb .= sprintf("<img class=\"puntero\" onclick=\"xajax_editarFactura(%s);\" src=\"../img/iconos/pencil.png\" title=\"Editar\"/>",
					$row['idFactura']);
			$htmlTb .= "</td>";
			$htmlTb .= "<td ".$rowspan.">";
...
function nuevaFactura(){
	...
	$objResponse->script("window.location.href='cc_factura_form.php?acc=0'");
	
	return $objResponse;
}

function vistaPreviaImportarFactura($frmImportarArchivo) {
	$objResponse = new xajaxResponse();
	...
	while ($archivoExcel->getActiveSheet()->getCell('A'.$i)->getValue() != '') {
		if ($itemExcel == true) {
			$arrayFilaImportar[] = array(
				"id_empresa" => $archivoExcel->getActiveSheet()->getCell('A'.$i)->getValue(),
				"id_cliente" => $archivoExcel->getActiveSheet()->getCell('B'.$i)->getValue(),
				"numero_factura" => $archivoExcel->getActiveSheet()->getCell('C'.$i)->getValue(),
				"numero_control" => $archivoExcel->getActiveSheet()->getCell('D'.$i)->getValue(),
				"fecha_registro" => $archivoExcel->getActiveSheet()->getCell('E'.$i)->getValue(),
				"fecha_vencimiento" => $archivoExcel->getActiveSheet()->getCell('F'.$i)->getValue(),
				"id_modulo" => $archivoExcel->getActiveSheet()->getCell('G'.$i)->getValue(),
				"id_empleado" => $archivoExcel->getActiveSheet()->getCell('H'.$i)->getValue(),
				"condicion_pago" => $archivoExcel->getActiveSheet()->getCell('I'.$i)->getValue(),
				"estado_factura" => $archivoExcel->getActiveSheet()->getCell('J'.$i)->getValue(),
				"total_factura" => $archivoExcel->getActiveSheet()->getCell('K'.$i)->getValue(),
				"saldo_factura" => $archivoExcel->getActiveSheet()->getCell('L'.$i)->getValue(),
				"observacion_factura" => $archivoExcel->getActiveSheet()->getCell('M'.$i)->getValue(),
				"aplica_libro" => $archivoExcel->getActiveSheet()->getCell('N'.$i)->getValue());
		}
		
		if (trim(strtoupper($archivoExcel->getActiveSheet()->getCell('A'.$i)->getValue())) == trim(strtoupper("Id Empresa"))
		|| trim(strtoupper(utf8_encode($archivoExcel->getActiveSheet()->getCell('A'.$i)->getValue()))) == trim(strtoupper("Id Empresa"))
		|| trim(strtoupper(utf8_decode($archivoExcel->getActiveSheet()->getCell('A'.$i)->getValue()))) == trim(strtoupper("Id Empresa"))
		|| trim(strtoupper(htmlentities($archivoExcel->getActiveSheet()->getCell('A'.$i)->getValue()))) == trim(strtoupper("Id Empresa"))
		|| trim(strtoupper($archivoExcel->getActiveSheet()->getCell('A'.$i)->getValue())) == trim(strtoupper("Id Empresa"))) {
			$itemExcel = true;
		}
	...
	if (isset($arrayFilaImportar)) {
		foreach ($arrayFilaImportar as $indiceFilaImportar => $valorFilaImportar) {
			$clase = (fmod($contFila, 2) == 0) ? "trResaltarFilaImportar" : "trResaltarFilaImportar";
			$contFila++;
			
			$idEmpresa = $valorFilaImportar['id_empresa'];
			$idCliente = $valorFilaImportar['id_cliente'];
			$idModulo = $valorFilaImportar['id_modulo'];
			$idEmpleado = $valorFilaImportar['id_empleado'];
			...
			// BUSCA LOS DATOS DEL MODULO
			$queryModulo = sprintf("SELECT * FROM pg_modulos WHERE id_modulo = %s;",
				valTpDato($idModulo, "int"));
			$rsModulo = mysql_query($queryModulo);
			if (!$rsModulo) return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
			$totalRowsModulo = mysql_num_rows($rsModulo);
			$rowModulo = mysql_fetch_assoc($rsModulo);
			
			// BUSCA LOS DATOS DEL EMPLEADO
			$queryEmpleado = sprintf("SELECT * FROM vw_pg_empleados vw_pg_empleado WHERE vw_pg_empleado.id_empleado = %s;",
				valTpDato($idEmpleado, "int"));
			...
			$claseEmpresa = ($totalRowsEmp > 0) ? "" : "divMsjError";
			$claseCliente = ($totalRowsCliente > 0) ? "" : "divMsjError";
			$claseModulo = ($totalRowsModulo > 0) ? "" : "divMsjError";
			$claseEmpleado = ($totalRowsEmpleado > 0) ? "" : "divMsjError";
			$claseTipoPago = (in_array($lstTipoPago[$valorFilaImportar['condicion_pago']],$lstTipoPago)) ? "" : "divMsjError";
			$claseEstadoFactura = (in_array($lstEstadoFactura[$valorFilaImportar['estado_factura']],$lstEstadoFactura)) ? "" : "divMsjError";
			$claseAplicaLibro = (in_array($lstAplicaLibro[$valorFilaImportar['aplica_libro']],$lstAplicaLibro)) ? "" : "divMsjError";
			
			// INSERTA EL ARTICULO SIN INJECT
			$objResponse->script(sprintf("$('#trItmPie').before('".
				"<tr id=\"trItm:%s\" align=\"left\" class=\"textoGris_11px %s\">".
					"<td align=\"right\">%s</td>".
					"<td class=\"%s\" nowrap=\"nowrap\">%s".
						"<input type=\"hidden\" id=\"hddIdEmpresa%s\" name=\"hddIdEmpresa%s\" value=\"%s\"></td>".
					"<td class=\"%s\" nowrap=\"nowrap\">%s".
						"<input type=\"hidden\" id=\"hddIdCliente%s\" name=\"hddIdCliente%s\" value=\"%s\"></td>".
					"<td align=\"right\">%s".
						"<input type=\"text\" id=\"hddNumeroFactura%s\" name=\"hddNumeroFactura%s\" style=\"display:none;\" value=\"%s\"></td>".
					"<td align=\"right\">%s".
						"<input type=\"text\" id=\"hddNumeroControlFactura%s\" name=\"hddNumeroControlFactura%s\" style=\"display:none;\" value=\"%s\"></td>".
					"<td align=\"center\">%s".
						"<input type=\"text\" id=\"hddFechaFactura%s\" name=\"hddFechaFactura%s\" style=\"display:none;\" value=\"%s\"></td>".
					"<td align=\"center\">%s".
						"<input type=\"text\" id=\"hddFechaVencimiento%s\" name=\"hddFechaVencimiento%s\" style=\"display:none;\" value=\"%s\"></td>".
					"<td class=\"%s\" nowrap=\"nowrap\">%s".
						"<input type=\"text\" id=\"hddIdModulo%s\" name=\"hddIdModulo%s\" style=\"display:none;\" value=\"%s\"></td>".
					"<td class=\"%s\" nowrap=\"nowrap\">%s".
						"<input type=\"text\" id=\"hddIdEmpleado%s\" name=\"hddIdEmpleado%s\" style=\"display:none;\" value=\"%s\"></td>".
					"<td class=\"%s\" nowrap=\"nowrap\">%s".
						"<input type=\"text\" id=\"hddTipoPago%s\" name=\"hddTipoPago%s\" style=\"display:none;\" value=\"%s\"></td>".
					"<td class=\"%s\" nowrap=\"nowrap\">%s".
						"<input type=\"text\" id=\"hddEstadoFactura%s\" name=\"hddEstadoFactura%s\" style=\"display:none;\" value=\"%s\"></td>".
					"<td align=\"right\">%s".
						"<input type=\"text\" id=\"hddTotalFactura%s\" name=\"hddTotalFactura%s\" style=\"display:none;\" value=\"%s\"></td>".
					"<td align=\"right\">%s".
						"<input type=\"text\" id=\"hddTotalSaldo%s\" name=\"hddTotalSaldo%s\" style=\"display:none;\" value=\"%s\"></td>".
					"<td nowrap=\"nowrap\">%s".
						"<input type=\"text\" id=\"hddObservacion%s\" name=\"hddObservacion%s\" style=\"display:none; text-align:left;\" value=\"%s\">".
					"<td class=\"%s\" nowrap=\"nowrap\">%s".
						"<input type=\"text\" id=\"hddAplicaLibro%s\" name=\"hddAplicaLibro%s\" style=\"display:none; text-align:center;\" value=\"%s\">".
						"<input id=\"cbxFila\" name=\"cbxFila[]\" type=\"checkbox\" checked=\"checked\" style=\"display:none\" value=\"%s\"></td>".
				"</tr>');",
				$contFila, $clase,
					$contFila,
					$claseEmpresa, utf8_encode($idEmpresa.".- ".$rowEmp['nombre_empresa']),
						$contFila, $contFila, $rowEmp['id_empresa'],
					$claseCliente, utf8_encode($idCliente.".- ".$rowCliente['nombre_cliente']),
						$contFila, $contFila, $rowCliente['id_cliente'],
					$valorFilaImportar['numero_factura'],
						$contFila, $contFila, $valorFilaImportar['numero_factura'],
					$valorFilaImportar['numero_control'],
						$contFila, $contFila, $valorFilaImportar['numero_control'],
					date(spanDateFormat, strtotime($valorFilaImportar['fecha_registro'])),
						$contFila, $contFila, date(spanDateFormat, strtotime($valorFilaImportar['fecha_registro'])),
					date(spanDateFormat, strtotime($valorFilaImportar['fecha_vencimiento'])),
						$contFila, $contFila, date(spanDateFormat, strtotime($valorFilaImportar['fecha_vencimiento'])),
					$claseModulo, utf8_encode($idModulo.".- ".$rowModulo['descripcionModulo']),
						$contFila, $contFila, $rowModulo['id_modulo'],
					$claseEmpleado, utf8_encode($idEmpleado.".- ".$rowEmpleado['nombre_empleado']),
						$contFila, $contFila, $rowEmpleado['id_empleado'],
					$claseTipoPago, $valorFilaImportar['condicion_pago'].".- ".$lstTipoPago[$valorFilaImportar['condicion_pago']],
						$contFila, $contFila, $valorFilaImportar['condicion_pago'],
					$claseEstadoFactura, $valorFilaImportar['estado_factura'].".- ".$lstEstadoFactura[$valorFilaImportar['estado_factura']],
						$contFila, $contFila, $valorFilaImportar['estado_factura'],
					number_format($valorFilaImportar['total_factura'], 2, ".", ","),
						$contFila, $contFila, number_format($valorFilaImportar['total_factura'], 2, ".", ","),
					number_format($valorFilaImportar['saldo_factura'], 2, ".", ","),
						$contFila, $contFila, number_format($valorFilaImportar['saldo_factura'], 2, ".", ","),
					$valorFilaImportar['observacion_factura'],
						$contFila, $contFila, $valorFilaImportar['observacion_factura'],
					$claseAplicaLibro, $valorFilaImportar['aplica_libro'].".- ".$lstAplicaLibro[$valorFilaImportar['aplica_libro']],
						$contFila, $contFila, $valorFilaImportar['aplica_libro'],
						$contFila));
		}
	}
...
$xajax->register(XAJAX_FUNCTION,"editarFactura");
$xajax->register(XAJAX_FUNCTION,"exportarFactura");
$xajax->register(XAJAX_FUNCTION,"formImportarFactura");
$xajax->register(XAJAX_FUNCTION,"importarFactura");
$xajax->register(XAJAX_FUNCTION,"imprimirFactura");
$xajax->register(XAJAX_FUNCTION,"listaFactura");
$xajax->register(XAJAX_FUNCTION,"nuevaFactura");
$xajax->register(XAJAX_FUNCTION,"vistaPreviaImportarFactura");
?>