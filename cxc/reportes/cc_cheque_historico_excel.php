...
$objPHPExcel->getActiveSheet()->setCellValue("L".$contFila, "Estado Cheque");
$objPHPExcel->getActiveSheet()->setCellValue("M".$contFila, "Saldo Cheque");
$objPHPExcel->getActiveSheet()->setCellValue("N".$contFila, "Total Cheque");
...
while ($row = mysql_fetch_assoc($rs)) {
	...
	$objPHPExcel->getActiveSheet()->setCellValue("D".$contFila, date(spanDateFormat, strtotime($row['fecha_cheque'])));
	...
	$objPHPExcel->getActiveSheet()->setCellValue("K".$contFila, utf8_encode($row['observacion_cheque']));
	$objPHPExcel->getActiveSheet()->setCellValue("L".$contFila, utf8_encode($row['descripcion_estado_cheque']));
	$objPHPExcel->getActiveSheet()->setCellValue("M".$contFila, $row['saldo_cheque']);
	$objPHPExcel->getActiveSheet()->setCellValue("N".$contFila, $row['monto_neto_cheque']);
