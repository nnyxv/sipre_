...
if ($vista == "print"){
	...
	// USO - PERSONAL
		...
		if ($rowCont['uso_personal'] != null){
			imagestring($img, 2, $posX, $posY+4, "X", $textColor);
		}
	...
	// 4. SUBTOTAL 1+2+3
		...
		$posY += 12;
		imagestring($img,2,275,$posY-1,
			str_pad(substr(utf8_decode(($mostrar == true) ? "4. SUBTOTAL (1+2+3)" : ""),0,30), 30, " ", STR_PAD_RIGHT).
			str_pad(formatoNumero($totalAdiContado), 20, " ", STR_PAD_LEFT),$textColor);
	
	// 5. PAGO INICIAL - TRADE IN
		...
		// F. PAGO INICIAL TOTAL AL CONTADO (D+E)
			$totDEF = $totContado + $rowPagos['pagos_pnd'];
			
			$posY += 11;
			imagestring($img,2,275,$posY-1,
				str_pad(substr(utf8_decode(($mostrar == true) ? "F. PAGO INICIAL TOTAL AL CONTADO (D+E)" : ""),0,30), 30, " ", STR_PAD_RIGHT).
				str_pad(formatoNumero($totDEF), 16, " ", STR_PAD_LEFT),$textColor);
		
		// PAGO INICIAL TOTAL (C+F)	
			if ($sign == '-'){
			  $sumAB = 0;
			}
			...
			$posY += 11;
			imagestring($img,2,275,$posY-1,
				str_pad(substr(utf8_decode(($mostrar == true) ? "PAGO INICIAL TOTAL (C+F)" : ""),0,30), 30, " ", STR_PAD_RIGHT).
				str_pad(formatoNumero($totContadoFinal), 20, " ", STR_PAD_LEFT),$textColor);
		
	// 6. SALDO A FAVOR DEL VENDEDOR POR LOS BIENES Y SERVICIOS ARRIBA INDICADOS (4-5) - ACUMULADO HASTA AHORA
		$saldAFavor1 = $totalAdiContado - $totContadoFinal;
		
		$posY += 22;
		imagestring($img,2,275,$posY-1,
			str_pad(substr(utf8_decode(($mostrar == true) ? "6. SALDO A FAVOR DEL VENDEDOR POR LOS BIENES Y SERVICIOS ARRIBA INDICADOS (4-5)" : ""),0,30), 30, " ", STR_PAD_RIGHT).
	...
	// 10. CANTIDADES PAGADAS A OTROS EN SU NOMBRE 	
		...
		// G. TOTAL DEL SEGURO DEL VEHICULO
			$totalOtrosCargos += $rowCont['monto_seguro'];
			
			$posY += 32;
			imagestring($img,2,275,$posY,
				str_pad(substr(utf8_decode(($mostrar == true) ? "G. TOTAL DEL SEGURO DEL VEHICULO" : ""),0,30), 30, " ", STR_PAD_RIGHT).
				str_pad(formatoNumero($rowCont['monto_seguro']), 16, " ", STR_PAD_LEFT),$textColor);
		
		// H. TOTAL DEL SEGURO PARA GARANTIZAR LA DEUDA
			$totalOtrosCargos += $arrayCreditLife['precio_unitario'];
			
			$posY += 11;
			imagestring($img,2,275,$posY,
				str_pad(substr(utf8_decode(($mostrar == true) ? "H. TOTAL DEL SEGURO PARA GARANTIZAR LA DEUDA" : ""),0,30), 30, " ", STR_PAD_RIGHT).
				str_pad(formatoNumero($arrayCreditLife['precio_unitario']), 16, " ", STR_PAD_LEFT),$textColor);
		
		// I. TOTAL DE OTROS SEGUROS OPCIONALES
			$posY += 11;
			imagestring($img,2,275,$posY,
				str_pad(substr(utf8_decode(($mostrar == true) ? "I. TOTAL DE OTROS SEGUROS OPCIONALES" : ""),0,30), 30, " ", STR_PAD_RIGHT).
				str_pad(formatoNumero(0), 16, " ", STR_PAD_LEFT),$textColor);
		...
		// J. - DESGLOSE GAP
			if (isset($arrayGap)) {
				...
				$posY += 11;
				imagestring($img,2,275,$posY,
					str_pad(substr(utf8_decode(($mostrar == true) ? "J. -" : strtoupper($arrayGap['nom_accesorio'])),0,30), 30, " ", STR_PAD_RIGHT).
					str_pad(formatoNumero($arrayGap['precio_unitario']), 16, " ", STR_PAD_LEFT),$textColor);
			} else {
				$posY += 11;
				imagestring($img,2,275,$posY,
					str_pad(substr(utf8_decode(($mostrar == true) ? "J. -" : "N/A"),0,30), 30, " ", STR_PAD_RIGHT).
					str_pad(formatoNumero(0), 16, " ", STR_PAD_LEFT),$textColor);
			}
		...
		// K. 
			...
			imagestring($img,2,336,$posY,
				str_pad(substr(utf8_decode(($mostrar == true) ? "K. PAGADO A" : "ASSURANT"),0,30), 30, " ", STR_PAD_RIGHT),$textColor);
			...
			imagestring($img,2,311,$posY,
				str_pad(substr(utf8_decode(($mostrar == true) ? "K. POR" : "CONT. SERVICIO"),0,24), 24, " ", STR_PAD_RIGHT).
		...
		// TOTAL DE OTROS CARGOS (7A+...R)
			$posY += 10;
			imagestring($img,2,275,$posY,
				str_pad(substr(utf8_decode(($mostrar == true) ? "TOTAL DE OTROS CARGOS (SUME DE LA LINEA 7A HASTA LA LINEA 7R)" : ""),0,30), 30, " ", STR_PAD_RIGHT).
				str_pad(formatoNumero($totalOtrosCargos), 20, " ", STR_PAD_LEFT),$textColor);
		
	// 8. CANTIDAD FINANCIADA (6+7)
		$cantidadFinanciada = $saldAFavor1 + $totalOtrosCargos;
		
		$posY += 12;
		imagestring($img,2,275,$posY,
			str_pad(substr(utf8_decode(($mostrar == true) ? "8. CANTIDAD FINANCIADA (6+7)" : ""),0,30), 30, " ", STR_PAD_RIGHT).
	...
	// 1.
		if ($arrayContServ['nom_accesorio'] != null){
			...
			imagestring($img,2,290,$posY,
				str_pad(substr(utf8_decode(($mostrar == true) ? "1. -" : "CONT. SERVICIO"),0,30), 30, " ", STR_PAD_RIGHT).
				str_pad(formatoNumero($arrayContServ['precio_unitario']), 17, " ", STR_PAD_LEFT),$textColor);
			
			$posY += 11;
			imagestring($img,2,290,$posY,
				str_pad(substr(utf8_decode(($mostrar == true) ? "DED." : ""),0,6), 6, " ", STR_PAD_RIGHT).
				str_pad(formatoNumero($arrayContServ['ded_cont_serv']), 9, " ", STR_PAD_LEFT).
				str_pad(substr(utf8_decode(($mostrar == true) ? "TERM." : ""),0,6), 6, " ", STR_PAD_RIGHT).
				str_pad($arrayContServ['per_cont_serv'], 5, " ", STR_PAD_LEFT),$textColor);
		} else {
			$posY = 28;
			imagestring($img,2,290,$posY,
				str_pad(substr(utf8_decode(($mostrar == true) ? "1. -" : "CONT. SERVICIO"),0,30), 30, " ", STR_PAD_RIGHT).
			...
			imagestring($img,2,290,$posY,
				...
				str_pad(substr(utf8_decode(($mostrar == true) ? "TERM." : ""),0,6), 6, " ", STR_PAD_RIGHT).
				str_pad((0), 5, " ", STR_PAD_LEFT),$textColor);
		}
	...
	// 2.
		$posY += 11;
		/*imagestring($img,2,290,$posY,
			str_pad(substr(utf8_decode(($mostrar == true) ? "2. -" : ""),0,30), 30, " ", STR_PAD_RIGHT).
			str_pad(formatoNumero(0), 17, " ", STR_PAD_LEFT),$textColor);*/
	...
	// 3.
		$posY += 11;
		/*imagestring($img,2,290,$posY,
			str_pad(substr(utf8_decode(($mostrar == true) ? "3. -" : ""),0,30), 30, " ", STR_PAD_RIGHT).
			str_pad(formatoNumero(0), 17, " ", STR_PAD_LEFT),$textColor);*/
