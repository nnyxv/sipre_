...
if ($valCadBusq[4] != "-1" && $valCadBusq[4] != "") {
	$rs = mysql_query(sprintf("SELECT * FROM pg_modulos WHERE descripcionModulo IN (%s);", valTpDato("'".str_replace(",","','",$valCadBusq[4])."'", "defined", "'".str_replace(",","','",$valCadBusq[4])."'")));
	if (!$rs) die(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
	while ($row = mysql_fetch_assoc($rs)) {
...
if ($valCadBusq[5] != "-1" && $valCadBusq[5] != "") {
	$lstTipoCuentaCliente = array(1 => "Prospecto", 2 => "Prospecto Aprobado (Cliente Venta)", 3 => "Sin Prospectación (Cliente Post-Venta)");
	foreach ($lstTipoCuentaCliente as $indice => $valor) {
		if (in_array($indice, explode(",", $valCadBusq[5]))) {
			$arrayTipoCuentaCliente[] = $valor;
		}
	}
	$arrayCriterioBusqueda[] = "Ver: ".((isset($arrayTipoCuentaCliente)) ? implode(", ", $arrayTipoCuentaCliente) : "");
}

if ($valCadBusq[6] != "-1" && $valCadBusq[6] != "") {
	$rs = mysql_query(sprintf("SELECT * FROM vw_pg_empleados vw_pg_empleado WHERE vw_pg_empleado.id_empleado IN (%s);", valTpDato($valCadBusq[6], "campo")));
	if (!$rs) die(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
	while ($row = mysql_fetch_assoc($rs)) {
		$arrayVendedor[] = utf8_encode($row['nombre_empleado']);
	}
...
$objPHPExcel->getActiveSheet()->setCellValue("K".$contFila, "Compañia");
$objPHPExcel->getActiveSheet()->setCellValue("L".$contFila, "Cant. Modelos");
$objPHPExcel->getActiveSheet()->setCellValue("M".$contFila, "Paga Impuesto");
$objPHPExcel->getActiveSheet()->setCellValue("N".$contFila, "Tipo");
$objPHPExcel->getActiveSheet()->setCellValue("O".$contFila, "Tipo de Pago");
...
while ($row = mysql_fetch_assoc($rs)) {
	...
	$objPHPExcel->getActiveSheet()->setCellValue("K".$contFila, $row['compania']);
	$objPHPExcel->getActiveSheet()->setCellValue("L".$contFila, $row['cantidad_modelos']);
	$objPHPExcel->getActiveSheet()->setCellValue("M".$contFila, (($row['paga_impuesto'] == 1) ? "SI" : "NO"));
	$objPHPExcel->getActiveSheet()->setCellValue("N".$contFila, $row['tipo']);
	$objPHPExcel->getActiveSheet()->setCellValue("O".$contFila, $arrayTipoPago[strtoupper($row['credito'])]);
...
cabeceraExcel($objPHPExcel, $idEmpresa, "O", true, ((isset($arrayCriterioBusqueda)) ? 9 : 8));
...
if (isset($arrayCriterioBusqueda)) {
	...
	$objPHPExcel->getActiveSheet()->mergeCells("A9:O9");
}
