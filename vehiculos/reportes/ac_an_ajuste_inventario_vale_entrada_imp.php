<?php
function cargarValeEntrada($idValeEntrada) {
	$objResponse = new xajaxResponse();
	
	$queryValeEntrada = sprintf("SELECT 
		an_vale_entrada.id_vale_entrada,
		an_vale_entrada.numeracion_vale_entrada,
		an_vale_entrada.fecha,
		an_vale_entrada.id_unidad_fisica,
		an_vale_entrada.id_cliente,
		cj_cc_cliente.nombre,
		cj_cc_cliente.apellido,
		cj_cc_cliente.lci,
		cj_cc_cliente.ci,
		cj_cc_cliente.direccion,
		an_vale_entrada.subtotal_factura,
		an_vale_entrada.observacion
	FROM an_vale_entrada
		INNER JOIN cj_cc_cliente ON (an_vale_entrada.id_cliente = cj_cc_cliente.id) WHERE id_vale_entrada = %s",
	...
	FROM an_unidad_fisica uni_fis
	 ...
	 WHERE uni_fis.id_unidad_fisica = %s",
		valTpDato($rowValeEntrada['id_unidad_fisica'],"int"));
	...
	$objResponse->assign("tdIdValeEntrada","innerHTML",strtoupper($rowValeEntrada['id_vale_entrada']));
	$objResponse->assign("tdFechaEmision","innerHTML",date(spanDateFormat, strtotime($rowValeEntrada['fecha'])));
	$objResponse->assign("tdIdPedidoCompra","innerHTML",strtoupper($rowValeEntrada['id_pedido_compra']));
	$objResponse->assign("tdNombreCliente","innerHTML",strtoupper(utf8_encode($rowValeEntrada['nombre']." ".$rowValeEntrada['apellido'])));
	$objResponse->assign("tdRifCliente","innerHTML",strtoupper($rowValeEntrada['lci']."-".$rowValeEntrada['ci']));
	$objResponse->assign("tdDireccionCliente","innerHTML",strtoupper(utf8_encode($rowValeEntrada['direccion'])));
	...
	$objResponse->assign("tdVersion","innerHTML",strtoupper($rowUnidadBasica['nom_version']));
	...
	$objResponse->assign("tdMontoVehiculo","innerHTML",number_format($rowValeEntrada['subtotal_factura'], 2, ".", ","));
	
	$objResponse->assign("tdSubTotal","innerHTML",number_format($rowValeEntrada['subtotal_factura'], 2, ".", ","));
	$objResponse->assign("tdTotal","innerHTML",number_format($rowValeEntrada['subtotal_factura'], 2, ".", ","));
	
	$objResponse->assign("tdObservacion","innerHTML",strtoupper(utf8_encode($rowValeEntrada['observacion'])));
		
	return $objResponse;
}

$xajax->register(XAJAX_FUNCTION,"cargarValeEntrada");
?>