...
while ($row = mysql_fetch_assoc($rs)) {
	...
	while($rowDetalle = mysql_fetch_assoc($rsDetalle)) {
		...
		$arrayTotal['costo_unitario'] += $rowDetalle['costo_unitario'];
	}
	$subtotalMontoUtilidad = $arrayTotal['precio_unitario'] - $arrayTotal['costo_unitario'];
	$subtotalPorcUtilidad = $subtotalMontoUtilidad * 100 / $arrayTotal['precio_unitario'];
	
	$arrayFila[] = array(
		'linea' => "-");
	$arrayFila[] = array(
		'cant_items' => $totalRowsDetalle,
		'subtotal_precio' => $arrayTotal['precio_unitario']);
	$arrayFila[] = array(
		'subtotal_costo' => $arrayTotal['costo_unitario']);
	$arrayFila[] = array(
	...
	if ($contFila == $totalRows) {
		$totalMontoUtilidad = $arrayTotalFinal['precio_unitario'] - $arrayTotalFinal['costo_unitario'];
		$totalPorcUtilidad = $totalMontoUtilidad * 100 / $arrayTotalFinal['precio_unitario'];
		
		$arrayFila[] = array(
			'total_cant_unidades' => $arrayTotalFinal['cant_unidades'],
			'total_precio_unitario' => $arrayTotalFinal['precio_unitario']);
		
		$arrayFila[] = array(
			'total_subtotal_costo' => $arrayTotalFinal['costo_unitario']);
			...
			if (fmod($contFilaY, $maxRows) == 1) {
				...
				imagefilledrectangle($img, 0, $posY-4, 569, $posY+4+27, $backgroundGris);
					imagestring($img,1,0,$posY,
						...
						strtoupper(str_pad(substr(utf8_decode("FECHA VENT."),0,10), 10, " ", STR_PAD_BOTH))." ".
						strtoupper(str_pad(substr(utf8_decode("NRO FACT"),0,8), 8, " ", STR_PAD_BOTH))." ".
					...
					imagestring($img,1,0,$posY,
						...
						strtoupper(str_pad(substr(utf8_decode("VENTA"),0,8), 8, " ", STR_PAD_BOTH))." ".
						strtoupper(str_pad(substr(utf8_decode("CLIENTE"),0,16), 16, " ", STR_PAD_BOTH))." ".
						strtoupper(str_pad(substr(utf8_decode("PRECIO"),0,14), 14, " ", STR_PAD_BOTH)),$textColor);
					$posY += 9;
					imagestring($img,1,0,$posY,
						strtoupper(str_pad(substr((""),0,6), 6, " ", STR_PAD_BOTH))." ".
						...
						strtoupper(str_pad(substr((""),0,10), 10, " ", STR_PAD_BOTH))." ".
						strtoupper(str_pad(substr((""),0,8), 8, " ", STR_PAD_BOTH))." ".
						strtoupper(str_pad(substr(utf8_decode("VENDEDOR"),0,16), 16, " ", STR_PAD_BOTH))." ".
			...
			if (strlen($arrayFila[$indice]['vehiculo']) > 0) {
				...
				imagefilledrectangle($img, 0, $posY, 569, $posY+9, ((fmod($contFila2, 2) == 0) ? $backgroundAzul : ""));
					imagestring($img,1,0,$posY,
						...
						strtoupper(str_pad(substr((date(spanDateFormat,strtotime($arrayFila[$indice]['fechaRegistroFactura']))),0,10), 10, " ", STR_PAD_BOTH))." ".
						strtoupper(str_pad(substr(($arrayFila[$indice]['numeroFactura']),0,8), 8, " ", STR_PAD_LEFT))." ".
				...
				imagefilledrectangle($img, 0, $posY, 569, $posY+9, ((fmod($contFila2, 2) == 0) ? $backgroundAzul : ""));
					imagestring($img,1,0,$posY,
						...
						strtoupper(str_pad(substr((""),0,10), 10, " ", STR_PAD_BOTH))." ".
						strtoupper(str_pad(substr((""),0,8), 8, " ", STR_PAD_LEFT))." ".
						strtoupper(str_pad(substr(($arrayFila[$indice]['nombre_cliente']),0,16), 16, " ", STR_PAD_RIGHT))." ".
						strtoupper(str_pad(substr((number_format($arrayFila[$indice]['precio_unitario'], 2, ".", ",")),0,14), 14, " ", STR_PAD_LEFT)),$textColor);
				...
				imagefilledrectangle($img, 0, $posY, 569, $posY+9, ((fmod($contFila2, 2) == 0) ? $backgroundAzul : ""));
					imagestring($img,1,0,$posY,
						...
						strtoupper(str_pad(substr((""),0,10), 10, " ", STR_PAD_BOTH))." ".
						strtoupper(str_pad(substr((""),0,8), 8, " ", STR_PAD_LEFT))." ".
						strtoupper(str_pad(substr(($arrayFila[$indice]['nombre_empleado']),0,16), 16, " ", STR_PAD_RIGHT))." ".
						strtoupper(str_pad(substr((number_format($arrayFila[$indice]['costo_unitario'], 2, ".", ",")),0,14), 14, " ", STR_PAD_LEFT)),$textColor);
				...
				imagefilledrectangle($img, 415, $posY, 569, $posY+9, $backgroundVerde);
					imagestring($img,1,0,$posY,
						...
						str_pad(number_format($arrayFila[$indice]['subtotal_precio'], 2, ".", ","), 15, " ", STR_PAD_LEFT),$textColor);
				...
				imagefilledrectangle($img, 415, $posY, 569, $posY+9, $backgroundVerde);
					imagestring($img,1,0,$posY,
						str_pad("", 83, " ", STR_PAD_LEFT).
						str_pad("", 31, $arrayFila[$indice]['linea_subtotal_utilidad'], STR_PAD_LEFT),$textColor);
				...
				imagefilledrectangle($img, 415, $posY, 569, $posY+9, $backgroundVerde);
					imagestring($img,1,0,$posY,
						str_pad("%UTL / UTL ".utf8_encode($row['nom_uni_bas']).":", 83, " ", STR_PAD_LEFT).
						str_pad(number_format($arrayFila[$indice]['porcentaje_utilidad'], 2, ".", ",")."%", 16, " ", STR_PAD_LEFT).
						str_pad(number_format($arrayFila[$indice]['monto_utilidad'], 2, ".", ","), 15, " ", STR_PAD_LEFT),$textColor);
				...
				imagefilledrectangle($img, 415, $posY, 569, $posY+9, $backgroundNaranja);
					imagestring($img,1,0,$posY,
						...
						str_pad(number_format($arrayFila[$indice]['total_precio_unitario'], 2, ".", ","), 15, " ", STR_PAD_LEFT),$textColor);
				...
				imagefilledrectangle($img, 415, $posY, 569, $posY+9, $backgroundNaranja);
					imagestring($img,1,0,$posY,
						str_pad("", 99, " ", STR_PAD_LEFT).
						str_pad(number_format($arrayFila[$indice]['total_subtotal_costo'], 2, ".", ","), 15, " ", STR_PAD_LEFT),$textColor);
				...
				imagefilledrectangle($img, 415, $posY, 569, $posY+9, $backgroundNaranja);
					imagestring($img,1,0,$posY,
						str_pad("", 83, " ", STR_PAD_LEFT).
						str_pad("", 31, $arrayFila[$indice]['linea_total_utilidad'], STR_PAD_LEFT),$textColor);
				...
				imagefilledrectangle($img, 415, $posY, 569, $posY+9, $backgroundNaranja);
					imagestring($img,1,0,$posY,
						str_pad("TOTAL %UTL / UTL:", 83, " ", STR_PAD_LEFT).
						str_pad(number_format($arrayFila[$indice]['total_porcentaje_utilidad'], 2, ".", ",")."%", 16, " ", STR_PAD_LEFT).
						str_pad(number_format($arrayFila[$indice]['total_monto_utilidad'], 2, ".", ","), 15, " ", STR_PAD_LEFT),$textColor);
			}
