...
if ($totalRows > 0) {
	...
	while ($contFila<1) {
		...
		/*	//if ($idEmpresa != "") {*/
			...
			$pdf->Cell(562,5,"".$rowEncabezado['nombre_combo']."",0,0,'C');
		...
		// DETALLE DEL COMBO
			$queryDetalle = sprintf("SELECT *
										FROM
											an_combo_detalle
											INNER JOIN an_accesorio ON (an_accesorio.id_accesorio = an_combo_detalle.id_accesorio)											
										WHERE
											id_combo = %s", $idCombo);
			...
			while ($rowDetalle = mysql_fetch_assoc($rsDetalle)){
				...
				$cantidadSolicitada = $rowDetalle['precio_accesorio'];
				$precioUnitario = ($iva * $cantidadSolicitada) / 100;
				$preciocantidad = ($iva * $cantidadSolicitada) / 100 + $cantidadSolicitada;
			...
			$pdf->Cell(60,14,number_format($rowEncabezado['total_sin_iva'],2,".",","),1,0,'R',true);
			$pdf->Ln();
			...
			$pdf->Cell(60,14,number_format($rowEncabezado['total_iva'],2,".",","),1,0,'R',true);
			$pdf->Ln();
			...
			$pdf->Cell(60,14,number_format($rowEncabezado['total_con_iva'],2,".",","),1,0,'R',true);
