...
if(!(validaAcceso("an_informe_inventario_serial_list"))) {
...
$sqlunidad = "SELECT
	...
	date_format(fact_comp.fecha_origen,'%m') as mes_fecha,
	date_format(fact_comp.fecha_origen,'%Y') as a_fecha,
...
WHERE uni_fis.estado_venta NOT IN ('VENDIDO')
	AND (fact_comp.fecha_origen <= str_to_date('".$ano."-".$mes."-".$umes."','%Y-%m-%d')
		OR fact_comp.fecha_origen IS NULL)
	AND unidad_emp.id_empresa = ".$idEmpresa."
	AND vw_iv_modelo.catalogo = 1
ORDER BY fact_comp.fecha_origen ASC, vw_iv_modelo.nom_modelo ASC;";
...
<head>
    ...
    <title>.: SIPRE <?php echo cVERSION; ?> :. Vehículos - Informe de Inventario Final por Serial</title>
...
<div id="divGeneralPorcentaje">
    ...
    <tr>
        <td class="tituloPaginaVehiculos">Informe de Inventario Final por Serial<br/><span class="texto_10px">(Inventario Mensual <?php echo $arrayMes[$mes].' '.$ano; ?>)</span></td>
    </tr>
    <tr>
        <td>&nbsp;</td>
    </tr>
    ...
    <tr>
    	<td>
            ...
            <?php 
            if (count($unidades) == 0) {
				echo "<tr height=\"24\"><td class=\"divMsjError\" colspan=\"9\">No se han registrado unidades restantes para este mes</td></tr>";
            } else {
                foreach ($unidades as $unidad) {
					$clase = (fmod($contFila, 2) == 0) ? "trResaltar4" : "trResaltar5";
                    $contFila++;
					
                    if ($mes_actual1 != $unidad['mes_fecha']) {
                        $mes_actual1 = $unidad['mes_fecha'];
                        $contFila = 1;
                        echo '<tr height="24">
                            <td align="left" class="tituloCampo" colspan="9">INVENTARIO RESTANTE ASIGNACI&Oacute;N DE '.$arrayMes[intval($unidad['mes_fecha'])].', '.$unidad['a_fecha'].'</td>
                        </tr>';
                    } ?>
            <tr align="left" class="<?php echo $clase; ?>" height="24">
                ...
                <td align="center"><?php echo $arrayMes[intval($unidad['mes_fecha'])].', ',$unidad['a_fecha'] ?></td>
