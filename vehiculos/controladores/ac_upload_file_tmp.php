...
if (window.parent.document.getElementById('hddUrlArchivo').value == '' && window.parent.document.getElementById('fleUrlArchivo').value.length > 0) {
	...
	if (eval((typeof('window.parent.xajax_visualizarImportacion') != "undefined")) && typeof('window.parent.xajax_visualizarImportacion') != "function" && !(window.parent.xajax_visualizarImportacion)) {
	} else {
		window.parent.xajax_visualizarImportacion(window.parent.xajax.getFormValues('frmImportarAlmacen'), window.parent.xajax.getFormValues('frmListaAlmacen'));
	}
}
</script>