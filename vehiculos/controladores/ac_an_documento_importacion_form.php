...
function calcularFacturaGasto($frmFacturaGasto) {
	...
	for ($cont = 0; isset($frmFacturaGasto['txtIvaFacturaGasto'.$cont]); $cont++) {
		$objResponse->script("
		fila = document.getElementById('trIvaFacturaGasto:".$cont."');
	...
	if (isset($arrayIva)) {
		foreach ($arrayIva as $indice => $valor) {
			$queryIva = sprintf("SELECT iva.*, IF (iva.tipo IN (3,2), 1, NULL) AS lujo FROM pg_iva iva WHERE iva.idIva = %s;", valTpDato($arrayIva[$indice][0], "int"));
			$rsIva = mysql_query($queryIva);
			if (!$rsIva) return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
			$rowIva = mysql_fetch_assoc($rsIva);
						
			if ($arrayIva[$indice][2] > 0) {
				...
				var elemento = '".
					...
					$indice,
						$indice, utf8_encode($rowIva['observacion']),
							$indice, $indice, $arrayIva[$indice][0],
						$indice, $indice, number_format(round($arrayIva[$indice][1],2), 2, ".", ","),
						$indice, $indice, $rowIva['iva'], "%",
						$indice, $indice, number_format(round($arrayIva[$indice][2],2), 2, ".", ","),
					
					$indice));
			}
			
			$subTotalIva += doubleval($arrayIva[$indice][2]);
		}
	}
...
function eliminarArticuloFacturaGasto($trItmArticuloFacturaGasto, $frmFacturaGasto) {
	...
	if (isset($trItmArticuloFacturaGasto) && $trItmArticuloFacturaGasto > 0) {
		$objResponse->script("
		fila = document.getElementById('trItmArticuloFacturaGasto:".$trItmArticuloFacturaGasto."');
		padre = fila.parentNode;
		padre.removeChild(fila);");
...
function eliminarFacturaGasto($trItmFacturaGasto, $frmFacturaGasto) {
	...
	if (isset($trItmFacturaGasto) && $trItmFacturaGasto > 0) {
		$objResponse->script("
		fila = document.getElementById('trItmFacturaGasto:".$trItmFacturaGasto."');
		padre = fila.parentNode;
		padre.removeChild(fila);");
...
function guardarFacturaGasto($frmFacturaGasto) {
	...
	if (!xvalidaAcceso($objResponse,"an_documento_importacion_list","insertar")) { return $objResponse; }
	...
	if ($sinAlmacen == false) {
		...
		if (isset($arrayObj)) {
			foreach ($arrayObj as $indice => $valor) {
				...
				if (!$rsArtEmp) { errorGuardarFacturaGasto($objResponse); return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__); }
				...
				if (!$rsArtAlmacen) { errorGuardarFacturaGasto($objResponse); return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__); }
				...
				$queryPrecio = sprintf("SELECT * FROM pg_precios WHERE id_precio NOT IN (6,7) AND estatus = 1 ORDER BY id_precio ASC;");
				$rsPrecio = mysql_query($queryPrecio);
				if (!$rsPrecio) { errorGuardarFacturaGasto($objResponse); return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__); }
				while ($rowPrecio = mysql_fetch_assoc($rsPrecio)) {
					...
					if (!$rsArtPrecio) { errorGuardarFacturaGasto($objResponse); return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__); }
		...
		$objResponse->script("window.location.href='an_documento_importacion_list.php';");
...
function insertarArticuloFacturaGasto($idArticulo, $frmFacturaGasto) {
	...
	for ($cont = 1; $cont <= 20; $cont++) {
		$objResponse->script(sprintf("byId('btnInsertarArticuloFacturaGasto%s').disabled = false;",
			$cont));
	}
...
function insertarFacturaGasto($idFacturaGasto, $frmFacturaGasto) {
	...
	// DESBLOQUEA LOS BOTONES DEL LISTADO
	for ($cont = 1; $cont <= 20; $cont++) {
		$objResponse->script(sprintf("byId('btnInsertarFacturaGasto%s').disabled = false;",
			$cont));
	}
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			$htmlTb .= "<td>";
				$htmlTb .= sprintf("<button type=\"button\" id=\"btnInsertarArticuloFacturaGasto%s\" onclick=\"validarInsertarArticuloFacturaGasto('%s');\" title=\"Seleccionar\"><img src=\"../img/iconos/tick.png\"/></button>",
					...
					$row['id_articulo']);
			$htmlTb .= "</td>";
			$htmlTb .= "<td align=\"right\">".utf8_encode(elimCaracter($row['codigo_articulo'],";"))."</td>";
...
function errorGuardarFacturaGasto($objResponse) {
	$objResponse->script("
	byId('btnAceptarFacturaGasto').disabled = false;
