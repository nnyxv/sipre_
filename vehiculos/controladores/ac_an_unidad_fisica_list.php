...
function formUnidadFisica($idUnidadFisica, $accionFormulario) {
	...
	if ($idUnidadFisica > 0) {
		...
		$objResponse->loadCommands(cargaLstAno($row['id_ano'], true));
		$objResponse->loadCommands(cargaLstCondicion($row['id_condicion_unidad'], (($accionFormulario == "editar") ? false : true)));
		$objResponse->loadCommands(cargaLstTipoTablilla($row['tipo_placa'], (($accionFormulario == "editar") ? false : true)));
		$objResponse->assign("txtKilometraje", "value", number_format($row['kilometraje'],0));
		
		$objResponse->loadCommands(cargaLstColor("lstColorExterno1", $row['id_color_externo'], (($accionFormulario == "editar") ? false : true)));
		$objResponse->loadCommands(cargaLstColor("lstColorInterno1", $row['id_color_interno'], (($accionFormulario == "editar") ? false : true)));
		$objResponse->loadCommands(cargaLstColor("lstColorExterno2", $row['id_color_externo2'], (($accionFormulario == "editar") ? false : true)));
		$objResponse->loadCommands(cargaLstColor("lstColorInterno2", $row['id_color_interno2'], (($accionFormulario == "editar") ? false : true)));
	...
	while ($rowUniFisAgregado = mysql_fetch_array($rsUniFisAgregado)) {
		...
		$objResponse->script(sprintf("$('#trItmPie').before('".
			...
			$contFila, $clase,
				...
				"Registrado por: ".utf8_encode($rowUniFisAgregado['nombre_empleado_registro']), date(spanDateFormat, strtotime($fechaDcto)),
				$classUniFisAgregado,
					"100%",
						$aVerDcto,
						$imgDctoModulo,
...
function guardarUnidadFisica($frmUnidadFisica, $frmListaUnidadFisica) {
	...
	$txtFechaExpiracionMarbete = ($frmUnidadFisica['txtFechaExpiracionMarbete'] != "") ? date("Y-m-d", strtotime($frmUnidadFisica['txtFechaExpiracionMarbete'])) : "";
	...
	WHERE id_unidad_fisica = %s;",
		...
		valTpDato($frmUnidadFisica['txtSerialMotor'], "text"),
		valTpDato($frmUnidadFisica['txtNumeroVehiculo'], "text"),
		valTpDato($frmUnidadFisica['txtTituloVehiculo'], "text"),
		valTpDato($frmUnidadFisica['txtPlaca'], "text"),
		valTpDato($frmUnidadFisica['lstTipoTablilla'], "text"),
...
function listaTradeInAuditoria($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 20, $totalRows = NULL) {
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			...
			$htmlTb .= "<td align=\"center\" nowrap=\"nowrap\"".((strlen($row['nombre_empleado']) > 0) ? "title=\"Registrado por: ".utf8_encode($row['nombre_empleado'])."\"" : "").">".date(spanDateFormat." h:i:s a", strtotime($row['tiempo_registro']))."</td>";
