...
function cargaLstEmpleado($selId = "", $nombreObjeto = "", $objetoDestino = "") {
	...
	WHERE fact_vent.idDepartamentoOrigenFactura IN (0,1)
...
function exportarVentaCliente($frmBuscar) {
	...
	$objResponse->script("window.open('reportes/an_venta_cliente_unidad_excel.php?valBusq=".$valBusq."','_self');");
...
function listaVentaCliente($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 20, $totalRows = NULL){
	...
	FROM cj_cc_encabezadofactura fact_vent
		...
		LEFT JOIN an_presupuesto pres_vent ON (ped_vent.id_presupuesto = pres_vent.id_presupuesto) %s", $sqlBusq);
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		FROM cj_cc_encabezadofactura fact_vent
			...
			INNER JOIN vw_iv_modelos vw_iv_modelo ON (uni_fis.id_uni_bas = vw_iv_modelo.id_uni_bas) %s %s", $sqlBusq, $sqlBusq2);
		...
		if ($totalRowsFacturaVenta > 0) {
			...
			$htmlTb .= "<tr align=\"center\" class=\"tituloColumna\">";
				$htmlTb .= "<td width=\"10%\">"."Unidad Básica"."</td>";
				...
				$htmlTb .= "<td width=\"6%\">"."Nro. Presupuesto"."</td>";
		...
		while ($rowFacturaVenta = mysql_fetch_assoc($rsFacturaVenta)) {
			...
			$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
				$htmlTb .= "<td>".utf8_encode($rowFacturaVenta['nom_uni_bas'])."</td>";
				$htmlTb .= "<td>".utf8_encode($rowFacturaVenta['vehiculo'])."</td>";
				...
				$htmlTb .= "<td align=\"right\">".utf8_encode($rowFacturaVenta['numeracion_pedido'])."</td>";
				$htmlTb .= "<td align=\"right\">".utf8_encode($rowFacturaVenta['numeracion_presupuesto'])."</td>";
