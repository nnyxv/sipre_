...
function cargaLstCondicionBuscar($selId = "", $bloquearObj = false){
	...
	$html = "<select ".(($totalRows > 2) ? "multiple=\"multiple\"" : "")." id=\"lstCondicionBuscar\" name=\"lstCondicionBuscar\" ".$class." ".$onChange." style=\"width:99%\">";
		$html .= "<option value=\"-1\">[ Seleccione ]</option>";
...
function cargaLstEstadoFactura($selId = "", $bloquearObj = false){
	...
	$html = "<select ".(($totalRows > 2) ? "multiple=\"multiple\"" : "")." id=\"lstEstadoFactura\" name=\"lstEstadoFactura\" ".$class." ".$onChange." style=\"width:99%\">";
...
function cargaLstModulo($idModulo = "", $selId = ""){
	...
	if (!$rs) return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
	$totalRows = mysql_num_rows($rs);
	$html = "<select ".(($totalRows > 2) ? "multiple=\"multiple\"" : "")." id=\"lstModulo\" name=\"lstModulo\" class=\"inputHabilitado\" onchange=\"byId('btnBuscar').click();\" style=\"width:99%\">";
		$html .= "<option value=\"-1\">[ Seleccione ]</option>";
	while ($row = mysql_fetch_assoc($rs)) {
...
function cargaLstTipoOrden($idEmpresa = "", $selId = "", $bloquearObj = false) {
	...
	if (!$rs) return $objResponse->alert(mysql_error()."\n\nLine: ".__LINE__);
	$totalRows = mysql_num_rows($rs);
	$html = "<select ".(($totalRows > 2) ? "multiple=\"multiple\"" : "")."  id=\"lstTipoOrden\" name=\"lstTipoOrden\" ".$class." ".$onChange.">";
		$html .= "<option value=\"-1\">[ Seleccione ]</option>";
	while ($row = mysql_fetch_assoc($rs)) {
...
function listaFactura($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 20, $totalRows = NULL) {
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			...
			$htmlTb .= "<td ".$rowspan.">";
			if ($estatus209 == 1 && !($row['anulada'] == "SI")) {
				$txtFechaProveedor = date(str_replace("d","01",spanDateFormat), strtotime($row['fechaRegistroFactura']));
				...
				|| restaFechas(spanDateFormat, $txtFechaProveedor, date(spanDateFormat), "meses") <= $cantMesesAnteriores) { // VERIFICA SI ES DE MESES ANTERIORES
					...
					|| date("m", strtotime($txtFechaProveedor)) == date("m")) { // VERIFICA SI EL REGISTRO DE COMPRA ESTA ENTRE LOS DIAS PERMITIDOS DEL MES EN CURSO
						if (!($row['id_pedido_reemplazo'] > 0) && $row['estatus_factura'] != 2) {
							$htmlTb .= sprintf("<a href=\"an_pedido_venta_form.php?idFactura=%s\" target=\"_self\"><img src=\"../img/iconos/pencil.png\"/ title=\"Editar\"></a>",
								$row['idFactura']);
						}
					}
				}
