...
function cargarExpediente($idExpediente, $frmExpediente) {
	...
	$query = sprintf("SELECT * FROM an_expediente_detalle_factura expediente_det_fact
	WHERE id_expediente = %s;",
		valTpDato($idExpediente, "int"));
	$rs = mysql_query($query);
	if (!$rs) return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
	...
	// BUSCA LOS CARGOS DEL EXPEDIENTE
	$query = sprintf("SELECT * FROM an_expediente_detalle_cargos expediente_det_cargo
	WHERE id_expediente = %s;",
		valTpDato($idExpediente, "int"));
	$rs = mysql_query($query);
	if (!$rs) return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
...
function guardarExpediente($frmExpediente, $frmListaExpediente) {
	...
	if ($idExpediente > 0) {
		if (!xvalidaAcceso($objResponse,"an_expediente_list","editar")) { errorGuardarExpediente($objResponse); return $objResponse; }
		...
		if (!$Result1) { errorGuardarExpediente($objResponse); return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__); }
		mysql_query("SET NAMES 'latin1';");
	} else {
		if (!xvalidaAcceso($objResponse,"an_expediente_list","insertar")) { errorGuardarExpediente($objResponse); return $objResponse; }
		
		$insertSQL = sprintf("INSERT INTO an_expediente (numero_expediente, numero_embarque)
		VALUE (%s, %s);",
			valTpDato($frmExpediente['txtNumeroExpediente'], "text"),
			valTpDato($frmExpediente['txtNumeroEmbarque'], "text"));
		mysql_query("SET NAMES 'utf8';");
	...
	$deleteSQL = sprintf("DELETE FROM an_factura_compra_gasto
	WHERE id_factura_compra IN (SELECT expediente_det_fact.id_factura_compra FROM an_expediente_detalle_factura expediente_det_fact
								WHERE expediente_det_fact.id_expediente = %s)
		AND id_modo_gasto IN (2)
		AND id_expediente_detalle_cargo IN (SELECT expediente_det_cargo.id_expediente_detalle_cargo FROM an_expediente_detalle_cargos expediente_det_cargo
	...
	$deleteSQL = sprintf("DELETE FROM an_expediente_detalle_factura WHERE id_expediente = %s;",
		valTpDato($idExpediente, "int"));
	$Result1 = mysql_query($deleteSQL);
	if (!$Result1) { errorGuardarExpediente($objResponse); return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__); }
	
	// ELIMINA LOS CARGOS DEL EXPEDIENTE
	$deleteSQL = sprintf("DELETE FROM an_expediente_detalle_cargos WHERE id_expediente = %s;",
		valTpDato($idExpediente, "int"));
	$Result1 = mysql_query($deleteSQL);
	if (!$Result1) { errorGuardarExpediente($objResponse); return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__); }
	...
	if (isset($arrayObj)) {
		foreach ($arrayObj as $indice => $valor) {
			...
			$insertSQL = sprintf("INSERT INTO an_expediente_detalle_factura (id_expediente, id_factura_compra)
			VALUE (%s, %s);",
				valTpDato($idExpediente, "int"),
			...
			WHERE fact_comp.id_factura_compra = %s;",
				valTpDato($frmExpediente['hddIdFacturaCompra'.$valor], "int"));
			$rs = mysql_query($query);
			if (!$rs) { errorGuardarExpediente($objResponse); return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__); }
			$row = mysql_fetch_assoc($rs);
	...
	if (isset($arrayObj1)) {
		foreach ($arrayObj1 as $indice1 => $valor1) {
			...
			$insertSQL = sprintf("INSERT INTO an_expediente_detalle_cargos (id_expediente, id_gasto, id_factura_compra_cargo, monto)
			VALUE (%s, %s, %s, %s);",
				valTpDato($idExpediente, "int"),
			...
			$Result1 = mysql_query($insertSQL);
			if (!$Result1) { 
				errorGuardarExpediente($objResponse);
				if (mysql_errno() == 1048) {
			...
			if (isset($arrayFactura)) {
				foreach ($arrayFactura as $indice2 => $valor2) {
					...
					$insertSQL = sprintf("INSERT INTO an_factura_compra_gasto (id_factura_compra, id_gasto, tipo, porcentaje_monto, monto, monto_medida, id_iva, iva, id_modo_gasto, id_tipo_medida, afecta_documento, id_factura_compra_cargo, id_condicion_gasto, id_expediente_detalle_cargo)
...
function insertarCargo($idGasto, $frmExpediente) {
	...
	for ($cont = 1; $cont <= 20; $cont++) {
		$objResponse->script(sprintf("byId('btnInsertarCargo%s').disabled = false;",
			$cont));
	}
...
function insertarFacturaCompra($idFacturaCompra, $frmExpediente) {
	...
	// DESBLOQUEA LOS BOTONES DEL LISTADO
	for ($cont = 1; $cont <= 20; $cont++) {
		$objResponse->script(sprintf("byId('btnInsertarFacturaCompra%s').disabled = false;",
			$cont));
	}
...
function listaOtrosCargos($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 10, $totalRows = NULL) {
	...
	$cond = (strlen($sqlBusq) > 0) ? " AND " : " WHERE ";
	$sqlBusq .= $cond.sprintf("(id_modo_gasto IN (2)
	AND estatus_iva = 0)");
...
function listaFacturaCompra($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 10, $totalRows = NULL) {
	...
	if ($valCadBusq[0] == 0) {
		...
		$query = sprintf("SELECT 
			...
			(IFNULL((SELECT COUNT(fact_comp_det_unidad.id_factura_compra) AS items
					FROM an_factura_compra_detalle_unidad fact_comp_det_unidad
					WHERE fact_comp_det_unidad.id_factura_compra = fact_comp.id_factura_compra), 0)
				+ IFNULL((SELECT COUNT(fact_comp_det_acc.id_factura_compra) AS items
						FROM an_factura_compra_detalle_accesorio fact_comp_det_acc
						WHERE fact_comp_det_acc.id_factura_compra = fact_comp.id_factura_compra), 0)) AS cant_items,
...
function insertarItemFacturaCompra($contFila, $hddIdFacturaCompra = "", $idExpedienteDetalleFactura = "") {
	...
	$query = sprintf("SELECT 
		...
		(IFNULL((SELECT COUNT(fact_comp_det_unidad.id_factura_compra) AS items
			...
			+ IFNULL((SELECT COUNT(fact_comp_det_acc.id_factura_compra) AS items
				FROM an_factura_compra_detalle_accesorio fact_comp_det_acc
				WHERE fact_comp_det_acc.id_factura_compra = fact_comp.id_factura_compra), 0)) AS items,
...
function errorGuardarExpediente($objResponse) {
	$objResponse->script("
	byId('btnGuardarExpediente').disabled = false;
	byId('btnCancelarExpediente').disabled = false;");
}
?>