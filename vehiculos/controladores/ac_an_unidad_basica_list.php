...
function buscarUnidadBasica($frmBuscar) {
	...
	$valBusq = sprintf("%s|%s|%s|%s|%s|%s|%s",
		...
		(is_array($frmBuscar['lstCatalogoBuscar']) ? implode(",",$frmBuscar['lstCatalogoBuscar']) : $frmBuscar['lstCatalogoBuscar']),
...
function cargaLstAnoBuscar($selId = "", $bloquearObj = false) {
	$objResponse = new xajaxResponse();
	...
	$html = "<select ".(($totalRows > 2) ? "multiple=\"multiple\"" : "")." id=\"lstAnoBuscar\" name=\"lstAnoBuscar\" ".$class." ".$onChange." style=\"width:99%\">";
		$html .= "<option value=\"-1\">[ Seleccione ]</option>";
	while ($row = mysql_fetch_assoc($rs)) {
...
function eliminarUnidadBasica($idUnidadBasica, $frmListaUnidadBasica) {
	...
	if (!xvalidaAcceso($objResponse,"an_unidad_basica_list","eliminar")) { return $objResponse; }
...
function exportarUnidadBasica($frmBuscar) {
	$objResponse = new xajaxResponse();
	...
	$objResponse->script("window.open('reportes/an_unidad_basica_excel.php?valBusq=".$valBusq."','_self');");
...
function formUnidadBasica($idUnidadBasica, $frmUnidadBasica, $bloquearObj = false) {
	...
	if ($idUnidadBasica > 0) {
		if (!xvalidaAcceso($objResponse,"an_unidad_basica_list","editar")) { usleep(0.5 * 1000000); $objResponse->script("byId('btnCancelarUnidadBasica').click();"); return $objResponse; }
		...
		$objResponse->assign("txtFechaLista", "value", date(spanDateFormat, strtotime($row['pvp_fecha'])));
		...
		if (!xvalidaAcceso($objResponse,"an_unidad_basica_list","insertar")) { usleep(0.5 * 1000000); $objResponse->script("byId('btnCancelarUnidadBasica').click();"); return $objResponse; }
...
function guardarUnidadBasica($frmUnidadBasica, $frmListaUnidadBasica) {
	...
	WHERE nom_uni_bas LIKE %s
		...
		valTpDato(str_replace(" ","_",$frmUnidadBasica['txtNombreUnidadBasica']), "text"),
	...
	if ($idUnidadBasica > 0) {
		if (!xvalidaAcceso($objResponse,"an_unidad_basica_list","editar")) { errorGuardarUnidadBasica($objResponse); return $objResponse; }
		...
		if (!xvalidaAcceso($objResponse,"an_unidad_basica_list","insertar")) { errorGuardarUnidadBasica($objResponse); return $objResponse; }
...
function listaPrecios($idUnidadBasica) {
	...
	if (!xvalidaAcceso($objResponse,"an_unidad_basica_list","")) { return $objResponse; }
...
function listaUnidadBasica($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 21, $totalRows = NULL) {
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<td valign=\"top\" width=\"33%\">";
			$htmlTb .= "<table align=\"left\" class=\"".$class."\" height=\"24\" border=\"0\" width=\"100%\">";
			$htmlTb .= "<tr>";
				$htmlTb .= sprintf("<td ".$classCatalogo." rowspan=\"5\" title=\"Id ".$row['id_uni_bas']."\" valign=\"top\"><input id=\"cbxItm\" name=\"cbxItm[]\" type=\"checkbox\" value=\"%s\"></td>",
					$row['id_uni_bas']);
...
$xajax->register(XAJAX_FUNCTION,"exportarUnidadBasica");
