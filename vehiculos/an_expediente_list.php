...
if(!(validaAcceso("an_expediente_list"))) {
...
include("controladores/ac_an_expediente_list.php");
...
<head>
    ...
    <title>.: SIPRE <?php echo cVERSION; ?> :. Vehículos - Expedientes de Importación</title>
	...
	function validarFrmExpediente() {
		...
		&& validarCampo('txtNumeroEmbarque','t','') == true) {
			byId('btnGuardarExpediente').disabled = true;
			byId('btnCancelarExpediente').disabled = true;
	...
	function validarInsertarFacturaCompra(idFacturaCompra) {
		...
		for (cont = 1; cont <= 20; cont++) {
			if (!(byId('btnInsertarFacturaCompra' + cont) == undefined)) {
				byId('btnInsertarFacturaCompra' + cont).disabled = true;
			}
		}
	...
	function validarInsertarCargo(idGasto) {
		// BLOQUEA LOS BOTONES DEL LISTADO
		for (cont = 1; cont <= 20; cont++) {
			if (!(byId('btnInsertarCargo' + cont) == undefined)) {
				byId('btnInsertarCargo' + cont).disabled = true;
			}
		}
...
<div id="divGeneralPorcentaje">
    ...
    <div id="divInfo" class="print">
        ...
        <tr>
        	<td class="tituloPaginaVehiculos">Expedientes de Importación</td>
        </tr>
        <tr>
        	<td>&nbsp;</td>
        </tr>
...
<form id="frmExpediente" name="frmExpediente" onsubmit="return false;" style="margin:0">
	...
	<div id="tblExpediente" style="max-height:500px; overflow:auto; width:960px;">
        <table border="0" width="100%">
        <tr>
        	<td>
            	<table width="100%">
