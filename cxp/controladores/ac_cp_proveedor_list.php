...
function formProveedor($idProveedor, $frmProveedor) {
	...
	if ($idProveedor > 0) {
		...
		$queryProveedor = sprintf("SELECT *,
			IF(LENGTH(prov.rif) > 0, CONCAT_WS('-', prov.lrif, prov.rif), NULL) AS rif_proveedor,
			IF(LENGTH(prov.nit) > 0, CONCAT_WS('-', prov.lnit, prov.nit), NULL) AS nit_proveedor,
			IF(LENGTH(prov.cicontacto) > 0, CONCAT_WS('-', prov.lcicontacto, prov.cicontacto), NULL) AS ci_contacto
...
function exportarProveedor($frmBuscar) {
	...
	$objResponse->script("window.open('reportes/cp_provedor_excel.php?valBusq=".$valBusq."','_self');");
