...
function listadoFacturas($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 10, $totalRows = NULL) {
	$objResponse = new xajaxResponse();
	...
	$query = sprintf("SELECT * FROM cp_notadecargo WHERE estatus_notacargo <> 1 %s", $sqlBusq);
	...
	$objResponse->assign("tdFlotanteTitulo","innerHTML","Notas De Cargo");
	...
	$htmlTableIni1 .= "<form id=\"frmBuscarFactura\"><table width=\"100%\">";
	$htmlTh1 .= "<tr>
					...
					<td align=\"left\"><input type=\"text\" name=\"txtBusq\" id=\"txtBusq\"/><input type=\"button\" name=\"btnBuscarFacturas\" id=\"btnBuscarFacturas\" value=\"Buscar\" onclick=\"xajax_buscarFactura(xajax.getFormValues('frmBuscarFactura'));\"/></td>
	...
	$htmlTh .= "
				<tr class=\"tituloColumna\">
					...
					<td align=\"center\" width=\"8%\">No. de N.Cargo</td>
					<td align=\"center\" width=\"8%\">No. de Control N.Cargo</td>
					<td align=\"center\" width=\"13%\">Rif</td>
					<td align=\"center\" width=\"18%\">Proveedor</td>
					<td align=\"center\" width=\"13%\">Fecha N.Cargo Origen</td>
					<td align=\"center\" width=\"13%\">Fecha N.Cargo Proveedor</td>
					<td align=\"center\" width=\"13%\">Saldo N.Cargo</td>
				</tr>";
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr class=\"".$clase."\">";
			$htmlTb .= "<td>"."<button type=\"button\" onclick=\"xajax_cargarDatosNotaCredito('".$row['id_notacargo']."',xajax.getFormValues('frmTotalNotaCredito'));\" title=\"Seleccionar\"><img src=\"../img/iconos/tick.png\"/></button>"."</td>";
			$htmlTb .= "<td align=\"center\">".$empresa."</td>";
			$htmlTb .= "<td align=\"right\">".$row['numero_notacargo']."</td>";
			$htmlTb .= "<td align=\"right\">".$row['numero_control_notacargo']."</td>";
			$htmlTb .= "<td align=\"center\">".$rowProveedor['lrif']."-".$rowProveedor['rif']."</td>";
			$htmlTb .= "<td align=\"center\">".htmlentities($rowProveedor['nombre'])."</td>";
...
function cargarDatosNotaCredito($idDocumento,$valForm){
	$objResponse = new xajaxResponse();
	...
	$queryFactura = sprintf("SELECT * FROM cp_notadecargo WHERE id_notacargo = '%s'", $idDocumento);
	...
	if($rowFactura['tipo_pago_notacargo']=='0'){
		$tipoPago="Contado";
	}
	if($rowFactura['tipo_pago_notacargo']=='1'){
		$tipoPago="Credito";
	}
	if($rowFactura['aplica_libros_notacargo']=='0'){
		$aplicaLibros="NO";
	}
	if($rowFactura['aplica_libros_notacargo']=='1'){
		$aplicaLibros="SI";
	}
	$fechaOrigen=substr($rowFactura['fecha_origen_notacargo'],8,2).substr($rowFactura['fecha_origen_notacargo'],4,4).substr($rowFactura['fecha_origen_notacargo'],0,4);
	$fechaFactura=substr($rowFactura['fecha_notacargo'],8,2).substr($rowFactura['fecha_notacargo'],4,4).substr($rowFactura['fecha_notacargo'],0,4);
	...
	$objResponse->assign("txtDireccionProv","value",$rowProveedor['direccion']);
	...
	$objResponse->assign("txtNumeroFacturaProveedor","value",$rowFactura['numero_notacargo']);
	$objResponse->assign("hddObjIdFactura","value",$rowFactura['id_notacargo']);
	$objResponse->assign("txtNumeroControl","value",$rowFactura['numero_control_notacargo']);
	...
	$objResponse->assign("txtObservacionFactura","value",$rowFactura['observacion_notacargo']);
	$objResponse->assign("hddIdEmpresa","value",$rowFactura['id_empresa']);

	
	$objResponse->assign("txtSubTotal","value",number_format($rowFactura['subtotal_notacargo'],2,",","."));
	$objResponse->assign("txtSubTotalDescuento","value",number_format($rowFactura['subtotal_descuento_notacargo'],2,",","."));
	$objResponse->assign("txtMontoExonerado","value",number_format($rowFactura['monto_exonerado_notacargo'],2,",","."));
	$objResponse->assign("txtMontoExento","value",number_format($rowFactura['monto_exento_notacargo'],2,",","."));
	$objResponse->assign("hddObjtxtSubTotal","value",$rowFactura['subtotal_notacargo']);
	$objResponse->assign("hddObjtxtSubTotalDescuento","value",$rowFactura['subtotal_descuento_notacargo']);
	$objResponse->assign("hddObjtxtMontoExonerado","value",$rowFactura['monto_exonerado_notacargo']);
	$objResponse->assign("hddObjtxtMontoExento","value",$rowFactura['monto_exento_notacargo']);
	...
	$queryIvas = sprintf("SELECT * FROM cp_notacargo_iva WHERE id_notacargo='%s'", $rowFactura['id_notacargo']);
	...
	$clase = "trResaltar4";
		...
		$ivaTotal = $rowIvas['baseimponible']+$rowIvas['subtotal_iva'];
		
		$totalBaseMasIva +=$rowIvas['subtotal_iva'];
		...
		$objResponse->script(sprintf("
				var elemento = new Element('tr', {'id':'trItmIva:%s', 'class':'textoGris_11px %s', 'title':'trItmIva:%s'}).adopt([
					...
					new Element('td', {'align':'right'}).setHTML(\"<input id='baseImponibleIva%s' name='baseImponibleIva%s' readonly='readonly' type='text' style='text-align:right' size='17' value='%s' />\"),
					new Element('td', {'align':'right'}).setHTML(\"<input id='porcentajeIva%s' name='porcentajeIva%s' readonly='readonly' type='text' style='text-align:right' size='5' value='%s' />%s\"),
					new Element('td', {'align':'right'}).setHTML(\"<input id='montoPorcentajeIva%s' name='montoPorcentajeIva%s' readonly='readonly' type='text' style='text-align:right' size='17' value='%s' />"."<input type='hidden' id='hddIdIva%s' name='hddIdIva%s' value='%s'/><input type='hidden' id='hddBaseimponible%s' name='hddBaseimponible%s' value='%s'/><input type='hidden' id='hddSubtotal%s' name='hddSubtotal%s' value='%s'/><input type='hidden' id='hddIva%s' name='hddIva%s' value='%s'/>\")]);
				elemento.injectBefore('tdDescuento');",
					$sigValor,$clase,$sigValor,
					$rowTipoIva['observacion'],
					$sigValor,$sigValor,number_format($rowIvas['baseimponible'],2,",","."),
					...
					$sigValor,$sigValor,$rowIvas['baseimponible'],
	...
	$queryGasto = sprintf("SELECT * FROM cp_notacargo_gastos  WHERE id_notacargo='%s'", $rowFactura['id_notacargo']);
	$rsGasto = mysql_query($queryGasto) or die(mysql_error());
	...
	$clase = "trResaltar4";
		...
		$queryGastos = sprintf("SELECT * FROM pg_gastos WHERE id_gasto ='%s' ",$rowGasto['id_gastos']);
		$rsTipoGasto = mysql_query($queryGastos);
		$rowTipoGasto = mysql_fetch_array($rsTipoGasto);
		
		$gastoTotal = $rowGasto['monto']*($rowGasto['iva']/100);
		
		$totalGastoBaseMasIva = $gastoTotal+ $rowGasto['monto'] + $totalGastoBaseMasIva;
		...
		$objResponse->script(sprintf("
				var elemento = new Element('tr', {'id':'trItmGasto:%s', 'class':'textoGris_11px %s', 'title':'trItmGasto:%s'}).adopt([
					new Element('td', {'align':'right','class':'tituloCampo'}).setHTML(\"%s:\"),
					new Element('td', {'align':'right'}).setHTML(\"<input id='txtMontoGasto%s' name='txtMontoGasto%s' readonly='readonly' type='text' style='text-align:right' size='17' value='%s' />\"),
					new Element('td', {'align':'right'}).setHTML(\"<input id='txtPorcentajeIva%s'  name='txtPorcentajeIva%s' readonly='readonly' type='text' style='text-align:right' size='5' value='%s' />%s\"),
					new Element('td', {'align':'right'}).setHTML(\"<input id='montoPorcentajeIva%s' name='montoPorcentajeIva%s' readonly='readonly' type='text' style='text-align:right' size='17' value='%s' />"."<input type='hidden' id='hddIdIvaGasto%s' name='hddIdIvaGasto%s' value='%s'/><input type='hidden' id='hddIdGasto%s' name='hddIdGasto%s' value='%s'/><input type='hidden' id='hddMontoGasto%s' name='hddMontoGasto%s' value='%s'/><input type='hidden' id='hddIvaGasto%s' name='hddIvaGasto%s' value='%s'/>\")]);
				elemento.injectBefore('tdDescuento');",
					$sigValor,$clase,$sigValor,
					$rowTipoGasto['nombre'],
					$sigValor,$sigValor,number_format($rowGasto['monto'],2,",","."),
					$sigValor,$sigValor,number_format($rowTipoIva['iva'],2,",","."),"%",
					$sigValor,$sigValor,number_format($rowGasto['iva'],2,",","."),
					$sigValor,$sigValor,$rowGasto['id_iva'],
					$sigValor,$sigValor,$rowGasto['id_gastos'],
	...
	$totalNotaCredito = ($totalGastoBaseMasIva + $totalBaseMasIva + $rowFactura['monto_exonerado_notacargo'] + $rowFactura['monto_exento_notacargo'] + $rowFactura['subtotal_notacargo']) - $rowFactura['subtotal_descuento_notacargo']; 
...
function guardarDatos($valFormFactura,$valFormNotaCredito,$valFormProveedor,$valFormTotalNotaCredito){
	...
	$queryFactura = sprintf("SELECT * FROM cp_notadecargo WHERE id_notacargo='%s'",$valFormFactura['hddObjIdFactura']);
	$rsFactura = mysql_query($queryFactura) or die(mysql_error());
	$rowFactura = mysql_fetch_array($rsFactura);
	
	$saldoNotaCredito = $valFormTotalNotaCredito['hddtxtTotalNotaCredito']-$rowFactura['saldo_notacargo'];
	...
	$queryUpdate = "UPDATE cp_notadecargo SET saldo_notacargo = '0', estatus_notacargo='1' WHERE id_notacargo = '$valFormFactura[hddObjIdFactura]'";
