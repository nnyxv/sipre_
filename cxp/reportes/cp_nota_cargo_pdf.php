...
$queryEncabezado = sprintf("SELECT 
	...
	cxp_nd.observacion_notacargo,
                rec.id_notacargo as reconvercion,
	motivo.id_motivo,
...
FROM cp_notadecargo cxp_nd
	...
	LEFT JOIN cp_prove_credito prov_cred ON (prov.id_proveedor = prov_cred.id_proveedor)
                LEFT JOIN cp_reconversion rec on (cxp_nd.id_notacargo = rec.id_notacargo)
...
while ($rowDetalle = mysql_fetch_assoc($rsDetalle)) {
	...
	if (fmod($contFila, $maxRows) == 1) {
		...
		$posY += 9;
                               imagestring($img,1,0,$posY,str_pad(utf8_decode("CÓDIGO"), 5, " ", STR_PAD_BOTH),$textColor);
                               imagestring($img,1,80,$posY,str_pad(utf8_decode("DESCRIPCIÓN"), 5, " ", STR_PAD_BOTH),$textColor);
                               imagestring($img,1,230,$posY,str_pad(utf8_decode("CANTIDAD"), 10, " ", STR_PAD_BOTH),$textColor);
                               imagestring($img,1,290,$posY,strtoupper(str_pad(utf8_decode($spanPrecioUnitario), 15, " ", STR_PAD_BOTH)),$textColor);
                               imagestring($img,1,380,$posY,str_pad(utf8_decode("TOTAL"), 15, " ", STR_PAD_BOTH),$textColor);
		$posY += 9;
		imagestring($img,1,0,$posY,str_pad("", 94, "-", STR_PAD_BOTH),$textColor);
	}
	...
	if (isset($tieneDetalle)) {
		...
		if (isset($arrayObservacionDcto)) {
			foreach ($arrayObservacionDcto as $indice => $valor) {
				...
				if ($indice == 0) {
                                                                               imagestring($img,1,230,$posY,str_pad(number_format(1, 2, ".", ","), 10, " ", STR_PAD_LEFT),$textColor);
                                                                               imagestring($img,1,290,$posY,str_pad(number_format($rowEncabezado['subtotal_notacargo'], 2, ".", ","), 15, " ", STR_PAD_LEFT),$textColor);
                                                                               imagestring($img,1,380,$posY,str_pad(number_format($rowEncabezado['subtotal_notacargo'], 2, ".", ","), 15, " ", STR_PAD_LEFT),$textColor);
				}
			}
		}
		$posY += 9;
		...
		imagestring($img,1,0,$posY,elimCaracter($rowDetalle['id_motivo'],";"),$textColor);
                               imagestring($img,1,80,$posY,strtoupper(substr($rowDetalle['descripcion'],0,28)),$textColor);
                               imagestring($img,1,230,$posY,str_pad(number_format($rowDetalle['cantidad'], 2, ".", ","), 5, " ", STR_PAD_LEFT),$textColor);
                               imagestring($img,1,290,$posY,str_pad(number_format($rowDetalle['precio_unitario'], 2, ".", ","), 15, " ", STR_PAD_LEFT),$textColor);
                               imagestring($img,1,380,$posY,str_pad(number_format($rowDetalle['cantidad'] * $rowDetalle['precio_unitario'], 2, ".", ","), 15, " ", STR_PAD_LEFT),$textColor);
	}
	...
	if (fmod($contFila, $maxRows) == 0 || $contFila == $totalRowsDetalle) {
		if ($contFila == $totalRowsDetalle) {
				...
				if (strlen($rowEncabezado['numero_siniestro']) > 0) {
					...
					imagestring($img,1,70,$posY,": ".$rowEncabezado['numero_siniestro'],$textColor);
                                               ...
                                               if ($rowEncabezado['reconvercion'] == NULL) {
                                                               if ($rowEncabezado['fecha_notacargo']>= '2018-08-20') {
                                                                               $aa = $rowEncabezado['subtotal_notacargo']*100000;
                                                                               

                                                               }else{

                                                                               $aa = $rowEncabezado['subtotal_notacargo'];
			...
			imagestring($img,1,325,$posY,":",$textColor);
                                               imagestring($img,1,$total2,$posY,str_pad(number_format($rowEncabezado['subtotal_notacargo'], 2, ".", ","), 15, " ", STR_PAD_LEFT),$textColor);
			...
			if ($rowEncabezado['subtotal_descuento_notacargo'] > 0) {
				...
				imagestring($img,1,345,$posY,str_pad(number_format($porcDescuento, 2, ".", ",")."%", 8, " ", STR_PAD_LEFT),$textColor);
                                                             imagestring($img,1,$total2,$posY,str_pad(number_format($rowEncabezado['subtotal_descuento_notacargo'], 2, ".", ","), 15, " ", STR_PAD_LEFT),$textColor);
			}
			...
			if ($totalGastosConIva > 0) {
				...
				imagestring($img,1,325,$posY,":",$textColor);
                                                               imagestring($img,1,$total2,$posY,str_pad(number_format($totalGastosConIva, 2, ".", ","), 15, " ", STR_PAD_LEFT),$textColor);
			}
			...
			while ($rowIvaDcto = mysql_fetch_assoc($rsIvaDcto)) {
				...
				imagestring($img,1,325,$posY,":",$textColor);
                                                               imagestring($img,1,$total2,$posY,str_pad(number_format($rowIvaDcto['base_imponible'], 2, ".", ","), 15, " ", STR_PAD_LEFT),$textColor);
				...
				imagestring($img,1,360,$posY,str_pad(number_format($rowIvaDcto['iva'], 2, ".", ",")."%", 6, " ", STR_PAD_LEFT),$textColor);
                                                               imagestring($img,1,$total2,$posY,str_pad(number_format($rowIvaDcto['subtotal_iva'], 2, ".", ","), 15, " ", STR_PAD_LEFT),$textColor);
			...
			if ($totalGastosSinIva > 0) {
				...
				imagestring($img,1,325,$posY,":",$textColor);
                                                               imagestring($img,1,$total2,$posY,str_pad(number_format($totalGastosSinIva, 2, ".", ","), 15, " ", STR_PAD_LEFT),$textColor); // <---
			}
			...
			if ($rowEncabezado['monto_exento_notacargo'] > 0) {
				...
				imagestring($img,1,325,$posY,":",$textColor);
                                                               imagestring($img,1,$total2,$posY,str_pad(number_format($rowEncabezado['monto_exento_notacargo'], 2, ".", ","), 15, " ", STR_PAD_LEFT),$textColor);
			}
			...
			$totalFactura = $rowEncabezado['subtotal_notacargo'] - $rowEncabezado['subtotal_descuento_notacargo'] + $totalIva + $totalGastosSinIva + $totalGastosConIva;
                                               if ($rowEncabezado['reconvercion'] == NULL) {
                                                               if ($rowEncabezado['fecha_notacargo']>= '2018-08-01' and $rowEncabezado['fecha_notacargo'] < '2018-08-20') {
                                                               $posY += 7; 
                                               ...
                                               }else if ($rowEncabezado['fecha_notacargo']>= '2018-08-20') {
                                                               $posY += 9;
                                                               imagestring($img,1,255,$posY,"TOTAL Bs.S",$textColor);
                                                               imagestring($img,1,325,$posY,":",$textColor);
