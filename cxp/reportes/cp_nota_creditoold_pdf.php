...
while ($rowDetalle = mysql_fetch_assoc($rsDetalle)) {
		...
		if (isset($arrayObservacionDcto)) {
			foreach ($arrayObservacionDcto as $indice => $valor) {
				...
				if ($indice == 0) {
					...
					imagestring($img,1,315,$posY,str_pad(number_format($rowEncabezado['subtotal_notacredito'], 2, ".", ","), 15, " ", STR_PAD_LEFT),$textColor);
	...
	if (fmod($contFila, $maxRows) == 0 || $contFila == $totalRowsDetalle) {
		if ($contFila == $totalRowsDetalle) {
			...
			if ($rowEncabezado['fecha_registro_notacredito']<='01-05-2018') {
				$posY += 9;
			imagestring($img,1,255,$posY,"TOTAL Bs.S",$textColor);
			imagestring($img,1,325,$posY,":",$textColor);
			imagestring($img,2,380,$posY,str_pad(number_format(($totalFactura/1000), 2, ".", ","), 15, " ", STR_PAD_LEFT),$textColor);
			}
