...
function calcularPagos($frmListaPagos, $frmDcto, $frmTotalDcto) {
	...
	// CALCULA SOLO CUANDO VIENE DEL MODULO DE FINANCIAMIENTO
	$objResponse->assign("txtSaldoCuotaFinanciamiento","value",number_format(str_replace(",", "", $frmDcto['txtCuotaFinanciamientoTotal']) - $txtMontoPagadoFactura, 2, ".", ","));
...
function cargaLstTipoPago($idFormaPago = "", $selId = "") {
	...
	$cond = (strlen($sqlBusq) > 0) ? " AND " : " WHERE ";
	$sqlBusq .= $cond.sprintf("idFormaPago NOT IN (9,10,11)");
...
function cargarDcto($idNotaCargo){
	...
	$queryFactura = sprintf("SELECT cxc_nd.* FROM cj_cc_notadecargo cxc_nd WHERE cxc_nd.idNotaCargo = %s;",
	...
	// DATOS DEL CLIENTE
	$objResponse->assign("hddIdFactura","value",$idNotaCargo);
	$objResponse->loadCommands(asignarCliente($rowFactura['idCliente'], $rowFactura['id_empresa'], ""));
	$objResponse->assign("txtNumeroFactura","value",$rowFactura['numeroNotaCargo']);
	$objResponse->assign("txtNumeroControlFactura","value",$rowFactura['numeroControlNotaCargo']);
	$objResponse->assign("txtFecha","value",date(spanDateFormat, strtotime($rowFactura['fechaRegistroNotaCargo'])));
	$objResponse->assign("txtFechaVencimiento","value",date(spanDateFormat, strtotime($rowFactura['fechaVencimientoNotaCargo'])));
	$objResponse->loadCommands(cargaLstModulo($rowFactura['idDepartamentoOrigenNotaCargo'],"",true));
	
	$objResponse->assign("txtTotalOrden","value",number_format($rowFactura['saldoNotaCargo'], 2, ".", ","));
	
	$objResponse->assign("txtTotalFactura","value",number_format($rowFactura['montoTotalNotaCargo'], 2, ".", ","));
	$objResponse->assign("txtMontoPorPagar","value",number_format($rowFactura['saldoNotaCargo'], 2, ".", ","));
	...
	// FINANCIAMIENTO - ASIGNANDO EL MONTO DEL SALDO DE LA CUOTA
	if ($rowFactura['idDepartamentoOrigenNotaCargo'] == '5' && (isset($_GET['idCuota']) && $_GET['idCuota'] != null && $_GET['idCuota'] != '')) {
		$idCuota = $_GET['idCuota'];
		$queryCuota  = sprintf("SELECT
			amort.monto_cuota
		...
		$rsCuota = mysql_query($queryCuota);
		if (!$rsCuota) { return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__); }
		$rowCuota = mysql_fetch_assoc($rsCuota);
	
		// MOSTRANDO EL HTML DE LA SECCION DE SALDO CUOTA FINANCIAMIENTO:
		$objResponse->script("byId('trItmPiePagoFinanciamiento').style.display = '';");
		$objResponse->script("byId('trSaldoFinanciamiento').style.display = '';");
		
		// ASIGNANDO EL VALOR INICIAL DE LA CUOTA MAXIMA QUE SE PAGARA EN FINANCIAMIENTO
		$cuota = $rowCuota['monto_cuota'];
		$objResponse->assign("txtCuotaFinanciamientoTotal","value","$cuota");
		$objResponse->assign("txtSaldoCuotaFinanciamiento","value","$cuota");	
	}
...
function guardarDcto($frmDcto, $frmDetallePago, $frmListaPagos){
	...
	if ((!xvalidaAcceso($objResponse,"cj_nota_cargo_por_pagar_list","insertar") && in_array($idCajaPpal, array(1)))
	|| (!xvalidaAcceso($objResponse,"cjrs_nota_cargo_por_pagar_list","insertar") && in_array($idCajaPpal, array(2)))) { return $objResponse; }
	
	// DESCONCATENA PARA SABER CUANTOS ITEMS HAY AGREGADOS
	$arrayObj2 = $frmListaPagos['cbx2'];
	
	$idNotaCargo = $frmDcto['hddIdFactura'];
	...
	// FINANCIAMIENTO - VERIFICANDO QUE EL SALDO ESTE EN CERO 
	if ($idModulo == '5' && (isset($_GET['idCuota']) && $_GET['idCuota'] != null && $_GET['idCuota'] != '')) {
		if ($frmListaPagos['txtSaldoCuotaFinanciamiento'] != 0) {
			return $objResponse->alert("Aun queda saldo en la cuota por financiamiento.");
		}
	}
...
//--//--//--//--//--//--//--//--//--//--//--//--//--//--//--//--//--//--//--//--//--//--//--//--//--//--//--//--//--//--//--//--//--//--//--//
	...
	/*************SI EL PAGO DE LA NOTA DE CARGO VIENE DEL MODULO DE FINANCIAMIENTO***********/
	if ($idModulo == '5' && (isset($_GET['idCuota']) && $_GET['idCuota'] != null && $_GET['idCuota'] != '')) {
		$idCuota = $_GET['idCuota'];	
		$updateSQL = sprintf("UPDATE fi_amortizacion SET
			estado_cuota = 1
		WHERE id_amortizacion = %s;", // estado_cuota = 1 = PAGADA
			valTpDato($idCuota, "int"));
	...
	//PAGANDO LA CUOTA ATRASADA DEL FINANCIAMIENTO
	
	if ($idModulo == '5' && (isset($_GET['idCuotaMora']) && $_GET['idCuotaMora'] != null && $_GET['idCuotaMora'] != '')) {
		
		$idCuota = $_GET['idCuotaMora'];
		$idPedidoFinanciamiento = $_GET['idPedidoFinanciamiento'];
		
		$updateSQL = sprintf("UPDATE fi_amortizacion SET
			estado_cuota = 0
		WHERE id_amortizacion = %s;", // ESTADO_CUOTA = 0 = PENDIENTE 
			valTpDato($idCuota, "int"));
		$rsUpdateSQL = mysql_query($updateSQL);
		if (!$rsUpdateSQL) return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__."\nFile: ".__FILE__);
		
		//ACTUALIZANDO EL ESTATUS DE PAGO DEL INTERES DE MORA
		
		$updateSQL = sprintf("UPDATE fi_amortizacion_interesmora SET
			estado_pago = 1
		WHERE id_amortizacion = %s
				AND id_pedido_financiamiento = %s;", // ESTADO_CUOTA = 1 = PAGADO 
			valTpDato($idCuota, "int"),
		...
		//VERIFICANDO SI EL CLIENTE NO ES MOROSO
		...
		while($rowIntMora = mysql_fetch_assoc($rs)){
			if($rowIntMora['estado_cuota'] == 2){ //estado_pago = 2 = SI HAY CUOTAS MOROSAS
				$swMoroso = true;
			}
		}		
		
		if($swMoroso == false){ //SI NO HAY CUOTAS MOROSAS ACTUALIZO EL ESTADO DEL PEDIDO
					
			$updateCuotaSQL = sprintf("UPDATE fi_pedido SET
					estatus_pedido = %s
							WHERE id_pedido_financiamiento = %s",
					valTpDato("1", "int"), //ASIGNANDO PEDIDO A UN ESTADO 1 =  PARCIALMENTE PAGADO 
					valTpDato($idPedidoFinanciamiento, "int"));
	...
	if (in_array($idCajaPpal, array(1))){
		...
		cerrarVentana = true;
		window.location.href='cj_nota_cargo_por_pagar_list.php';"));
		...
		$objResponse->script(sprintf("
		cerrarVentana = true;
		window.location.href='cjrs_nota_cargo_por_pagar_list.php';"));
	}
...
function insertarPago($frmListaPagos, $frmDetallePago, $frmDeposito, $frmLista, $frmDcto, $frmTotalDcto) {
	...
	// INSERTANDO CONDICION DEL MODULO DE FINANCIAMIENTO
	if ($frmDcto['lstModulo'] == '5' && (isset($_GET['idCuota']) && $_GET['idCuota'] != null && $_GET['idCuota'] != '')) {
		$idCuota = $_GET['idCuota'];
		$queryCuota  = sprintf("SELECT 
			amort.monto_cuota
		FROM fi_amortizacion amort
		...
		if (str_replace(",", "", $frmListaPagos['txtSaldoCuotaFinanciamiento']) < str_replace(",", "", $frmDetallePago['txtMontoPago'])) {
			return $objResponse->alert("El monto a pagar no puede ser mayor que la cuota a amortizar.");
		}
	}
