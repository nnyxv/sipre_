...
function exportarAnticipo($frmBuscar) {
	...
	$objResponse->script("window.open('reportes/cjrs_anticipo_historico_excel.php?valBusq=".$valBusq."','_self');");
...
function imprimirAnticipo($frmBuscar){
	...
	$objResponse->script(sprintf("verVentana('reportes/cjrs_anticipo_historico_pdf.php?valBusq=%s&lstOrientacionPDF=%s',890,550)", $valBusq, $frmBuscar['lstOrientacionPDF']));
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			...
			if ($row['saldoAnticipo'] > 0 && $row['estatus'] == 1) {
				if (in_array($row['idDepartamento'], array(2,4,5))){ // 2 = Vehiculos, 4 = Alquiler, 5 = Financiamiento
					...
					$htmlTb .= sprintf("<a href=\"cjrs_anticipo_form.php?id=%s&vw=a\" target=\"_self\"><img src=\"../img/iconos/application_view_columns_add.png\" title=\"Pagar Dctos.\"/></a>",
						$row['idAnticipo']);
				}
			}
			$htmlTb .= "</td>";
