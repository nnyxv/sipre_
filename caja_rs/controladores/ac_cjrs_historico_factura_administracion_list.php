...
function cargaLstModulo($idModulo = "", $selId = ""){
	...
	$query = sprintf("SELECT * FROM pg_modulos WHERE id_modulo IN (3)");
	$rs = mysql_query($query);
	if (!$rs) return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
	$totalRows = mysql_num_rows($rs);
...
function cargaLstVendedor($selId = "", $bloquearObj = false) {
	...
	WHERE cxc_fact.idDepartamentoOrigenFactura IN (3)
...
function exportarFactura($frmBuscar) {
	...
	$objResponse->script("window.open('reportes/cjrs_factura_historico_excel.php?valBusq=".$valBusq."','_self');");
...
function imprimirFactura($frmBuscar){
	...
	$objResponse->script(sprintf("verVentana('reportes/cjrs_factura_historico_pdf.php?valBusq=%s&lstOrientacionPDF=%s',890,550)", $valBusq, $frmBuscar['lstOrientacionPDF']));
