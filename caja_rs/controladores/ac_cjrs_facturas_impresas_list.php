...
function formEditarConsecutivoFiscal($idFatura,$consecutivo_fiscal){
	...
	if (!xvalidaAcceso($objResponse,"cjrs_facturas_impresas_list","editar")){ 
...
function exportarFactura($frmBuscar) {
	...
	$objResponse->script("window.open('reportes/cjrs_factura_impresa_excel.php?valBusq=".$valBusq."','_self');");
...
function imprimirFactura($frmBuscar){
	...
	$objResponse->script(sprintf("verVentana('reportes/cjrs_factura_impresa_pdf.php?valBusq=%s&lstOrientacionPDF=%s',890,550)", $valBusq, $frmBuscar['lstOrientacionPDF']));
