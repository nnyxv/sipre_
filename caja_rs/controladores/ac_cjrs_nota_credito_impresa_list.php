...
function formEditarConsecutivoFiscal($idNotaCredito,$consecutivo_fiscal){
	...
	if (!xvalidaAcceso($objResponse,"cjrs_nota_credito_impresa_list","editar")){ 
...
function exportarNotaCredito($frmBuscar) {
	...
	$objResponse->script("window.open('reportes/cjrs_nota_credito_impresa_excel.php?valBusq=".$valBusq."','_self');");
...
function imprimirNotaCredito($frmBuscar){
	...
	$objResponse->script(sprintf("verVentana('reportes/cjrs_nota_credito_impresa_pdf.php?valBusq=%s&lstOrientacionPDF=%s',890,550)", $valBusq, $frmBuscar['lstOrientacionPDF']));
