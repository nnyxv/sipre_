...
function guardarNotaCargo($numeroCheque,$monto,$idCliente,$idBanco){
	...
	if ($totalRowsCheque > 0) {
		...
		$objResponse->script(sprintf("window.location.href='cjrs_devolucion_cheque_list.php';"));
...
function imprimirDevolucionCheque($frmBuscar){
	...
	$objResponse->script(sprintf("verVentana('reportes/cjrs_devolucion_cheque_pdf.php?valBusq=%s',890,550)", $valBusq));
...
function validarDevolucionCheque($numeroCheque,$montoCheque,$idCliente,$idBanco){
	...
	if (xvalidaAcceso($objResponse, "cjrs_devolucion_cheque_list", "insertar")){
...
function validarAperturaCaja($numeroCheque = "", $montoCheque = "" ,$idCliente = "", $idBanco = ""){
	...
	if (mysql_num_rows($rsCierreCaja) > 0){
		...
		$objResponse->script("location.href='cjrs_cierre_caja.php'");
	} else {
		...
		if (mysql_num_rows($rsVerificarApertura) == 0){
			...
			$objResponse->script("location.href='cjrs_apertura_caja.php'");
		}
	}
