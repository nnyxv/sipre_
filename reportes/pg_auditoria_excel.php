...
FROM vw_iv_usuarios vw_iv_usu
	...
	INNER JOIN pg_elemento_menu elemento_menu ON (audit.id_elemento_menu = elemento_menu.id_elemento_menu) %s
ORDER BY fecha DESC", $sqlBusq);
...
while ($rowMaestro = mysql_fetch_assoc($rsMaestro)) {
	...
	$arrayCol[$contFila][0] = date("h:ia d-m-Y",strtotime($rowMaestro['fecha']));
	$arrayCol[$contFila][1] = htmlentities($rowMaestro['nombre_empleado'])." ";
	$arrayCol[$contFila][2] = htmlentities($rowMaestro['nombre_departamento'])." ";
	$arrayCol[$contFila][3] = htmlentities($rowMaestro['nombre_cargo'])." ";
	$arrayCol[$contFila][4] = htmlentities($rowMaestro['nombre_usuario']);
	$arrayCol[$contFila][5] = htmlentities($rowMaestro['nombre']);
	$arrayCol[$contFila][6] = htmlentities($rowMaestro['accion']);
	$arrayCol[$contFila][7] = (($rowMaestro['acceso'] == 1) ? "Permitido" : "Denegado");
}
...
$excel->download('ERP_Auditoria.xls');
?>