...
function asignarMotivo($idMotivo, $frmNotaDebito){
	$objResponse = new xajaxResponse();
	
	if(count($frmNotaDebito["hddIdMotivo"]) > 0){
		return $objResponse->alert("Solo se puede agregar un motivo");
	}
	foreach($frmNotaDebito["hddIdMotivo"] as $indice => $valor){
		if($idMotivo == $valor){
...
function buscarNotaDebito($valForm) {
	...
	$objResponse->loadCommands(listaNotaDebito(0, "id_nota_debito", "DESC", $valBusq));
...
function cargaLstEstado(){
	...
	$html = "<select id=\"lstEstado\" name=\"lstEstado\" onChange=\"xajax_buscarNotaDebito(xajax.getFormValues('frmBuscar'))\" class=\"inputHabilitado\">";
...
function cargaSaldoCuenta($idCuenta){
	...
	$objResponse->assign("hddSaldoCuenta","value",$rowCuenta['saldo_tem']);
	$objResponse->assign("hddDiferido","value",$rowCuenta['Diferido']);
	
	return $objResponse;
}

function formNotaDebito($idNotaDebito){
	$objResponse = new xajaxResponse();
	
	if ($idNotaDebito) {	
		$queryConsulta = sprintf("SELECT 
			nota_debito.id_nota_debito,
			nota_debito.id_numero_cuenta,
			nota_debito.fecha_registro,
			nota_debito.fecha_aplicacion,
			nota_debito.fecha_conciliacion,
			nota_debito.folio_tesoreria,
			nota_debito.id_beneficiario_proveedor,
			nota_debito.observaciones,
			nota_debito.folio_estado_cuenta_banco,
			nota_debito.estado_documento,
			nota_debito.origen,
			nota_debito.id_usuario,
			nota_debito.monto_nota_debito,
			nota_debito.control_beneficiario_proveedor,
			nota_debito.id_empresa,
			nota_debito.desincorporado,
			nota_debito.numero_nota_debito,
			nota_debito.tipo_nota_debito,
			nota_debito.id_motivo,
			(CASE nota_debito.tipo_nota_debito
				WHEN 1 THEN 'Normal'
				WHEN 2 THEN 'Tarjeta Débito'
				WHEN 3 THEN 'Tarjeta Crédito'
				WHEN 4 THEN 'Transferencia'
			END) AS descripcion_tipo_nota_debito,
			cuenta.idCuentas,
			cuenta.id_empresa,
		...
		FROM te_nota_debito nota_debito
			INNER JOIN vw_iv_empresas_sucursales vw_iv_emp_suc ON (nota_debito.id_empresa = vw_iv_emp_suc.id_empresa_reg)
			INNER JOIN cuentas cuenta ON (nota_debito.id_numero_cuenta = cuenta.idCuentas)
			INNER JOIN bancos banco ON (cuenta.idBanco = banco.idBanco)			
			INNER JOIN te_estados_principales ON (nota_debito.estado_documento = te_estados_principales.id_estados_principales)
			INNER JOIN te_origen ON (nota_debito.origen = te_origen.id)
			LEFT JOIN pg_motivo motivo ON (nota_debito.id_motivo = motivo.id_motivo)
		WHERE nota_debito.id_nota_debito = %s",
		...
		$objResponse->assign("txtOrigenNotaDebito","value",utf8_encode($rowConsulta['descripcion_origen']));
		$objResponse->assign("txtImporteMovimiento","value",number_format($rowConsulta['monto_nota_debito'],2,".",","));
		$objResponse->assign("txtNumeroDocumentoConsulta","value",utf8_encode($rowConsulta['numero_nota_debito']));
		$objResponse->assign("selTipoNotaDebito","value",$rowConsulta['tipo_nota_debito']);
		...
		$queryMotivo = sprintf("SELECT 
			det_motivo.id_nota_debito_detalle_motivo,
			det_motivo.id_motivo,
			det_motivo.precio_unitario
		FROM te_nota_debito_detalle_motivo det_motivo
		WHERE det_motivo.id_nota_debito = %s",
			valTpDato($idNotaDebito, "int"));									  
		...
		while($rowMotivo = mysql_fetch_assoc($rsMotivo)){
			$objResponse->loadCommands(insertarItemMotivo($rowMotivo['id_motivo'], $rowMotivo['id_nota_debito_detalle_motivo'], $rowMotivo['precio_unitario']));
		}
		...
		$objResponse->script("
			...
			byId('selTipoNotaDebito').className = 'inputInicial';
		...
		$objResponse->assign("txtOrigenNotaDebito","value","Tesorería");
		...
		$objResponse->script("
			...
			byId('selTipoNotaDebito').className = 'inputHabilitado';
...
function guardarNotaDebito($valForm){
	...
	if (!xvalidaAcceso($objResponse,"te_nota_debito","insertar")){ return $objResponse; }
	...
	$queryFoliosUpdate = sprintf("UPDATE te_folios SET numero_actual = (numero_actual + 1) WHERE id_folios = 1");
	...
	VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
		...
		valTpDato($valForm['selTipoNotaDebito'], "int"),
	...
	foreach($valForm["hddIdMotivo"] as $indice => $valor){
		$queryMotivo = sprintf("INSERT INTO te_nota_debito_detalle_motivo (id_nota_debito, id_motivo, precio_unitario) 
		VALUES (%s, %s, %s)",
			valTpDato($idNotaDebito, "int"),
...
function insertarItemMotivo($idMotivo, $hddIdNotaDebitoDet = "", $precioUnitario = "") {
	$objResponse = new xajaxResponse();
	
	$aClassReadonly = ($hddIdNotaDebitoDet > 0) ? "class=\"inputSinFondo\" readonly=\"readonly\"" : "class=\"inputCompletoHabilitado\"";
	$aEliminar = ($hddIdNotaDebitoDet > 0) ? "" :
	...
	$htmlItmPie = sprintf("$('#trItmPie').before('".
		"<tr align=\"left\" class=\"textoGris_11px trItemMotivo\">".
			...
			"<td>%s".
				"<input type=\"hidden\" id=\"hddIdNotaDebitoDet[]\" name=\"hddIdNotaDebitoDet[]\" readonly=\"readonly\" value=\"%s\"/>".
...
function listaMotivo($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 10, $totalRows = NULL){
	...
	$cond = (strlen($sqlBusq) > 0) ? " AND " : " WHERE ";
	$sqlBusq .= $cond.sprintf("modulo = 'TE' AND ingreso_egreso = 'E'");
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			$htmlTb .= "<td>";
				$htmlTb .= sprintf("<button type=\"button\" onclick=\"xajax_asignarMotivo(%s, xajax.getFormValues('frmNotaDebito'));\" title=\"Seleccionar\"><img src=\"../img/iconos/tick.png\"/></button>",
					$row['id_motivo']);
			$htmlTb .= "</td>";
...
function listaNotaDebito($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 20, $totalRows = NULL) {
	...
	$cond = (strlen($sqlBusq) > 0) ? " AND " : " WHERE ";
	$sqlBusq .= $cond.sprintf("nota_debito.desincorporado != 0");
	
	if ($valCadBusq[0] != "-1" && $valCadBusq[0] != "") {
		$cond = (strlen($sqlBusq) > 0) ? " AND " : " WHERE ";
		$sqlBusq .= $cond.sprintf("nota_debito.id_empresa = %s",
			valTpDato($valCadBusq[0], "int"));
	}
			
	if ($valCadBusq[1] != "-1" && $valCadBusq[1] != "") {
		$cond = (strlen($sqlBusq) > 0) ? " AND " : " WHERE ";
		$sqlBusq .= $cond.sprintf("nota_debito.estado_documento = %s",
			valTpDato($valCadBusq[1], "int"));
	}
	
	if ($valCadBusq[2] != "" && $valCadBusq[3] != "") {
		$cond = (strlen($sqlBusq) > 0) ? " AND " : " WHERE ";
		$sqlBusq .= $cond.sprintf("DATE(nota_debito.fecha_registro) BETWEEN %s AND %s ",
	...
	if ($valCadBusq[4] != "-1" && $valCadBusq[4] != "") {
		$cond = (strlen($sqlBusq) > 0) ? " AND " : " WHERE ";
		$sqlBusq .= $cond.sprintf("(nota_debito.id_nota_debito LIKE %s 
		OR nota_debito.observaciones LIKE %s)",
	...
	$query = sprintf("SELECT 
		...
		FROM te_nota_debito_detalle_motivo te_nd_det_motivo
			INNER JOIN pg_motivo motivo ON (te_nd_det_motivo.id_motivo = motivo.id_motivo)
		WHERE te_nd_det_motivo.id_nota_debito = nota_debito.id_nota_debito) AS descripcion_motivo,
	...
	FROM te_nota_debito nota_debito
		...
		INNER JOIN bancos banco ON (cuenta.idBanco = banco.idBanco)
		INNER JOIN vw_iv_empresas_sucursales vw_iv_emp_suc ON (nota_debito.id_empresa = vw_iv_emp_suc.id_empresa_reg)
		LEFT JOIN pg_motivo motivo ON (nota_debito.id_motivo = motivo.id_motivo) %s", $sqlBusq);
	$sqlOrd = ($campOrd != "") ? sprintf(" ORDER BY %s %s", $campOrd, $tpOrd) : "";
	$queryLimit = sprintf("%s %s LIMIT %d OFFSET %d", $query, $sqlOrd, $maxRows, $startRow);
	$rsLimit = mysql_query($queryLimit);
	if (!$rsLimit) return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);	
	...
	$htmlTblIni .= "<table border=\"0\" width=\"100%\">";        
        $htmlTh .= "<tr align=\"center\" class=\"tituloColumna\">";
		$htmlTh .= ordenarCampo("xajax_listaNotaDebito", "1%", $pageNum, "estado_documento", $campOrd, $tpOrd, $valBusq, $maxRows, "");
		$htmlTh .= ordenarCampo("xajax_listaNotaDebito", "1%", $pageNum, "origen", $campOrd, $tpOrd, $valBusq, $maxRows, "");
		$htmlTh .= ordenarCampo("xajax_listaNotaDebito", "", $pageNum, "nombre_empresa", $campOrd, $tpOrd, $valBusq, $maxRows, "Empresa");
		$htmlTh .= ordenarCampo("xajax_listaNotaDebito", "7%", $pageNum, "id_nota_debito", $campOrd, $tpOrd, $valBusq, $maxRows, "Nro. Nota D&eacute;bito");
		$htmlTh .= ordenarCampo("xajax_listaNotaDebito", "5%", $pageNum, "fecha_registro", $campOrd, $tpOrd, $valBusq, $maxRows, "Fecha Registro");
		$htmlTh .= ordenarCampo("xajax_listaNotaDebito", "5%", $pageNum, "fecha_aplicacion", $campOrd, $tpOrd, $valBusq, $maxRows, "Fecha Aplicaci&oacute;n");
		$htmlTh .= ordenarCampo("xajax_listaNotaDebito", "5%", $pageNum, "fecha_conciliacion", $campOrd, $tpOrd, $valBusq, $maxRows, "Fecha Conciliaci&oacute;n");
		$htmlTh .= ordenarCampo("xajax_listaNotaDebito", "15%", $pageNum, "nombreBanco", $campOrd, $tpOrd, $valBusq, $maxRows, "Banco");
		$htmlTh .= ordenarCampo("xajax_listaNotaDebito", "15%", $pageNum, "numeroCuentaCompania", $campOrd, $tpOrd, $valBusq, $maxRows, "Cuenta");
		$htmlTh .= ordenarCampo("xajax_listaNotaDebito", "30%", $pageNum, "observaciones", $campOrd, $tpOrd, $valBusq, $maxRows, "Observaci&oacute;n");
		$htmlTh .= ordenarCampo("xajax_listaNotaDebito", "", $pageNum, "monto_nota_debito", $campOrd, $tpOrd, $valBusq, $maxRows, "Monto");
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb.= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			...
			$htmlTb .= "<td align=\"right\">".$row['id_nota_debito']."</td>";
			...
			$htmlTb .= "<td align=\"right\">".number_format($row['monto_nota_debito'],2,".",",")."</td>";
			$htmlTb .= "<td align=\"center\"><a class=\"modalImg\" id=\"aNuevo\" rel=\"#divFlotante1\" onclick=\"abrirDivFlotante1(this, '', ".$row['id_nota_debito'].");\"><img class=\"puntero\" src=\"../img/iconos/ico_view.png\" /></a></td>";
			if($row['estado_documento']==3 || $row['origen'] != 0){
				//$htmlTb .= "<td align=\"center\"><img class=\"puntero\")\" src=\"../img/iconos/ico_quitarf2.gif\" /></td>";
			}
			if($row['estado_documento']!=3 && $row['origen'] == 0){
				//$htmlTb .= "<td align=\"center\"><img class=\"puntero\")\" src=\"../img/iconos/ico_quitarf2.gif\" /></td>";
				//$htmlTb .= "<td align=\"center\"><img class=\"puntero\" onclick=\"xajax_eliminarNota(".$row['id_nota_debito'].")\" src=\"../img/iconos/ico_quitar.gif\" /></td>";
			}
                        $htmlTb .= "<td align=\"center\" ><img class=\"puntero\" onclick=\"verVentana('reportes/te_imprimir_nd_pdf.php?id=".$row['id_nota_debito']."',1100,600);\" src=\"../img/iconos/ico_print.png\"></td>";
			// Modificado Ernesto
	...
	$objResponse->assign("tdListaNotaDebito","innerHTML",$htmlTblIni.$htmlTf.$htmlTh.$htmlTb.$htmlTf.$htmlTblFin);
...
$xajax->register(XAJAX_FUNCTION,"formNotaDebito");
