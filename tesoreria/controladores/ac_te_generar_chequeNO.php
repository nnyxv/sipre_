...
function anularCheque($formCheque, $borrarISLR = "NO"){
	...
	if($rowCheque['fecha_registro'] < '2018-08-20'){		
		$montoCheque = $rowCheque['monto_cheque']/100000;
		$montoCheque = number_format($montoCheque,2);
		//$objResponse->alert($montoCheque);
	}else{
		$montoCheque = $rowCheque['monto_cheque'];
	}
	//////////////////////////////////////////////////////////////////////////////////////////////////
	
	//$saldoActual= $rowCuenta['saldo_tem'] + $rowCheque['monto_cheque'];
		
	$saldoActual= $rowCuenta['saldo_tem'] + $montoCheque;	
	...
	///////////////////////////R E C O N V E R S I Ó N   M O N E T A R I A////////////////////////////PROBAR QUITANDO ESTA SENTENCIA, NO HACE FALTA PORQUE NO ACTUALIZA EN LA TABLA TE_CHEQUE SINO QUE MUEVE EL CHEQUE A TE_CHEQUE_ANULADO
	if($rowCheque['fecha_registro'] < '2018-08-20'){
		$SqlUpCheque = sprintf("UPDATE te_cheques SET monto_cheque = ".$montoCheque."
							WHERE id_cheque = %s",							
							valTpDato($formCheque['hddIdChequeA'], "int"));
							
							$objResponse->alert($SqlUpCheque);
							
		$rsUpdateSaldoCH = mysql_query($SqlUpCheque);
		if($rsUpdateSaldoCH){ return $objResponse->alert(mysql_error()."\n\nLine: ".__LINE__); }
	}	
	//////////////////////////////////////////////////////////////////////////////////////////////////
	...
	if ($rowCheque['id_factura'] == 0) {//sino tiene documento asignado (id de factura o id de nota de cargo) puede poseer propuesta de varios pagos
		...
		while ($rowFacturasPropuesta = mysql_fetch_assoc($rsFacturasPropuesta)){//si tiene alguna propuesta
			if($rowFacturasPropuesta['tipo_documento'] == 0){//Factura
				...
				if($rowCheque['fecha_registro'] < '2018-08-20'){
					//$objResponse->alert("sin documento asgnado, propuesta-factura ".$rowFacturasPropuesta['id_factura']);
					$idFactura2 = $rowFacturasPropuesta['id_factura'];
					
					$objResponse->alert("Se esta intentando anular un cheque correspondiente a una fecha anterior a la Reconversion Monetaria. Verificando si los documentos asociados fueron convertidos, por favor espere un momento... ".$idFactura2);
					...
					if($numReg == 0){
						...
						$mensaje = "Items Actualizados. Anulando cheque, espere un momento...";
						$objResponse->alert("$mensaje");
													
					}else{						
						$objResponse->alert("Los items de la factura correspondiente al cheque fueron convertidos a Bolivares Soberanos. Anulando cheque, espere un momento...");
					}
				}
				...
				if($rowCheque['fecha_registro'] < '2018-08-20'){
					...
					//$objResponse->alert("sin documento asgnado, propuesta-ncar ".$rowFacturasPropuesta['id_factura']);
					$objResponse->alert("Se esta intentando anular un cheque correspondiente a una fecha anterior a la Reconversion Monetaria. Verificando si los documentos asociados fueron convertidos, por favor espere un momento... ".$idNotaCargo);
					...
					if($numReg == 0){
						...
						//return $objResponse->alert("Los items de la nota de cargo correspondiente al cheque ya fueron convertidos a Bolivares Soberanos. Anulando cheque, espere un momento...");
						$objResponse->alert("Los items de la nota de cargo correspondiente al cheque ya fueron convertidos a Bolivares Soberanos. Anulando cheque, espere un momento...");
				...
				//////////////////////////////////////////////////////////////////////////////////////////
				if($rowCheque['fecha_registro'] < '2018-08-20'){                                         //
					$totalMontoNotaCargo = $totalMontoNotaCargo/100000;                                  ////  
				}                                                                                         //  
				//////////////////////////////////////////////////////////////////////////////////////////
		...
		if($rowCheque['tipo_documento'] == "0"){
			...
			if($rowCheque['fecha_registro'] < '2018-08-20'){
				//$objResponse->alert("documento asgnado, factura ".$rowCheque['id_factura']);	
				
				$idFactura2 = $rowCheque['id_factura'];
					...
					if($numReg == 0){
						...
						//return $objResponse->alert("Los items de la factura correspondiente al cheque fueron convertidos a Bolivares Soberanos. Anulando cheque, espere un momento...");
						$objResponse->alert("Los items de la factura correspondiente al cheque fueron convertidos a Bolivares Soberanos. Anulando cheque, espere un momento...");
					}
				//}
			}			
			...
			if($rowCheque['fecha_registro'] < '2018-08-20'){				
				$idNotaCargo2 = $rowCheque['id_factura'];
...
//$xajax->register(XAJAX_FUNCTION,"reconversionNCargo");
