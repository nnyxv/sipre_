...
class errorMessage{
	...
	public function getMessage(){
		return utf8_decode($this->errorMsg);
	}
}
...
class field implements iAttribute{
	...
	public function applyFunction(){
		...
		if($this->getFunction()!=""){
			return $this->getFunction().sqlPS.$s.sqlPE.$alias;
		}else{
			return $s.$alias;
		}
	}
...
class query implements iAttribute{
	...
	public function getSelect(){
		...
		return $r.implode(sqlCOMA,$lista).sqlFROM.implode(sqlCOMA,$entidades).$this->getCriteria().$this->getOrder().$this->getLimit();
	}
