...
function imprimirFactura($frmBuscar){
	...
	$objResponse->script(sprintf("verVentana('reportes/cjvh_factura_cobrar_pdf.php?valBusq=%s&lstOrientacionPDF=%s',890,550)", $valBusq, $frmBuscar['lstOrientacionPDF']));
...
function listaFactura($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 20, $totalRows = NULL) {
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			...
			$htmlTb .= "<td ".$rowspan.">";
				$htmlTb .= sprintf("<a href=\"cj_facturas_por_pagar_form.php?id=%s\" target=\"_self\"><img src=\"../img/iconos/money_add.png\" title=\"Pagar Factura\"/></a>",
					$row['idFactura']);
			$htmlTb .= "</td>";
			$htmlTb .= "<td ".$rowspan.">";
