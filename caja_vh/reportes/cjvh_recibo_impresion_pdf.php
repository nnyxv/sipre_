...
while ($rowRecibo = mysql_fetch_assoc($rsRecibo)) {
	...
	switch ($rowRecibo['idTipoDeDocumento']) {
		case 4 : // 4 = ANTICIPO
			...
			$queryFactura = sprintf("SELECT cxc_ant.*,
				...
				cliente.direccion,
				cliente.estado,
				A.fecha_reconversion,
				cliente.telf,
				cxc_ant.idAnticipo
			FROM cj_cc_anticipo cxc_ant
				INNER JOIN cj_cc_cliente cliente ON (cxc_ant.idCliente = cliente.id)
			LEFT JOIN  cj_cc_anticipo_reconversion A ON (A.id_anticipo=cxc_ant.idAnticipo)
			...
			FROM cj_cc_detalleanticipo cxc_pago
				...
				LEFT JOIN  cj_cc_anticipo_reconversion A ON (A.id_anticipo=cxc_pago.idAnticipo)
	...
	$pdf->MultiCell($totalAncho,0,"",1,'C',true); // cierra linea de tabla
	$pdf->Ln();
/*************************************reconversion Monetaria 2018*****************************/
	if ($swich == 0) {
		$fecha_reconversion = null;
	}else{

		if ( $rowRecibo['reconversion']!=null) {
			$fecha_reconversion = $rowRecibo['reconversion'];
		}else if ($rowFactura['reconversion']!=null) {
			$fecha_reconversion = $rowFactura['reconversion'];
		}else{
	...
	if ($fecha_reconversion==null) {
		if ($rowRecibo['fechaDocumento']>="2018-08-01" and $rowRecibo['fechaDocumento']<"2018-08-20") {
				$pdf->SetFillColor(255,255,255);
			...
			$pdf->Ln();
		}else if ($rowRecibo['fechaDocumento']>="2018-08-20") {
				
			$pdf->SetFillColor(255,255,255);
			$pdf->Cell($arrayCol[0]['tamano'] + $arrayCol[1]['tamano'] + $arrayCol[2]['tamano'] + $arrayCol[3]['tamano'],14,"",'T',0,'L',true);
			$pdf->SetFillColor(204,204,204,204);
			$pdf->Cell($arrayCol[4]['tamano'],14,"TOTAL: ",1,0,'R',true);
			$pdf->Cell($arrayCol[5]['tamano'],14,"Bs.S ".number_format($arrayTotal[9], 2, ".", ","),1,0,'R',true);
			$pdf->Ln();
			$pdf->SetFillColor(255,255,255);
			$pdf->Cell($arrayCol[0]['tamano'] + $arrayCol[1]['tamano'] + $arrayCol[2]['tamano'] + $arrayCol[3]['tamano'],14,"",'T',0,'L',true);
			$pdf->SetFillColor(204,204,204,204);
			$pdf->Cell($arrayCol[4]['tamano'],14,"TOTAL: ",1,0,'R',true);
			$pdf->Cell($arrayCol[5]['tamano'],14,$rowMoneda['abreviacion'].number_format($arrayTotal[9]*100000, 2, ".", ","),1,0,'R',true);
			$pdf->Ln();
		}else{
			$pdf->SetFillColor(255,255,255);
			$pdf->Cell($arrayCol[0]['tamano'] + $arrayCol[1]['tamano'] + $arrayCol[2]['tamano'] + $arrayCol[3]['tamano'],14,"",'T',0,'L',true);
		...
		if ($rowRecibo['fechaDocumento']>="2018-08-01" and $rowRecibo['fechaDocumento']<"2018-08-20") {
			...
			$pdf->SetFillColor(204,204,204,204);
			$pdf->Cell($arrayCol[4]['tamano'],14,"TOTAL: ",1,0,'R',true);
			$pdf->Cell($arrayCol[5]['tamano'],14,$rowMoneda['abreviacion'].number_format($arrayTotal[9]*10000, 2, ".", ","),1,0,'R',true);
			$pdf->Ln();
