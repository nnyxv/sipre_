...
$cond = (strlen($sqlBusq) > 0) ? " AND " : " WHERE ";
$sqlBusq .= $cond.sprintf("cxc_nd.saldoNotaCargo > 0
...
if ($totalRows > 0) {
	...
	if ($_GET["lstOrientacionPDF"] == "V") {
		...
		$arrayCol = array(
			...
			array("tamano" => "40", "descripcion" => "NRO. NOTA DE DEBITO\n"),
			array("tamano" => "40", "descripcion" => "NRO. CONTROL\n"),
			array("tamano" => "120", "descripcion" => "CLIENTE\n\n"),
			array("tamano" => "70", "descripcion" => "MOTIVO\n\n"),
			array("tamano" => "45", "descripcion" => "ESTADO NOTA DE DEBITO\n"),
			array("tamano" => "50", "descripcion" => "SALDO NOTA DE DEBITO\n"),
			array("tamano" => "50", "descripcion" => "TOTAL NOTA DE DEBITO\n"));
	} else {
		// ENCABEZADO DE LA TABLA
		$arrayCol = array(
			array("tamano" => "20", "descripcion" => "\n\n"),
			...
			array("tamano" => "60", "descripcion" => "NRO. NOTA DE DEBITO\n"),
			array("tamano" => "60", "descripcion" => "NRO. CONTROL\n\n"),
			array("tamano" => "150", "descripcion" => "CLIENTE\n\n"),
			array("tamano" => "85", "descripcion" => "MOTIVO\n\n"),
			array("tamano" => "60", "descripcion" => "ESTADO NOTA DE DEBITO\n"),
			array("tamano" => "70", "descripcion" => "SALDO NOTA DE DEBITO\n"),
			array("tamano" => "70", "descripcion" => "TOTAL NOTA DE DEBITO\n"));
	}
	...
	$pdf->Cell($totalAncho,10,"NOTAS DE DEBITO POR COBRAR",0,0,'C');
	...
	while ($row = mysql_fetch_assoc($rs)) {
		...
		$pdf->Cell($arrayCol[5]['tamano'],14,($row['numeroControlNotaCargo']),'LR',0,'R',true);
		$pdf->Cell($arrayCol[6]['tamano'],14,($row['id_cliente'].".- ".$row['nombre_cliente']),'LR',0,'L',true);
		$pdf->Cell($arrayCol[7]['tamano'],14,($row['descripcion_motivo']),'LR',0,'L',true);
		$pdf->Cell($arrayCol[8]['tamano'],14,($row['descripcion_estado_nota_cargo']),'LR',0,'C',true);
		$pdf->Cell($arrayCol[9]['tamano'],14,number_format($row['saldoNotaCargo'], 2, ".", ","),'LR',0,'R',true);
		$pdf->Cell($arrayCol[10]['tamano'],14,number_format($row['montoTotalNotaCargo'], 2, ".", ","),'LR',0,'R',true);
		$pdf->Ln();
