...
if(!(validaAcceso("cj_historico_cierre_list"))) {
...
<head>
	...
	<title>.: SIPRE <?php echo cVERSION; ?> :. <?php echo $nombreCajaPpal; ?> - Histórico de Cierre <?php echo (count($arrayTipoPago) == 1) ? $arrayDescripcionTipoPago[$arrayTipoPago[0]] : ""; ?></title>
...
<div id="divGeneralPorcentaje">
    <div id="divInfo">
        ...
        <tr>
        	<td align="center" class="tituloPaginaCaja">Corte de Caja <?php echo (count($arrayTipoPago) == 1) ? $arrayDescripcionTipoPago[$arrayTipoPago[0]] : ""; ?> (<?php echo $nombreCajaPpal; ?>)<br/><span class="textoNegroNegrita_10px">(Resumen de Saldos)</span></td>
        </tr>
        </table>
        ...
        if ($rowAperturaCaja = mysql_fetch_array($rsAperturaCaja)){
            $txtFechaActual = date("Y-m-d", strtotime($rowAperturaCaja['fechaCierre']));
        ...
        <form id="frmCierre" name="frmCierre">
            ...
            if ($rowConfig400['valor'] == 0) { // MOSTRAR POR SUCURSALES
                $andEmpresaAPER = sprintf(" AND ape.id_empresa = %s",
                    valTpDato($idEmpresa, "int"));
                $andEmpresaCIER = sprintf(" AND cierre.id_empresa = %s",
                    valTpDato($idEmpresa, "int"));
            } else if ($rowConfig400['valor'] == 1) { // MOSTRAR POR EMPRESAS
                $andEmpresaAPER = "";
                $andEmpresaCIER = "";
            ...
            FROM ".$apertCajaPpal." ape
				...
				INNER JOIN vw_iv_usuarios vw_iv_usuario_cierre ON (cierre.id_usuario = vw_iv_usuario_cierre.id_usuario)
            WHERE cierre.idCierre = %s %s;",
				valTpDato(spanDatePick, "date"),
				valTpDato($_GET['idCierre'], "int"),
				$andEmpresaAPER);
            $rsAperturaCaja = mysql_query($queryAperturaCaja);
            ...
            while ($rowAperturaCaja = mysql_fetch_assoc($rsAperturaCaja)) {
                ...
                <tr align="left">
                    <td colspan="5"><textarea id="txtObservacionCierre" name="txtObservacionCierre" readonly="readonly" rows="3" style="width:99%"><?php echo $rowAperturaCaja['observacion']; ?></textarea></td>
                </tr>
                <tr>
                ...
                <tr align="center" style="font-size:12px;">
                    ...
                    <td>
						...
						WHERE vw_pg_empleado.id_empresa = %s
							...
							AND vw_pg_empleado.activo = 1;",
							valTpDato($idEmpresa, "int"),
							valTpDato(10, "int")); // 10 = JEFE FACTURACION DE VEHICULOS
                        $rsEmpleado = mysql_query($queryEmpleado);
                        if (!$rsEmpleado) die(mysql_error()."<br>Error Nro: ".mysql_errno()."<br>Line: ".__LINE__);
                        if ($rowEmpleado = mysql_fetch_array($rsEmpleado)) {
                            echo utf8_encode($rowEmpleado['nombre_empleado']);
            ...
            if (!($totalRowsAperturaCaja > 0)) { ?>
                ...
                window.location.href="cj_apertura_caja.php";
                </script>
            <?php
            } ?>
        </form>
...
function saldosDeCierre($montoApertura){
	...
	WHERE cxc_pago.idCierre = ".valTpDato($_GET['idCierre'], "int")."
	...
	WHERE cxc_ch.idCierre = ".valTpDato($_GET['idCierre'], "int")."
	...
	WHERE cxc_tb.idCierre = ".valTpDato($_GET['idCierre'], "int")."
	...
	if (in_array(idArrayPais,array(1))) { // 1 = Venezuela, 2 = Panama, 3 = Puerto Rico
		$queryNumerosControl = sprintf("SELECT
			MIN(q.numerocontrol) AS primernrocontrol,
			MAX(q.numerocontrol) AS ultimonrocontrol
		FROM (SELECT cxc_fact.numerocontrol FROM cj_cc_encabezadofactura cxc_fact
				WHERE cxc_fact.idDepartamentoOrigenFactura IN (%s)
					AND cxc_fact.aplicaLibros = 1
					AND cxc_fact.fechaRegistroFactura = %s %s
				
				UNION
				
				SELECT cxc_nc.numeroControl FROM cj_cc_notacredito cxc_nc
				WHERE cxc_nc.idDepartamentoNotaCredito IN (%s)
					AND cxc_nc.aplicaLibros = 1
					AND cxc_nc.fechaNotaCredito = %s %s) AS q;",
			...
			valTpDato($fechaApertura, "date"),
			$andEmpresaNC);
		$rsNumerosControl = mysql_query($queryNumerosControl);
		if (!$rsNumerosControl) die(mysql_error()."<br>Error Nro: ".mysql_errno()."<br>Line: ".__LINE__);
		$rowNumerosControl = mysql_fetch_assoc($rsNumerosControl);
	
		$varPrimerNroControl = $rowNumerosControl['primernrocontrol'];
		$varUltimoNroControl = $rowNumerosControl['ultimonrocontrol'];
