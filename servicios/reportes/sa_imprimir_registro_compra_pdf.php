...
FROM sa_orden
	...
	LEFT JOIN pg_usuario ON (sa_orden_tot.id_usuario = pg_usuario.id_usuario)
	LEFT JOIN cp_factura_iva ON (cp_factura.id_factura = cp_factura_iva.id_factura)
	LEFT JOIN pg_empleado ON (pg_empleado.id_empleado = pg_usuario.id_empleado)
	#LEFT OUTER JOIN pg_iva ON (cp_factura_iva.id_iva = pg_iva.idIva)
WHERE cp_factura.id_factura = %s",
...
imagestring($img,1,440,50,date(spanDateFormat, strtotime($row['fecha_origen'])),$textColor); // <----

imagestring($img,1,340,60,"FECHA VENCIMIENTO",$textColor);
imagestring($img,1,430,60,":",$textColor);
imagestring($img,1,440,60,date(spanDateFormat, strtotime($row['fecha_vencimiento'])),$textColor); // <----
//imagestring($img,1,440,60,date(spanDateFormat, strtotime($row['fecha_vencimiento']))." CRED. ".$row['diasDeCredito']." D".utf8_decode("Í")."AS",$textColor); // <----
...
WHERE id_orden_tot = %s",
	valTpDato($row['id_orden_tot'],"int"));
...
$sqlIvaTot = sprintf("SELECT cp_factura_iva.subtotal_iva, 
				        ...
				        pg_iva.observacion 
				        FROM cp_factura_iva 
				        LEFT JOIN pg_iva ON cp_factura_iva.id_iva = pg_iva.idIva 
				        WHERE id_factura = %s GROUP BY id_iva",
...
*/
	...
	if ($row['reconversion']==null) {
			...
			$posX = 510+((5*13)-(strlen($totalFacturaa)*5));
		    ...
		    if ($row['fecha_origen']>='2018-08-01' and $row['fecha_origen']<'2018-08-20') {
		        $totalFactura = number_format($row['montoTotalFactura']*100000,2,".",",");
		        $posX = 500+((5*13)-(strlen($totalFactura)*5));
		        imagestring($img,1,360,620,"TOTAL",$textColor);//antes 630 ahora $lineaY
		        imagestring($img,1,485,620,":",$textColor);
		        imagestring($img,1,$posX,620,$totalFactura,$textColor); // <----
		        $totalFacturaa = number_format($row['montoTotalFactura'],2,".",",");
