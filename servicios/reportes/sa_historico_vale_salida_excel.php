...
if ($valCadBusq[1] != "" && $valCadBusq[2] != "") {
	$cond = (strlen($sqlBusq) > 0) ? " AND " : " WHERE ";
	$sqlBusq .= $cond.sprintf("DATE(sa_vale_salida.fecha_vale) BETWEEN %s AND %s",
...
if ($valCadBusq[6] != "-1" && $valCadBusq[6] != "") {
	...
	OR chasis LIKE %s)",
		...
		$sqlBusq .= $cond.sprintf("sa_det_vale_salida_articulo.id_det_vale_salida_articulo IS NOT NULL");
                $join .= " LEFT JOIN sa_det_vale_salida_articulo ON sa_vale_salida.id_vale_salida = sa_det_vale_salida_articulo.id_vale_salida ";
	}
		...
		$sqlBusq .= $cond.sprintf("sa_det_vale_salida_tempario.id_det_vale_salida_tempario IS NOT NULL");
                $join .= " LEFT JOIN sa_det_vale_salida_tempario ON sa_vale_salida.id_vale_salida = sa_det_vale_salida_tempario.id_vale_salida ";
	}
        if ($valCadBusq[9] != "-1" && $valCadBusq[9] != "") {//con TOT
		$cond = (strlen($join) > 0) ? $andOr : " AND ( ";
		$sqlBusq .= $cond.sprintf("sa_det_vale_salida_tot.id_det_vale_salida_tot IS NOT NULL");
                $join .= " LEFT JOIN sa_det_vale_salida_tot ON sa_vale_salida.id_vale_salida = sa_det_vale_salida_tot.id_vale_salida ";
	}
		...
		$sqlBusq .= $cond.sprintf("sa_det_vale_salida_notas.id_det_vale_salida_nota IS NOT NULL");
                $join .= " LEFT JOIN sa_det_vale_salida_notas ON sa_vale_salida.id_vale_salida = sa_det_vale_salida_notas.id_vale_salida ";
	}
...
$queryMaestro = sprintf("SELECT *,
	...
	color_fuente,
	id_orden_retrabajo,	
        (IFNULL((((orden.subtotal - orden.subtotal_descuento) * orden.iva) / 100),0) + (orden.subtotal - orden.subtotal_descuento)) AS total
FROM sa_orden orden
	...
	INNER JOIN sa_estado_orden estado_orden ON (orden.id_estado_orden = estado_orden.id_estado_orden)
        INNER JOIN sa_vale_salida ON (orden.id_orden = sa_vale_salida.id_orden)
	LEFT JOIN sa_retrabajo_orden orden_retrabajo ON (orden.id_orden = orden_retrabajo.id_orden) %s %s 
GROUP BY sa_vale_salida.id_vale_salida
ORDER BY sa_vale_salida.numero_vale DESC  ",
        $join,
        $sqlBusq
        );

$rsMaestro = mysql_query($queryMaestro);
if (!$rsMaestro) die(mysql_error()."<br><br>Line: ".__LINE__."<br>".$queryMaestro);
...
while ($rowMaestro = mysql_fetch_assoc($rsMaestro)) {
	...
	$arrayCol[$contFila][0] = date("d-m-Y",strtotime($rowMaestro['fecha_vale']));
	$arrayCol[$contFila][1] = $rowMaestro['numero_vale'];
	$arrayCol[$contFila][2] = date("d-m-Y",strtotime($rowMaestro['tiempo_orden']));
	$arrayCol[$contFila][3] = ($rowMaestro['numero_orden']);
	$arrayCol[$contFila][4] = ($rowMaestro['numeracion_recepcion']);
	$arrayCol[$contFila][5] = htmlentities($rowMaestro['nombre_empleado'])." ";
	$arrayCol[$contFila][6] = htmlentities($rowMaestro['nombre_tipo_orden']);
	$arrayCol[$contFila][7] = htmlentities($rowMaestro['nombre_cliente']);
	$arrayCol[$contFila][8] = htmlentities($rowMaestro['nom_uni_bas']);
	$arrayCol[$contFila][9] = htmlentities($rowMaestro['placa']);
	$arrayCol[$contFila][10] = htmlentities($rowMaestro['chasis']);
	$arrayCol[$contFila][11] = htmlentities($rowMaestro['nombre_estado']);
	$arrayCol[$contFila][12] = ($rowMaestro['id_orden_retrabajo']);
	$arrayCol[$contFila][13] = round($rowMaestro['total'],2);
	
	// TOTALES
	$arrayTotalPagina[13] += $arrayCol[$contFila][13];
}
...
if (isset($arrayPagina)) {
	foreach ($arrayPagina as $indice => $valor) {
		...
		$excel->add_row(array("","","","","","","","","","","","","","","","","","","",""));
		$excel->add_row(array('ORDENES DE SERVICIO - VALES DE SALIDA'."|19"), 'trCabecera');
		$excel->add_row(array("","","","","","","","","","","","","","","","","","","",""));
...
$excel->download('ERP_Orden_Servicio_vales_salida.xls');
?>