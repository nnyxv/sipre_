...
ORDER BY cj_cc_notacredito.numeracion_nota_credito DESC", 
        $join,
        $sqlBusq);
...
while ($rowMaestro = mysql_fetch_assoc($rsMaestro)) {
	...
	$arrayCol[$contFila][7] = htmlentities($rowMaestro['nombre_empleado'])." ";
	$arrayCol[$contFila][8] = htmlentities($rowMaestro['nombre_tipo_orden']);
	$arrayCol[$contFila][9] = htmlentities($rowMaestro['nombre_cliente']);
	$arrayCol[$contFila][10] = htmlentities($rowMaestro['nom_uni_bas']);
	$arrayCol[$contFila][11] = htmlentities($rowMaestro['placa']);
	$arrayCol[$contFila][12] = htmlentities($rowMaestro['chasis']);
	$arrayCol[$contFila][13] = htmlentities($rowMaestro['nombre_estado']);
	$arrayCol[$contFila][14] = ($rowMaestro['id_orden_retrabajo']);
	$arrayCol[$contFila][15] = $rowMaestro['total'];
	
	// TOTALES
	$arrayTotalPagina[15] += $arrayCol[$contFila][15];
}
...
$excel->download('ERP_Orden_Servicio_Notas.xls');
?>