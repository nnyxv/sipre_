<?php
@session_start();
define('PAGE_PRIV','sa_informe_cliente_taller');
...
//implementando xajax;
	...
	function load_page($args=''){//$page,$maxrows,$order,$ordertype,$capa,
		...
		if($argumentos['h_estado']!=''){
			$query->where(new criteria(sqlEQUAL,"id_estado_orden",18));
		}
		...
		if($rec){
			if($rec->getNumRows()==0){
				...
				<td>'.$paginador->get($listado->fact,'N Factura').'</td>
				...
				<td>'.$paginador->get($listado->tipo_auto,'Modelo').'</td>
				...
				foreach($rec as $v){
                    ...
                    <td align="center">'.$row1['numeroFactura'].'</td>
	...
	$estados=$c->sa_estado_orden->doQuery($c,new criteria(sqlEQUAL,'activo',1))->where(new criteria(sqlEQUAL,'id_estado_orden',18))->orderBy('orden')->doSelect()->getAssoc('id_estado_orden','nombre_estado');
...
<div style="width:1200px; background:#FFFFFF; margin: auto;">
	<table align="center" border="0" width="100%">
		<tr>
			<td align="right" class="titulo_pagina" ><span ></span><br />
