...
function cargaLstTipoOrden($selId = "", $idTipoValeRecepcion = NULL) {
	...
	WHERE pg_usuario.id_usuario = %s
		AND sa_permiso_tipo_orden_usuario.id_empresa = %s
	...
	$html = "<select id=\"lstTipoOrden\" name=\"lstTipoOrden\" onchange=\"
		...
		var tipoOrdenSelec = this.value;
		var filtroOrdenSelect = xajax.call('buscarFiltroOrden', {mode:'synchronous', parameters:[tipoOrdenSelec]});		
		var filtroOrdenAnt = xajax.call('buscarFiltroOrden', {mode:'synchronous', parameters:[$('hddTipoOrdenAnt').value]});		
		var validarCambio = true;
		
		// Permitir cambio de (contado a credito) o de (credito a contado) o a (activos)
		if ((filtroOrdenSelect == 1 && filtroOrdenAnt == 2) || (filtroOrdenSelect == 2 && filtroOrdenAnt == 1)
			|| (filtroOrdenSelect == 7 && filtroOrdenAnt == 8) || (filtroOrdenSelect == 8 && filtroOrdenAnt == 7)
			|| filtroOrdenSelect == 4) {
			validarCambio = false;
		}
		
		if ($('hddItemsCargados').value > 0 && validarCambio == true) {
			...
			if ($('hddTipoOrdenAnt').value > 0 || filtroOrdenSelect != 1) {
				abrir2();
