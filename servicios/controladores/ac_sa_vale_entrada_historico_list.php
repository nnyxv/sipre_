...
function buscarOrden($valForm) {
	...
	$objResponse->loadCommands(listadoOrdenes(0, "numero_vale_entrada", "DESC", $valBusq));
...
function cargaLstEmpleado($selId = "", $nombreObjeto = "", $objetoDestino = "", $idEmpresa = "") {
	...
	FROM sa_vale_entrada vale_entrada
		INNER JOIN vw_pg_empleados empleado ON (vale_entrada.id_empleado = empleado.id_empleado)
		WHERE vale_entrada.id_empresa = ".$idEmpresa."
...
function listadoOrdenes($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 20, $totalRows = NULL) {
	...
	if ($valCadBusq[0] != "-1" && $valCadBusq[0] != "") {
		$cond = (strlen($sqlBusq) > 0) ? " AND " : " WHERE ";
		$sqlBusq .= $cond.sprintf("sa_vale_entrada.id_empresa = %s",
	...
	if ($valCadBusq[1] != "" && $valCadBusq[2] != "") {
		...
		$sqlBusq .= $cond.sprintf("DATE(sa_vale_entrada.fecha_creada) BETWEEN %s AND %s",
	...
	if ($valCadBusq[3] != "-1" && $valCadBusq[3] != "") {
		$cond = (strlen($sqlBusq) > 0) ? " AND " : " WHERE ";
		$sqlBusq .= $cond.sprintf("sa_vale_entrada.id_empleado = %s",
	...
	if ($valCadBusq[6] != "-1" && $valCadBusq[6] != "") {
		...
		OR sa_vale_entrada.numero_vale_entrada LIKE %s
		OR sa_vale_entrada.numero_vale_salida LIKE %s
	...
	$query = sprintf("SELECT
		sa_vale_entrada.id_vale_salida,
		sa_vale_entrada.id_vale_entrada,
		sa_vale_entrada.fecha_vale_salida,
		sa_vale_entrada.fecha_creada,
		sa_vale_entrada.numero_vale_salida,
		sa_vale_entrada.numero_vale_entrada,
		sa_vale_entrada.estado_vale,
		orden.tiempo_orden,
		orden.id_orden,
		orden.numero_orden,
		orden.id_recepcion,
	...
	FROM sa_orden orden
		...
		INNER JOIN sa_vale_entrada ON (orden.id_orden = sa_vale_entrada.id_orden) %s", $sqlBusq);
...
//		$htmlTh .= "<td colspan=\"2\"></td>";
		$htmlTh .= ordenarCampo("xajax_listadoOrdenes", "6%", $pageNum, "fecha_creada", $campOrd, $tpOrd, $valBusq, $maxRows, ("Fecha"));
		$htmlTh .= ordenarCampo("xajax_listadoOrdenes", "6%", $pageNum, "numero_vale_entrada", $campOrd, $tpOrd, $valBusq, $maxRows, ("N° Vale Entrada"));
		$htmlTh .= ordenarCampo("xajax_listadoOrdenes", "6%", $pageNum, "numero_vale_salida", $campOrd, $tpOrd, $valBusq, $maxRows, ("N° Vale Salida"));
...
//			$htmlTb .= "<td align=\"center\">".$imgEstatusPedido."</td>";
			$htmlTb .= "<td align=\"center\" nowrap=\"nowrap\">".date("d-m-Y",strtotime($row['fecha_creada']))."</td>";
			$htmlTb .= "<td align=\"right\" idvaleentradaoculto=\"".$row['id_vale_entrada']."\">".($row['numero_vale_entrada'])."</td>";
			$htmlTb .= "<td align=\"right\" idvalesalidaoculto=\"".$row['id_vale_salida']."\">".($row['numero_vale_salida'])."</td>";
			...
			//impresion vale pdf
				$htmlTb .= sprintf("<img class=\"puntero\" onclick=\"verVentana('sa_devolucion_vale_salida_pdf.php?valBusq=0|%s', 1000, 500);\" src=\"../img/iconos/ico_print.png\" title=\"Ver PDF Vale de Entrada\"/>",
								$row['id_vale_salida']);
