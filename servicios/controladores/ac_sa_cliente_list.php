...
function cargaLstModulo($selId = ""){
	...
	$query = sprintf("SELECT * FROM pg_modulos WHERE id_modulo IN(0,1,2)");
	$rs = mysql_query($query);
	if (!$rs) return $objResponse->alert(mysql_error()."\n\nLine: ".__LINE__."\n\nSQL: ".$query);
...
function formCliente($idCliente, $frmCliente) {
	...
	if ($idCliente > 0) {
		if (!xvalidaAcceso($objResponse,"sa_cliente_list","editar")) { usleep(0.5 * 1000000); $objResponse->script("byId('btnCancelarCliente').click();"); return $objResponse; }
		...
		if (!xvalidaAcceso($objResponse,"sa_cliente_list","insertar")) { usleep(0.5 * 1000000); $objResponse->script("byId('btnCancelarCliente').click();"); return $objResponse; }
		...
		$objResponse->call("selectedOption","lstTipoCliente","Servicios");
...
function guardarCliente($frmCliente, $frmListaCliente) {
	...
	if ($idCliente > 0) {
		...
		|| (($frmCliente['lstCredito'] != "0" || $frmCliente['lstCredito'] != "Si") && xvalidaAcceso($objResponse,"sa_cliente_list","editar")))) {
		...
		|| (($frmCliente['lstCredito'] != "0" || $frmCliente['lstCredito'] != "Si") && xvalidaAcceso($objResponse,"sa_cliente_list","insertar")))) {
			errorGuardarCliente($objResponse); return $objResponse;
		}
...
function listaCliente($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 20, $totalRows = NULL) {
	...
	FROM cj_cc_cliente cliente
		LEFT OUTER JOIN cj_cc_cliente_empresa cliente_emp ON (cliente.id = cliente_emp.id_cliente) %s", $sqlBusq);
...
function insertarItemClienteEmpresa($contFila, $idClienteEmpresa = "", $idEmpresa = "") {
	...
	if ($idClienteEmpresa > 0) {
		...
		FROM cj_cc_credito cred
			RIGHT OUTER JOIN cj_cc_cliente_empresa cliente_emp ON (cred.id_cliente_empresa = cliente_emp.id_cliente_empresa)
