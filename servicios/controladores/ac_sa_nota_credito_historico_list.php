...
function buscarOrden($valForm) {
	...
	$objResponse->loadCommands(listadoOrdenes(0, "numeracion_nota_credito", "DESC", $valBusq));
...
function exportarHistorico($valForm) {
	...
	$objResponse->script("window.open('reportes/sa_historico_nota_credito_excel.php?valBusq=".$valBusq."','_self');");
...
function listadoOrdenes($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 20, $totalRows = NULL) {
	...
	$query = sprintf("SELECT
		cj_cc_notacredito.idNotaCredito,
		cj_cc_notacredito.numeracion_nota_credito,
		cj_cc_notacredito.montoNetoNotaCredito,
		cj_cc_notacredito.numeroControl,
		cj_cc_notacredito.fechaNotaCredito,
	...
	FROM sa_orden orden
		...
		RIGHT JOIN cj_cc_encabezadofactura fact_vent ON (orden.id_orden = fact_vent.numeroPedido)
        RIGHT JOIN cj_cc_notacredito ON (fact_vent.idFactura = cj_cc_notacredito.idDocumento) %s %s 
	...
	$htmlTh .= "<tr align=\"center\" class=\"tituloColumna\">";
		...
		$htmlTh .= "<td colspan=\"1\"></td>";
		$htmlTh .= ordenarCampo("xajax_listadoOrdenes", "6%", $pageNum, "fechaNotaCredito", $campOrd, $tpOrd, $valBusq, $maxRows, ("Fecha NC"));
		$htmlTh .= ordenarCampo("xajax_listadoOrdenes", "6%", $pageNum, "numeracion_nota_credito", $campOrd, $tpOrd, $valBusq, $maxRows, ("Nº NC"));
		$htmlTh .= ordenarCampo("xajax_listadoOrdenes", "6%", $pageNum, "numeroFactura", $campOrd, $tpOrd, $valBusq, $maxRows, ("N° Factura"));
		$htmlTh .= ordenarCampo("xajax_listadoOrdenes", "6%", $pageNum, "numeroControl", $campOrd, $tpOrd, $valBusq, $maxRows, ("N° Control NC"));
		...
		$htmlTh .= ordenarCampo("xajax_listadoOrdenes", "8%", $pageNum, "montoNetoNotaCredito", $campOrd, $tpOrd, $valBusq, $maxRows, ("Total NC"));
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb.= "<tr class=\"".$clase."\" onmouseover=\"this.className = 'trSobre';\" onmouseout=\"this.className = '".$clase."';\" height=\"22\">";
			...
			$htmlTb .= "<td align=\"center\" nowrap=\"nowrap\">".date("d-m-Y",strtotime($row['fechaNotaCredito']))."</td>";
			...
			$htmlTb .= "<td class=\"noprint\" style=\"white-space:nowrap;\">";
				...
				$htmlTb .= sprintf("<img class=\"puntero\" onclick=\"verVentana('reportes/sa_devolucion_venta_pdf.php?valBusq=%s', 1000, 500);\" src=\"../img/iconos/ico_print.png\" title=\"Ver PDF Nota de Crédito\"/>",
					$row['idNotaCredito']);		
