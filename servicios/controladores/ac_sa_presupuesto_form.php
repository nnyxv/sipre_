...
//		
	...
	if($_GET['acc_ret'] != 1) { //CUANDO SE GENERA LA ORDEN RETRABAJO
		$queryRepuestosGenerales = sprintf("SELECT
			...
			%s.id_precio,
			%s.precio_unitario,
			(SELECT GROUP_CONCAT(id_iva) FROM sa_det_presup_articulo_iva WHERE sa_det_presup_articulo_iva.id_det_presup_articulo = %s.%s) as id_iva,
			(SELECT GROUP_CONCAT(iva) FROM sa_det_presup_articulo_iva WHERE sa_det_presup_articulo_iva.id_det_presup_articulo = %s.%s) as iva,
			%s.id_articulo,
			%s.%s,
			%s.aprobado,
			%s.costo,			
		...
		while ($rowDetRep = mysql_fetch_assoc($rsDetRep)) {
			...
			if ($_GET['cons'] == 1) { // FACTURACION
				...
				$rowDetRep['id_det_orden_articulo']);
			...
			WHERE sa_det_orden_articulo.id_det_orden_articulo = %s
				...
				valTpDato($rowDetRep['id_det_orden_articulo'],"int"));//AND sa_solicitud_repuestos.estado_solicitud != 0
			$rs = mysql_query($query);
			...
			if ($rowDetRep['aprobado'] == 1) {
				...
				if ($valFormTotalDcto['hddAccionTipoDocumento']!=4 && $valFormTotalDcto['hddAccionTipoDocumento']!=3) {
			...
			$solicitudRep = "";
                        ...
                        if($rowDetRep['id_det_orden_articulo']){
                            $sqlSolicitudRep= "SELECT * FROM sa_det_solicitud_repuestos
                            WHERE id_det_orden_articulo = ".$rowDetRep['id_det_orden_articulo']."
			...
			$sigValor, $clase, $sigValor,
				...
				$cantidad_art_total,
				$sigValor, cantidadSolicitada($rowDetRep['id_det_orden_articulo']), // Articulos Solicitados total
				$sigValor, cantidadDespachada($rowDetRep['id_det_orden_articulo']), // Articulos Despachados antes $cantidad_art
...
//		}
		...
		} else if ($valFormTotalDcto['hddTipoDocumento'] == 1 && $valFormTotalDcto['hddAccionTipoDocumento'] != 3) { //PRESUPUESTO
			if ($sigValor == 1) {
				$objResponse->script("
				$('frmListaArticulo').style.display='none';
				$('tblListaArticulo').style.display='none';");
		...
		$queryDetPaq = sprintf("SELECT 
			...
			(CASE (SELECT id_estado_orden FROM %s WHERE %s = %s LIMIT 1)
				WHEN 13 THEN
					...
					WHERE %s.%s = %s
						AND %s.id_paquete = idPaq
						AND (SELECT COUNT(*) FROM sa_det_presup_articulo_iva WHERE sa_det_presup_articulo_iva.id_det_presup_articulo = %s.id_det_presup_articulo) = 0
						AND %s.estado_articulo <> 'DEVUELTO'
						AND (sa_det_solicitud_repuestos.id_estado_solicitud = 3
							OR sa_det_solicitud_repuestos.id_estado_solicitud = 5))
				ELSE
		...
		while ($rowDetPaq = mysql_fetch_assoc($rsDetPaq)) {//de cada paquete se consulta sus items
			...
			while ($valorRepuestoPaq = mysql_fetch_array($rsRepuestoPaq)) {
				...
				WHERE det_orden_art.id_det_orden_articulo = %s
					...
					valTpDato($valorRepuestoPaq['id_det_orden_articulo'],"int"));
				$rs = mysql_query($query);
				if (!$rs) return $objResponse->alert(mysql_error()."\n\nLine: ".__LINE__);
				$row = mysql_fetch_assoc($rs);
			...
			if ($value_checkedPaq == 1) {
                            ...
                            $sqlIvasIdPorcPaq = sprintf("SELECT GROUP_CONCAT(sa_det_presup_articulo_iva.id_iva) AS id_iva_paquete, GROUP_CONCAT(sa_det_presup_articulo_iva.iva) AS porc_iva_paquete, GROUP_CONCAT(sa_det_presup_articulo.precio_unitario*sa_det_presup_articulo.cantidad) AS base_imponible_paquete
                                                        FROM sa_det_presup_articulo
                                                        INNER JOIN sa_det_presup_articulo_iva ON sa_det_presup_articulo.id_det_presup_articulo = sa_det_presup_articulo_iva.id_det_presup_articulo
                                                        WHERE id_presupuesto = %s AND id_paquete = %s AND estado_articulo != 'DEVUELTO'",
                                                valTpDato($idDocumento,"int"),
...
//				}
		...
		if ($valFormTotalDcto['hddAccionTipoDocumento'] == 2 || $valFormTotalDcto['hddAccionTipoDocumento'] == 4) {
			...
			if($valFormTotalDcto['hddTipoDocumento']==1) { //PRESUPUESTO
				if($sigValor==1) {
					$objResponse->script("
					$('frmListaManoObra').style.display='';");
				}
			}
		}
	...
	}//FIN CUANDO GENERA ORDEN DE RETRABAJO
	...
	if($valFormTotalDcto['hddTipoDocumento'] == 1){//1 es Presupuesto
		$numeroOrdenPresupuestoMostrar = $numeroPresupuestoMostrar;
                $queryNumeroOrden = sprintf("SELECT numero_orden "
                        . "FROM sa_presupuesto "
                        . "LEFT JOIN sa_orden ON sa_presupuesto.id_orden = sa_orden.id_orden "
                        . "WHERE id_presupuesto = %s",
                        valTpDato($idDocumento,"int"));
                $rsNumeroOrden = mysql_query($queryNumeroOrden);
                if(!$rsNumeroOrden){ return $objResponse->alert(mysql_error()."\n\nLine: ".__LINE__); }
                $rowNumeroOrden = mysql_fetch_assoc($rsNumeroOrden);
                
                $numeroOrdenMostrar = $rowNumeroOrden["numero_orden"];
                //presupuesto
...
//		}
                $idDocumentoVenta = $valFormDcto['txtIdPresupuesto'];
		if($idDocumentoVenta =="" || $idDocumentoVenta == " "){
                    return $objResponse->alert("No hay documento de venta".$valFormDcto['txtIdPresupuesto']);
                }
				...
				if ($valFormListaArt['hddIdPedDet'.$valor] != "") {
					...
					if ($valFormTotalDcto['hddTipoDocumento'] == 1) {
						$queryActualizarArt = sprintf("UPDATE sa_det_presup_articulo SET
							aprobado = %s
						WHERE id_det_presup_articulo = %s",
				...
				if ($valFormListTemp['hddIdPedDetTemp'.$valorTemp] != "") {					
					...
					if ($valFormTotalDcto['hddTipoDocumento'] == 1) {
						$queryActualizarTemp = sprintf("UPDATE sa_det_presup_tempario SET
							aprobado = %s
						WHERE id_det_presup_tempario = %s",
...
// VERIFICA SI LOS PAQUETES ALMACENADOS EN LA BD DE LA ORDEN AUN ESTAN AGREGADOS EN EL FORMULARIO
		...
		FROM sa_det_presup_articulo		
			INNER JOIN sa_paquetes ON (sa_det_presup_articulo.id_paquete = sa_paquetes.id_paquete)
			INNER JOIN sa_presupuesto ON (sa_det_presup_articulo.id_presupuesto = sa_presupuesto.id_presupuesto)
			INNER JOIN sa_det_presup_tempario ON (sa_paquetes.id_paquete = sa_det_presup_tempario.id_paquete)
		WHERE sa_presupuesto.id_presupuesto = %s
			AND sa_det_presup_articulo.estado_articulo = 'PENDIENTE'
			AND sa_det_presup_tempario.estado_tempario = 'PENDIENTE';",
			valTpDato($valFormDcto['txtIdPresupuesto'], "int"));
		...
		while ($rowPedidoDet = mysql_fetch_assoc($rsPedidoDet)) {
			...
			if ($existRegDet == false) {
				$deleteSQL = sprintf("DELETE FROM sa_det_presup_articulo
				WHERE id_presupuesto = %s
					AND id_paquete = %s
				...
				$deleteSQL = sprintf("DELETE FROM sa_det_presup_tempario
				WHERE id_presupuesto = %s
					AND id_paquete = %s
				...
				$queryArtPaq = sprintf("SELECT * FROM sa_det_presup_articulo
				WHERE id_presupuesto = %s
					AND id_paquete = %s
					AND estado_articulo = 'PENDIENTE';",
					valTpDato($valFormDcto['txtIdPresupuesto'], "int"),
				...
				while ($rowArtPaq = mysql_fetch_assoc($rsArtPaq)) {
					...
					if ($existArtPaq == false || $valFormPaq['hddIdPedDetPaq'.$valorItmPaq] == "") {
						...
						WHERE id_det_presup_articulo = %s
							AND estado_articulo = 'PENDIENTE';",
							valTpDato($rowArtPaq['id_det_presup_articulo'], "int"));
						$Result1 = mysql_query($deleteSQL);
						if (!$Result1) return $objResponse->alert(mysql_error()."\n\nLine: ".__LINE__);
					}
				}
				...
				$queryTempPaq = sprintf("SELECT * FROM sa_det_presup_tempario
				WHERE id_presupuesto = %s
					AND id_paquete = %s
					AND estado_tempario = 'PENDIENTE';",
					valTpDato($valFormDcto['txtIdPresupuesto'], "int"),
				...
				while ($rowTempPaq = mysql_fetch_assoc($rsTempPaq)) {
					...
					if ($existTempPaq == false || $valFormPaq['hddIdPedDetPaq'.$valorItmPaq] == "") {
						...
						WHERE id_det_presup_tempario = %s
							AND estado_tempario = 'PENDIENTE';",
							valTpDato($rowTempPaq['id_det_presup_tempario'], "int"));
						$Result1 = mysql_query($deleteSQL);
						if (!$Result1) return $objResponse->alert(mysql_error()."\n\nLine: ".__LINE__);
					}
				}
		...
		$queryOrdenDetRep = sprintf("SELECT * FROM sa_det_presup_articulo
		WHERE id_presupuesto = %s
		...
		while ($rowOrdenDetRep = mysql_fetch_assoc($rsOrdenDetRep)) {
			...
			if (isset($arrayObj)) {
				foreach($arrayObj as $indice => $valor) {
					if ($rowOrdenDetRep['id_det_presup_articulo'] == $valFormListaArt['hddIdPedDet'.$valor]) {
						$existRegDet = true;
					}
				}
			}
			...
			if ($existRegDet == false) {
				if($rowOrdenDetRep['estado_articulo'] != 'DEVUELTO'){
					...
					WHERE id_det_presup_articulo = %s
						...
						valTpDato($rowOrdenDetRep['id_det_presup_articulo'], "int"),
						valTpDato($valFormDcto['txtIdPresupuesto'], "int"));
		...
		$queryPresupuestoDet = sprintf("SELECT * FROM sa_det_presup_tempario
		WHERE id_presupuesto = %s
			AND id_paquete IS NULL",
			valTpDato($valFormDcto['txtIdPresupuesto'],"int"));
		...
		while ($rowPresupuestoDet = mysql_fetch_assoc($rsPresupuestoDet)) {
			...
			if (isset($arrayObjTemp)) {
				foreach($arrayObjTemp as $indice => $valor) {
					if ($rowPresupuestoDet['id_det_presup_tempario'] == $valFormListTemp['hddIdPedDetTemp'.$valor]) {
						$existRegDet = true;
					}
				}
			}
			...
			if ($existRegDet == false) {
				if($rowPresupuestoDet['estado_tempario'] != 'DEVUELTO') {
					...
					WHERE id_det_presup_tempario = %s
						AND id_presupuesto= %s",
						valTpDato($rowPresupuestoDet['id_det_presup_tempario'], "int"),
						valTpDato($valFormDcto['txtIdPresupuesto'], "int"));
		...
		$queryOrdenDetTot = sprintf("SELECT * FROM sa_det_presup_tot WHERE id_presupuesto = %s", valTpDato($valFormDcto['txtIdPresupuesto'],"int"));
		...
		while ($rowOrdenTot = mysql_fetch_assoc($rsOrdenDetTot)) {
			...
			if (isset($arrayObjTot)) {
				foreach($arrayObjTot as $indice => $valor) {
					if ($rowOrdenTot['id_det_presup_tot'] == $valFormListTot['hddIdPedDetTot'.$valor]) {
						$existRegDet = true;
					}
				}
			}
			...
			if ($existRegDet == false) {	
				$deleteSQL = sprintf("DELETE FROM sa_det_presup_tot WHERE id_det_presup_tot = %s",
					valTpDato($rowOrdenTot['id_det_presup_tot'], "int"));
				$Result1 = mysql_query($deleteSQL);
				if (!$Result1) return $objResponse->alert(mysql_error()."\n\nLine: ".__LINE__);
		...
		$queryOrdenDet = sprintf("SELECT * FROM sa_det_presup_notas WHERE id_presupuesto = %s",
			valTpDato($valFormDcto['txtIdPresupuesto'],"int"));
		...
		while ($rowOrdenDet = mysql_fetch_assoc($rsOrdenDet)) {
			...
			if (isset($arrayObjNota)) {
				foreach($arrayObjNota as $indice => $valor) {
					if ($rowOrdenDet['id_det_presup_nota'] == $valFormListNota['hddIdPedDetNota'.$valor]) {
						$existRegDet = true;
					}
				}
			}
			...
			if ($existRegDet == false) {				
				...
				WHERE id_det_presup_nota = %s
					AND id_presupuesto = %s",
					valTpDato($rowOrdenDet['id_det_presup_nota'], "int"),
					valTpDato($valFormDcto['txtIdPresupuesto'], "int"));
		...
		$queryOrdenDetDcto = sprintf("SELECT * FROM sa_det_presup_descuento WHERE id_presupuesto = %s",
			valTpDato($valFormDcto['txtIdPresupuesto'],"int"));
		...
		while ($rowOrdenDetDcto = mysql_fetch_assoc($rsOrdenDetDcto)) {
			...
			if (isset($arrayObjDcto)) {
				foreach($arrayObjDcto as $indice => $valor) {
					if ($rowOrdenDetDcto['id_det_presup_descuento'] == $valFormTotalDcto['hddIdDetDcto'.$valor]) {
						$existRegDet = true;
					}
				}
			}
			...
			if ($existRegDet == false) {
				$deleteSQL = sprintf("DELETE FROM sa_det_presup_descuento
				WHERE id_det_presup_descuento = %s
					AND id_presupuesto = %s",
                ...
                //aqui habia oro delete de paquetes -probar
		//////////////////////////////////////// FIN DELETE ////////////////////////////////////////////////////////////
                
                
                
                /////////////////////////////////SI HAY NUEVOS AGREGADOS
...
//                                                                                $insertSQL = sprintf("INSERT INTO sa_det_presup_tempario (id_presupuesto, id_tempario, precio, costo, costo_orden, id_modo,  base_ut_precio, operador, ut, tiempo_asignacion, id_mecanico, id_empleado_aprobacion, aprobado, precio_tempario_tipo_orden, origen_tempario)
//                                                                                SELECT 
//                                                                                        ".$valFormDcto['txtIdPresupuesto'].",
...
//
//                                                                                $insertSQL = sprintf("INSERT INTO sa_det_presup_articulo (id_presupuesto, id_articulo, cantidad, id_precio, precio_unitario, id_articulo_costo, id_articulo_almacen_costo, costo, id_iva, iva, tiempo_asignacion, id_empleado_aprobacion, aprobado)
...
//
//                                                                                                $ivasSql = sprintf("INSERT INTO sa_det_presup_articulo_iva (id_det_presup_articulo, base_imponible, subtotal_iva, id_iva, iva) 
//                                                                                                                VALUES (%s, %s, %s, %s, %s)",
...
//                                                                                }
//
//                                                                                $insertSQL = sprintf("INSERT INTO sa_det_presup_notas (id_presupuesto, descripcion_nota, precio, aprobado)
//                                                                                VALUE (%s, %s, %s, %s);",
//                                                                                        valTpDato($valFormDcto['txtIdPresupuesto'], "int"),
...
//                                                                        if ($valFormListTot['hddIdPedDetTot'.$valorTot] == "") {
//                                                                                $insertSQL = sprintf("INSERT INTO sa_det_presup_tot (id_presupuesto, id_orden_tot, id_porcentaje_tot, porcentaje_tot, aprobado)
//                                                                                VALUE (%s, %s, %s, %s, %s);",
//                                                                                        valTpDato($valFormDcto['txtIdPresupuesto'], "int"),
...
//                                                                                $insertSQL = sprintf("INSERT INTO sa_det_presup_descuento (id_presupuesto, id_porcentaje_descuento, porcentaje)
//                                                                                VALUE (%s, %s, %s);",
//                                                                                        valTpDato($valFormDcto['txtIdPresupuesto'], "int"),
...
//                                                                                        $insertSQL = sprintf("INSERT INTO sa_det_presup_tempario (
//                                                                                                id_presupuesto,
//                                                                                                id_paquete,
//                                                                                                id_tempario, 
//                                                                                                precio, 
...
//                                                                                }
//                                                                               $objResponse->alert("primer guardado de paquetes");
//                                                                                if ($rowEmpresa['paquete_combo']==0) { // PAQUETE REPUESTOS costo promedio
//                                                                                $insertSQL = sprintf("INSERT INTO sa_det_presup_articulo (id_presupuesto, id_articulo, id_paquete, cantidad, id_precio, precio_unitario, id_articulo_costo, id_articulo_almacen_costo, costo, tiempo_asignacion, id_empleado_aprobacion, aprobado) 		
//                                                                                SELECT 
//                                                                                        %s,
...
//
//                                                                                    $sqlIvasRepPaquete = sprintf("SELECT id_det_presup_articulo, id_articulo, cantidad, precio_unitario FROM sa_det_presup_articulo 
//                                                                                                                  WHERE id_paquete = %s AND id_presupuesto = %s",
//                                                                                                        valTpDato($valFormPaq['hddIdPaq'.$valorPaq], "int"),
//                                                                                                        valTpDato($valFormDcto['txtIdPresupuesto'], "int"));
...
//
//                                                                                            $ivasSqlPaq = sprintf("INSERT INTO sa_det_presup_articulo_iva (id_det_presup_articulo, base_imponible, subtotal_iva, id_iva, iva) 
//                                                                                                                    VALUES (%s, %s, %s, %s, %s)",
//                                                                                                            valTpDato($rowDetRepPaquete["id_det_presup_articulo"],"int"),
...
//                                                }//FIN PAQUETES DUPLICADO
		...
		mysql_query("SET NAMES 'latin1';");	
                ...
                foreach($valFormTotalDcto["ivaActivo"] as $key => $idIvas){
                    ...
                    if($valFormTotalDcto["txtBaseImponibleIva".$idIvas] != 0 && $valFormTotalDcto["txtTotalIva".$idIvas] != 0 && $valFormTotalDcto["txtIvaVenta".$idIvas] != 0){
                        ...
                        $ivasSql = sprintf("SELECT * FROM sa_presupuesto_iva 
                                        WHERE id_presupuesto = %s AND id_iva = %s LIMIT 1",
                                    valTpDato($valFormDcto['txtIdPresupuesto'],"int"),
                        ...
                        if($rowExisteElIva){//SI EXISTE ACTUALIZO POR SI HUBO CAMBIOS
                            ...
                            $ivasSql = sprintf("UPDATE sa_presupuesto_iva 
                                                SET base_imponible = %s, subtotal_iva = %s, iva = %s
                                                WHERE id_presupuesto = %s AND id_iva = %s
                                                ",                                    
                            ...
                            $ivasSql = sprintf("INSERT INTO sa_presupuesto_iva (id_presupuesto, base_imponible, subtotal_iva, id_iva, iva) 
                                                VALUES (%s, %s, %s, %s, %s)",
                                        valTpDato($valFormDcto['txtIdPresupuesto'],"int"),
                                        valTpDato($valFormTotalDcto["txtBaseImponibleIva".$idIvas],"real_inglesa"),
                                        valTpDato($valFormTotalDcto["txtTotalIva".$idIvas],"real_inglesa"),
                        ...
                        $ivasSql = sprintf("DELETE FROM sa_presupuesto_iva 
                                            WHERE id_presupuesto = %s AND id_iva = %s",
                                    valTpDato($valFormDcto['txtIdPresupuesto'],"int"),
                                    valTpDato($idIvas,"int")
                                    );
                ...
                if($valFormTotalDcto['hddAccionTipoDocumento'] == 4){//aprobacion de presupuesto
                    $genero = generarPresupuestoOrden(valTpDato($valFormDcto['txtIdPresupuesto'],"int"));
                    if(is_array($genero)){
                        if(isset($genero[0]) && $genero[0] == false){
                            return $objResponse->alert($genero[1]);
                        }
                    }
                }
		...
		if ($valFormTotalDcto['hddTipoDocumento'] == 1) { // PRESUPUESTO
			...
			$insertSQL = sprintf("INSERT INTO sa_presupuesto (numero_presupuesto, fecha_presupuesto, id_cliente, id_orden, id_recepcion, id_tipo_orden, id_empresa, id_empleado, tiempo_orden, id_estado_orden, porcentaje_descuento, subtotal_descuento, idIva, iva, subtotal, base_imponible, subtotal_iva, tipo_presupuesto, monto_exento, total_presupuesto)
			VALUE (%s, now(), %s, %s, %s, %s, %s, %s, NOW(), %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s);",
				$numeroPresupuesto,
				valTpDato($valFormDcto['txtIdCliente'], "int"),
				valTpDato($valFormDcto['idOrdenPresupuesto'], "int"),
				valTpDato($valFormDcto['txtIdValeRecepcion'], "int"),
				valTpDato($id_tipo_orden, "int"),
				valTpDato($valFormDcto['txtIdEmpresa'], "int"),
				valTpDato($valFormDcto['hddIdEmpleado'], "int"),
				3, // 3 = Por Aprobacion de Presupuesto
				valTpDato($valFormTotalDcto['txtDescuento'], "real_inglesa"),
				valTpDato($valFormTotalDcto['txtSubTotalDescuento'], "real_inglesa"),
				valTpDato($idIvaSimple, "int"),//solo simple impuestos
				valTpDato($porcentajeIvaSimple, "real_inglesa"),//solo simple impuestos
				...
				valTpDato($valFormTotalDcto['txtMontoExento'], "real_inglesa"),//nuevo
                                valTpDato($valFormTotalDcto['txtTotalPresupuesto'], "real_inglesa")//nuevo
				); //tipo de presupuesto 0 = cotizacion    1 = Presupuesto		
			mysql_query("SET NAMES 'utf8';");	
			$Result1 = mysql_query($insertSQL);
			if (!$Result1) return $objResponse->alert(mysql_error()."\n\nLine: ".__LINE__);
		...
		foreach($arrayObjPaq as $indicePaq => $valorPaq) {
			...
			if ($idPaquete > 0) {
				if ($valFormPaq['hddIdPedDetPaq'.$valorPaq] == "") {
					...
					$objResponse->alert("2");//ALERTA5 //Entra al guardar y act
					if ($rowEmpresa['paquete_combo']==0) { // PAQUETE REPUESTOS costo promedio
					$insertSQL = sprintf("INSERT INTO sa_det_presup_articulo (id_presupuesto, id_articulo, id_paquete, cantidad, id_precio, precio_unitario, id_articulo_costo, id_articulo_almacen_costo, costo, tiempo_asignacion, id_empleado_aprobacion, aprobado) 		
					SELECT 
						%s,
...
function listadoOrdenes($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 10, $totalRows = NULL) {
	...
	$startRow = $pageNum * $maxRows;
        ...
        $cond = (strlen($sqlBusq) > 0) ? " AND " : " WHERE ";
	$sqlBusq .= $cond."orden.id_estado_orden NOT IN (13,18,21,24)";
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$contFila++;
                ...
                $botonSeleccionar = '<button title="Seleccionar Orden" type="button" onClick="xajax_agregarOrden('.$row['id_orden'].','.$row['id_recepcion'].');"><img src="../img/iconos/ico_aceptar.gif"></button>';
		$htmlTb.= "<tr class=\"".$clase."\" height=\"22\">";
                    $htmlTb .= "<td>".$botonSeleccionar."</td>";                   
	...
	$objResponse->assign("tdFlotanteTitulo","innerHTML", ("Ordenes de Servicios"));
	$objResponse->assign("tblListados","width","990px");
	$objResponse->script("
	if ($('divFlotante').style.display == 'none') {
		$('divFlotante').style.display='';
...
function agregarOrden($idOrden,$idRecepcion){
    $objResponse = new xajaxResponse();
   
    $queryOrden = sprintf("SELECT sa_orden.numero_orden, sa_orden.id_tipo_orden, sa_tipo_orden.id_filtro_orden, sa_estado_orden.nombre_estado
                FROM sa_orden
                LEFT JOIN sa_tipo_orden ON sa_orden.id_tipo_orden = sa_tipo_orden.id_tipo_orden 
    ...
    $row = mysql_fetch_assoc($rsOrden);
    
    if($row["id_filtro_orden"] == 5){
        $objResponse->script("xajax_cargaLstTipoOrden(".$row['id_tipo_orden'].");");
    }else{
        $queryTipoOrden = sprintf("SELECT id_tipo_orden, nombre_tipo_orden FROM sa_tipo_orden WHERE id_tipo_orden = %s LIMIT 1",
                $row["id_tipo_orden"]);
        $rsTipoOrden = mysql_query($queryTipoOrden);
        if(!$rsTipoOrden) { return $objResponse->alert(mysql_error()."\n\nLine: ".__LINE__."\nquery:".$queryTipoOrden); }
        $rowTipoOrden = mysql_fetch_assoc($rsTipoOrden);
        $selectTipoOrden = "<select id='lstTipoOrden' name='lstTipoOrden'>";
            $selectTipoOrden .= "<option value = ".$rowTipoOrden["id_tipo_orden"].">".utf8_encode($rowTipoOrden["nombre_tipo_orden"])."</option>";
        $selectTipoOrden .= "</select>";
        
        $objResponse->assign("tdlstTipoOrden","innerHTML",$selectTipoOrden);
    }
    
    $objResponse->assign("idOrdenPresupuesto","value",$idOrden);//es el id de la orden seleccionada
    $objResponse->assign("numeroOrdenMostrar","value",$row["numero_orden"]);
    $objResponse->assign("txtEstadoOrden","value", utf8_encode($row["nombre_estado"]));
    $objResponse->script("xajax_asignarValeRecepcion(".$idRecepcion.",3);");
...
function generarPresupuestoOrden($idPresupuesto){//ya validado antes
    
    //actualizo la orden con la infromacion del presupuesto
    $query = sprintf("SELECT * FROM sa_presupuesto WHERE id_presupuesto = %s LIMIT 1",
            $idPresupuesto);
    $rs = mysql_query($query);
    if(!$rs) { return array(false, mysql_error()."\n\nLine: ".__LINE__."\n\nQuery:".$query); }    
    $rowPresupuesto = mysql_fetch_assoc($rs);
    
    $query = sprintf("SELECT sa_orden.id_tipo_orden, id_filtro_orden 
                      FROM sa_orden
                      LEFT JOIN sa_tipo_orden ON sa_orden.id_tipo_orden = sa_tipo_orden.id_tipo_orden
                      WHERE id_orden = %s LIMIT 1",
            $rowPresupuesto["id_orden"]);
    $rs = mysql_query($query);
    if(!$rs) { return array(false, mysql_error()."\n\nLine: ".__LINE__."\n\nQuery:".$query); }    
    $rowOrden = mysql_fetch_assoc($rs);
   
    if($rowOrden["id_filtro_orden"] == "5"){//si esta SIN ASIGNAR colocar el tipo de orden del presupuesto
        $query = sprintf("UPDATE sa_orden SET id_tipo_orden = %s
                          WHERE id_orden = %s",
            $rowPresupuesto["id_tipo_orden"],
            $rowPresupuesto["id_orden"]);
        $rs = mysql_query($query);
        if(!$rs) { return array(false, mysql_error()."\n\nLine: ".__LINE__."\n\nQuery:".$query); }    
        $rowOrden = mysql_fetch_assoc($rs);
    ...
    $query = sprintf("UPDATE sa_orden SET
			...
			base_imponible = %s, 
			subtotal_iva = %s,
                        id_estado_orden = 19,    #presupuesto aprobado
                        monto_exento = %s,
                        total_orden = %s
                        WHERE id_orden = %s",
                    $rowPresupuesto["porcentaje_descuento"],
                    $rowPresupuesto["subtotal_descuento"],
                    valTpDato($rowPresupuesto["idIva"],"double"),
                    valTpDato($rowPresupuesto["iva"],"int"),
                    $rowPresupuesto["subtotal"],
                    $rowPresupuesto["base_imponible"],
                    $rowPresupuesto["subtotal_iva"],
                    $rowPresupuesto["monto_exento"],
                    $rowPresupuesto["total_presupuesto"],
                    $rowPresupuesto["id_orden"]);
    ...
    //actualizar ivas
        //elimino ivas que no tenga el presupuesto
    $query = sprintf("DELETE FROM sa_orden_iva 
                      WHERE id_orden = %s 
                      AND id_iva NOT IN(SELECT id_iva FROM sa_presupuesto_iva WHERE id_presupuesto = %s)",
            $rowPresupuesto["id_orden"],
            $idPresupuesto);
    $rs = mysql_query($query);
    if(!$rs) { return array(false, mysql_error()."\n\nLine: ".__LINE__."\n\nQuery:".$query); }      
        //actualizo ivas existentes
    $query = sprintf("UPDATE sa_orden_iva
                      INNER JOIN sa_presupuesto_iva ON sa_orden_iva.id_iva = sa_presupuesto_iva.id_iva
                        SET sa_orden_iva.base_imponible = sa_presupuesto_iva.base_imponible,
                             sa_orden_iva.subtotal_iva = sa_presupuesto_iva.subtotal_iva
                      WHERE sa_orden_iva.id_orden = %s 
                      AND sa_presupuesto_iva.id_presupuesto = %s",
            $rowPresupuesto["id_orden"],
            $idPresupuesto);
    $rs = mysql_query($query);
    if(!$rs) { return array(false, mysql_error()."\n\nLine: ".__LINE__."\n\nQuery:".$query); }      
        //inserto ivas nuevos
    $query = sprintf("INSERT INTO sa_orden_iva (id_orden, base_imponible, subtotal_iva, id_iva, iva, lujo)
                                    (SELECT %s, base_imponible, subtotal_iva, id_iva, iva, lujo
                                    FROM sa_presupuesto_iva 
                                    WHERE id_presupuesto = %s 
                                    AND id_iva NOT IN(SELECT id_iva FROM sa_orden_iva WHERE id_orden = %s)                                     
                                    )",
            $rowPresupuesto["id_orden"],
            $idPresupuesto,
            $rowPresupuesto["id_orden"]);
    ...
    $query = sprintf("UPDATE sa_presupuesto SET estado_presupuesto = 1 WHERE id_presupuesto = %s", $idPresupuesto);
    $rs = mysql_query($query);
    if(!$rs) { return array(false, mysql_error()."\n\nLine: ".__LINE__."\n\nQuery:".$query); }  
    
    //agregar articulos a la orden, son del presupuesto que no tengan relacionado orden
//    $query = sprintf("INSERT INTO sa_det_orden_articulo (id_orden, id_articulo, id_paquete, cantidad, id_precio, precio_unitario, id_articulo_costo, id_articulo_almacen_costo, costo, id_iva, iva, aprobado, tiempo_asignacion, tiempo_aprobacion, id_empleado_aprobacion, estado_articulo) 
//                                                       (SELECT %s, id_articulo, id_paquete, cantidad, id_precio, precio_unitario, id_articulo_costo, id_articulo_almacen_costo, costo, id_iva, iva, aprobado, tiempo_asignacion, tiempo_aprobacion, id_empleado_aprobacion, estado_articulo 
//                                                     FROM sa_det_presup_articulo WHERE id_presupuesto = %s AND id_det_orden_articulo IS NULL)", 
//                                                     $rowPresupuesto["id_orden"],
//                                                     $idPresupuesto);
//    $rs = mysql_query($query);
//    if(!$rs) { return array(false, mysql_error()."\n\nLine: ".__LINE__."\n\nQuery:".$query); } 
    
    $query = sprintf("SELECT id_det_presup_articulo, id_articulo, id_paquete, cantidad, id_precio, precio_unitario, id_articulo_costo, id_articulo_almacen_costo, costo, id_iva, iva, aprobado, tiempo_asignacion, tiempo_aprobacion, id_empleado_aprobacion, estado_articulo
                      FROM sa_det_presup_articulo WHERE id_presupuesto = %s AND id_det_orden_articulo IS NULL",
            $idPresupuesto);
    $rs = mysql_query($query);
    if(!$rs) { return array(false, mysql_error()."\n\nLine: ".__LINE__."\n\nQuery:".$query); }
    
    while($rowArticuloPresup = mysql_fetch_assoc($rs)){
        $query = sprintf("INSERT INTO sa_det_orden_articulo (id_orden, id_articulo, id_paquete, cantidad, id_precio, precio_unitario, id_articulo_costo, id_articulo_almacen_costo, costo, id_iva, iva, aprobado, tiempo_asignacion, tiempo_aprobacion, id_empleado_aprobacion, estado_articulo)
                                      VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, '%s', '%s', %s, '%s')",
            $rowPresupuesto["id_orden"],
            $rowArticuloPresup["id_articulo"], 
            valTpDato($rowArticuloPresup["id_paquete"],"int"), 
            $rowArticuloPresup["cantidad"], 
            $rowArticuloPresup["id_precio"], 
            $rowArticuloPresup["precio_unitario"], 
            $rowArticuloPresup["id_articulo_costo"], 
            $rowArticuloPresup["id_articulo_almacen_costo"], 
            $rowArticuloPresup["costo"], 
            valTpDato($rowArticuloPresup["id_iva"],"int"), 
            valTpDato($rowArticuloPresup["iva"],"double"), 
            $rowArticuloPresup["aprobado"], 
            $rowArticuloPresup["tiempo_asignacion"], 
            $rowArticuloPresup["tiempo_aprobacion"], 
            $rowArticuloPresup["id_empleado_aprobacion"], 
            $rowArticuloPresup["estado_articulo"]
                );
        $rs2 = mysql_query($query);
        if(!$rs2) { return array(false, mysql_error()."\n\nLine: ".__LINE__."\n\nQuery:".$query); }
        $idDetOrdenArticulo = mysql_insert_id();
        
        $query = sprintf("SELECT base_imponible, subtotal_iva, id_iva, iva, lujo 
                        FROM sa_det_presup_articulo_iva 
                        WHERE id_det_presup_articulo = %s",
            $rowArticuloPresup["id_det_presup_articulo"]);
        $rs3 = mysql_query($query);
        if(!$rs3) { return array(false, mysql_error()."\n\nLine: ".__LINE__."\n\nQuery:".$query); }
        
        while($rowArtIvaPresup = mysql_fetch_assoc($rs3)){
            $query = sprintf("INSERT INTO sa_det_orden_articulo_iva (id_det_orden_articulo, base_imponible, subtotal_iva, id_iva, iva, lujo)
                                    VALUES(%s, %s, %s, %s, %s, %s)",
            $idDetOrdenArticulo,
            $rowArtIvaPresup["base_imponible"],
            $rowArtIvaPresup["subtotal_iva"],
            $rowArtIvaPresup["id_iva"],
            $rowArtIvaPresup["iva"],
            valTpDato($rowArtIvaPresup["lujo"],"int")
                    );
            $rs4 = mysql_query($query);
            if(!$rs4) { return array(false, mysql_error()."\n\nLine: ".__LINE__."\n\nQuery:".$query); }
        }
        
    }
    
    //agregar temparios a la orden, son del presupuesto que no tengan relacionado orden    
    $query = sprintf("INSERT INTO sa_det_orden_tempario (id_orden, id_paquete, id_tempario, precio, costo, costo_orden, id_modo, base_ut_precio, operador, aprobado, ut, tiempo_aprobacion, tiempo_asignacion, id_empleado_aprobacion, origen_tempario, estado_tempario,precio_tempario_tipo_orden) 
                                                       (SELECT %s, id_paquete, id_tempario, precio, costo, costo_orden, id_modo, base_ut_precio, operador, aprobado, ut, tiempo_aprobacion, tiempo_asignacion, id_empleado_aprobacion, origen_tempario, estado_tempario,precio_tempario_tipo_orden
                                                     FROM sa_det_presup_tempario WHERE id_presupuesto = %s AND id_det_orden_tempario IS NULL)", 
                                                     $rowPresupuesto["id_orden"],
                                                     $idPresupuesto);
    $rs = mysql_query($query);
    if(!$rs) { return array(false, mysql_error()."\n\nLine: ".__LINE__."\n\nQuery:".$query); } 
    
    //agregar notas de cargo, son del presupuesto que no tengan relacionado orden
    $query = sprintf("INSERT INTO sa_det_orden_notas (id_orden, descripcion_nota, precio, aprobado) 
                                                    (SELECT %s, descripcion_nota, precio, aprobado
                                                    FROM sa_det_presup_notas WHERE id_presupuesto = %s AND id_det_orden_nota IS NULL)", 
                                                    $rowPresupuesto["id_orden"],
                                                    $idPresupuesto);
    $rs = mysql_query($query);
    if(!$rs) { return array(false, mysql_error()."\n\nLine: ".__LINE__."\n\nQuery:".$query); } 
        
    //agregar tot, son del presupuesto que no tengan relacionado orden
    $query = sprintf("INSERT INTO sa_det_orden_tot (id_orden, id_orden_tot, id_porcentaje_tot, porcentaje_tot, aprobado) 
                                                   (SELECT %s, id_orden_tot, id_porcentaje_tot, porcentaje_tot, aprobado
                                                    FROM sa_det_presup_tot WHERE id_presupuesto = %s AND id_det_orden_tot IS NULL)", 
                                                    $rowPresupuesto["id_orden"],
                                                    $idPresupuesto);
    $rs = mysql_query($query);
    if(!$rs) { return array(false, mysql_error()."\n\nLine: ".__LINE__."\n\nQuery:".$query); }
    
    //actualizar tot como ASIGNADOS a orden
    $query = sprintf("UPDATE sa_orden_tot SET id_orden_servicio = %s, estatus = 2 WHERE id_orden_tot IN(SELECT id_orden_tot FROM sa_det_presup_tot WHERE id_presupuesto = %s AND id_det_orden_tot IS NULL)",
                            $rowPresupuesto["id_orden"],
                            $idPresupuesto);
    $rs = mysql_query($query);
    if(!$rs) { return array(false, mysql_error()."\n\nLine: ".__LINE__."\n\nQuery:".$query); }
    
    //agregar descuentos adicionales, no existe relacion
    $query = sprintf("INSERT INTO sa_det_orden_descuento (id_orden, id_porcentaje_descuento, porcentaje) 
                                                        (SELECT %s, id_porcentaje_descuento, porcentaje
                                                        FROM sa_det_presup_descuento WHERE id_presupuesto = %s)", 
                                                        $rowPresupuesto["id_orden"],
                                                        $idPresupuesto);
    $rs = mysql_query($query);
    if(!$rs) { return array(false, mysql_error()."\n\nLine: ".__LINE__."\n\nQuery:".$query); }
    
    return array(true, "Se ha generado a la orden");
...
$xajax->register(XAJAX_FUNCTION,"agregarOrden");
