<?php
@session_start();
define('PAGE_PRIV','sa_informe_orden');// nuevo gregor
//define('PAGE_PRIV','sa_informe_ordenes');//anterior
...
include("controladores/ac_iv_general.php");//tiene la funcion listado de empresas final
	...
	function load_page($args=''){//$page,$maxrows,$order,$ordertype,$capa,
		...
		$query = new query($c);
		//$query->orderBy('numero_orden','DESC');//gregor, sirve pero elimina la paginacion al hacer "click"
		$query->add($listado);
		...
		if($argumentos['h_estado']!=''){
			$query->where(new criteria(sqlEQUAL,"id_estado_orden","'".$argumentos['h_estado']."'"));
		}
		...
		$query->where(new criteria(sqlNOTEQUAL,"id_estado_orden",18));
		...
		if($rec){
			if($rec->getNumRows()==0){
				...
				$html.='</tbody>
                                        <!--footer>
                                            <tr>
                                                <td colspan="11" align="right"><b>Total por P&aacute;gina:</b></td>
	...
	$estados=$c->sa_estado_orden->doQuery($c,new criteria(sqlEQUAL,'activo',1))->where(new criteria(sqlNOTEQUAL,'id_estado_orden',18))->orderBy('orden')->doSelect()->getAssoc('id_estado_orden','nombre_estado');
	...
	function listado_tipo_orden($idEmpresa = ""){
		...
		$queryTipoOrden = "SELECT id_tipo_orden, nombre_tipo_orden FROM sa_tipo_orden WHERE id_empresa = ".$idEmpresa." ";
        ...
        //$tipos=$c->sa_tipo_orden->doQuery($c)->orderBy('nombre_tipo_orden')->doSelect()->getAssoc('id_tipo_orden','nombre_tipo_orden');
                    ...
                    <table align="center">
                        <tr>
                            <td>
                                ...
                                <fieldset style="display:inline; white-space: nowrap;">
