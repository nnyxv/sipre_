<?php
//require("sa_mantenimiento_modelo_xajax.php");
...
//implementando xajax;
	...
	require_once("control/funciones.inc.php");
        ...
        define('PAGE_PRIV','sa_mantenimiento_modelo');
	...
	function load_page($page,$maxrows,$order,$ordertype,$capa,$args=''){
		...
		//if($from_vehiculos){		
			$an_modelo = $c->an_modelo->join($c->an_marca,$c->an_marca->id_marca,$c->an_modelo->id_marca);
		//}else{
		//	$an_modelo = $c->an_modelo;
		//}
		$query = new query($c);
		$query->add($an_modelo);
		...
		if($argumentos['busca']!=''){
			$query->where(new criteria(sqlOR,array(
				new criteria(' like ',$c->an_modelo->nom_modelo,"'%".$argumentos['busca']."%'"),
				new criteria(' like ',$c->an_modelo->des_modelo,"'%".$argumentos['busca']."%'")				
			)));
		...
		/*if($from_vehiculos){
			//$r->alert('filtrar por vehiculos');
			$query->where(new criteria(sqlEQUAL,$an_modelo->empresa_catalogo,$_SESSION['session_empresa']));
		}*/
		...
		if($rec){
			if($rec->getNumRows()==0){
				...
				<td>'.$paginador->get($c->an_marca->nom_marca,'Marca').'</td>
				<td>'.$paginador->get($c->an_modelo->nom_modelo,'Modelo').'</td>
				<td>'.$paginador->get($c->an_modelo->des_modelo,'Descripci&oacute;n').'</td>
				<td>&nbsp;</td>
				<td>&nbsp;</td>
				<td>&nbsp;</td>
				</tr></thead><tbody>';
				...
				foreach($rec as $v){
					...
					<td align="center">'.$rec->nom_modelo.'</td>
					<td align="center">'.$rec->des_modelo.'</td>
					<td align="center"><img src="'.getUrl('img/iconos/view.png').'" width="16" border="0" onClick="xajax_cargar('.$rec->id_modelo.',\'view\');"></td>
					
					<td align="center"><img src="'.getUrl('img/iconos/edit.png').'" width="16" border="0" onClick="xajax_cargar('.$rec->id_modelo.',\'edit\');"></td>
					<td align="center"><img src="'.getUrl('img/iconos/delete.png').'" border="0" onClick=" if(_confirm(\'&iquest;Desea Eliminar?\')) xajax_cargar('.$rec->id_modelo.',\'delete\');""></td>
					</tr>';
					if($class==''){
						$class='impar';
					}else{
	...
	function cargar_empresas($id_modelo=null,$c=null){
		$r=getResponse();
		if($c==null){
			$c= new connection();
			$c->open();
		...
		//cargando la lista de empresas:
			...
			if($id_modelo!=''){
				...
				$subquery->where(new criteria(sqlEQUAL,$c->sa_unidad_empresa->id_modelo,$id_modelo));
				$t->add($subquery);
			}
			...
			/*$sq->where(new criteria(sqlOR,array(
				new criteria(sqlEQUAL,$c->sa_v_unidad_empresa->id_modelo,$rec->id_modelo),
				new criteria(sqlIS,$c->sa_v_unidad_empresa->id_modelo,sqlNULL)
			)));*/
	...
	function cargar_marca($c,$id_modelo){
		$r=getResponse();
		if($c==null){
			$c= new connection();
			$c->open();
		...
		//buscando las marcas
		$recmarca=$c->an_modelo->doSelect($c)->getAssoc('id_modelo','nom_marca');
		$select = inputSelect('mar_uni_bas',$recmarca,$id_modelo,array('onchange'=>'xajax_cargar_modelo(null,null,this.value);xajax_cargar_version(null,null,null);'));
		$r->assign('field_mar_uni_bas',inner,($select));
		return $r;
	}
	
	function cargar_modelo($c,$id_modelo,$id_modelo){
		$r=getResponse();
		if ($id_modelo==''){
		...
		//buscando las marcas
		$recmodelo=$c->an_modelo->doSelect($c,new criteria(sqlEQUAL,$c->an_modelo->id_modelo,$id_modelo))->getAssoc('id_modelo','nom_modelo');
	...
	function cargar($id, $mode='view'){
		...
		$q->where(new criteria(sqlEQUAL,$c->an_modelo->id_modelo,$id));
		if($mode!='delete'){
			$rec=$q->doSelect();
			...
			if($rec){
				$r->assign('id_modelo','value',$rec->id_modelo);
				$r->assign('id_marca','value',$rec->id_marca);
				$r->assign('nom_modelo','value',$rec->nom_modelo);
				$r->assign('des_modelo','value',$rec->des_modelo);

			}
			
			//$r->loadCommands(cargar_empresas($rec->id_modelo,$c));
			...
			$c->begin();
			$rec=$c->an_modelo->doDelete($c,new criteria(sqlEQUAL,$c->an_modelo->id_modelo,$id));
			if($rec===true){
				$c->commit();
	...
	function guardar($form){
		...
		$an_modelo = new table("an_modelo");
		
		$an_modelo->add(new field('id_modelo','',field::tInt,$form['id_modelo']));
		$an_modelo->add(new field('id_marca','',field::tInt,$form['id_marca']));
		$an_modelo->add(new field('nom_modelo','',field::tString,$form['nom_modelo']));
		$an_modelo->add(new field('des_modelo','',field::tString,$form['des_modelo']));
		...
		if($form['id_modelo']==''){
			...
			$result=$an_modelo->doInsert($c,$an_modelo->id_modelo);
			//$id_modelo=$c->soLastInsertId();
		}else{
			...
			$result=$an_modelo->doUpdate($c,$an_modelo->id_modelo);
	...
	$c = new connection();
	$c->open();
	$marcas=$c->an_marca->doSelect($c)->getAssoc('id_marca','nom_marca');
...
<html>
	<head>
		...
		<?php //include("sa_mantenimiento_modelo_js.php"); ?>
        <style type="text/css">
	button img{
		padding-right:1px;
		padding-left:1px;
...
<div class="window" id="edit_window" style="width:300px;min-width:300px;max-width:300px;top:0px;left:0px;visibility:hidden;">
	<div class="title" id="title_window">
		<span id="subtitle"></span>&nbsp;Modelo
	</div>
	<div class="content">
		
<?php
	//include("sa_mantenimiento_modelo_form.php");
?>		
	...
	<table class="insert_table">
		<tbody>
			...
			<tr>
				...
				<td class="field" id="field_nom_modelo">
					<input type="text" name="nom_modelo" id="nom_modelo" maxlength="30" />
				</td>
			</tr>
			<tr>
				<td class="label">
				...
				<td class="field" id="field_des_modelo">
					<input type="text" name="des_modelo" id="des_modelo" maxlength="30" />
				</td>
			</tr>
