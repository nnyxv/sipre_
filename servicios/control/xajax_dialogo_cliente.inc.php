<?php
//para incluir y s�lo busca CLIENTES
	...
	function dialogo_cliente($page,$maxrows,$order,$ordertype,$capa,$args=''){//$busqueda, $callback, $parent=''
		...
		if($rec){
			...
			<div>
				...
				<button title="buscar" onclick="'.$func.'" ><img border="0" src="'.getUrl('img/iconos/find.png').'" alt="buscar" /></button>
				<button title="Volver" onclick="'.$rfunc.'" ><img border="0" src="'.getUrl('img/iconos/cc.png').'" alt="Volver" /></button>
				</div>
			';
			if($rec->getNumRows()!=0){
				if($insertmode){
				...
				<td>&nbsp;</td>
				<td>'.$paginator->get($cliente->ci,$spanCI.' - '.$spanRIF).'</td>
				...
				foreach ($rec as $reg){
					if(!$insertmode){
						...
						$but='<td width="1%"><button type="button" '.$trclickfunc.'><img border="0" alt="cargar" src="'.getUrl('img/iconos/minselect.png').'" /></button></td>';
					}else{
						$but='<td width="1%"></td>';
			...
			if(!$insertmode){
				$html.='<img class="close_window" src="../img/iconos/close_dialog.png" alt="cerrar" title="Cerrar" style="cursor:pointer;" onclick="close_window(\''.$capa.'\');" border="0" />';
			}
		}
		if(!$insertmode){
