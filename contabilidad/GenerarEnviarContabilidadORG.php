...
$exec = EjecutarExec($con,$SqlStr) or die($SqlStr."<br>Line: ".__LINE__);
	...
	while ($row = ObtenerFetch($exec)) {
		...
		if ($valor == 3) { //3 = Puerto Rico
			$Deber = bcadd($Debe,$montoiva,2); //SUMA
		}
...
$exec =  EjecutarExec($con,$SqlStr) or die($SqlStr."<br>Line: ".__LINE__); 
    ...
    while ($row = ObtenerFetch($exec)) {
		...
		if(buscarDoc($id,$cc,$ct,$dt,$fecha)==0){
			...
			if ($row[0] > 0){
				$MontoRetenidoIVA = $row[0];     	   
				$cuentaRetenido = buscarContable(76,17,$sucursal);// 76 Fijos Administracion - 75 CXC Administracion
			}
...
$cc='04';
	...
	while ($row = ObtenerFetch($exec)) {
		...
		if(!is_null($idDocumento)){ // SI POSEE GASTOS
