...
while ($row = ObtenerFetch($rs3)) {
	...
	if(trim(ObtenerResultado($rs3,5,$iFila)) == 1){
		$estatus = "Datos Incompletos";
	}else {
		$estatus ="";
	}
 ...
 ?>
	<tr bgColor='white' id=<?php print($iFila);?> onClick="<?php print("jElegir('$CodigoBuscar','$DesBuscar','$sObjeto1','$sObjeto2','$sPlantillaBus')");?>" style="cursor:pointer"   onMouseOut="FueraFila('',this);" onMouseMove="SobreFila('',this);" >
		<td width="20%" align="center" class="RenglonesMov"><?php print($CodigoBuscar);?></td>
		<td width="60%" class="RenglonesMov"><?php print($DesBuscar);?></td>
        <?php if($sTabla == 'deprecactivos'){?>
    	<td width="25%" class="RenglonesMov"><?php print($estatus);?></td>
		<?php } ?>
	</tr>
