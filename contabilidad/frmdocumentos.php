...
<title>.: SIPRE 2.0 :. Contabilidad - Documentos</title>
...
<!--*****************************************************************************************-->
function Buscar(){
document.frmdocumentos.target='mainFrame';
document.frmdocumentos.method='post';
document.frmdocumentos.action='frmdocumentos.php';
document.frmdocumentos.StatusOculto.value='BU';
document.frmdocumentos.submit();
}// function AbrirBus(sObjeto,oArreglo){
...
function BuscarDescrip(sValor,sCampoBuscar,oArreglo){
    if (Alltrim(sValor) != ''){
document.frmdocumentos.TACondicion.value=sCampoBuscar +"= '" + sValor + "'"
document.frmdocumentos.TAValores.value=oArreglo;
document.frmdocumentos.method='post';
document.frmdocumentos.target='topFrame';
document.frmdocumentos.action='BusTablaParametros.php';
document.frmdocumentos.submit();
 }// if (Alltrim(sValor) != ''){
 }//function BuscarDescrip(sValor,sCampoBuscar,oArreglo){
...
function Ejecutar(sStatus){
       ...
       document.frmdocumentos.action='frmdocumentos.php';
      if (sStatus == "LI"){
        document.frmdocumentos.StatusOculto.value = "LI"
        document.frmdocumentos.submit();
      }
      if (sStatus == "IN"){
        if (VerificarFechasJ(document.frmdocumentos))
        {
           return false;
        }
        if (CamposBlancosJ(document.frmdocumentos))
        {
           return false;
        }
        document.frmdocumentos.StatusOculto.value = "IN"
        document.frmdocumentos.submit();
      }
      else if (sStatus == "UP")
      {
         ...
         document.frmdocumentos.StatusOculto.value = "UP"
         document.frmdocumentos.submit();
      }
      else if (sStatus == "DE"){
        if (confirm('Desea Eliminar el registro')){
         document.frmdocumentos.StatusOculto.value = "DE"
         document.frmdocumentos.submit();
        }
      }
     else if (sStatus == 'BU'){
         if (document.frmdocumentos.T_codigo.value== '' || document.frmdocumentos.Desha.value == 'readonly'){
           return false;
        }
         document.frmdocumentos.StatusOculto.value = "BU"
         document.frmdocumentos.submit();
...
//**********************************************************************
        ...
        actualizarCampos('documentos', 1, 'descripcion',$T_descripcion);
...
//**********************************************************************
        ...
        $exc = EjecutarExec($con,$SqlStr) or die($SqlStr);
        actualizarCampos('documentos', 2, 'codigo',$T_codigo);
...
//**********************************************************************
        ...
        $exc = EjecutarExec($con,$SqlStr) or die($SqlStr);
                actualizarCampos('documentos', 3, 'codigo',$T_codigo);
                $StatusOculto ='LI';
                $T_codigo='';
                $T_descripcion='';
}
...
?>
			Documentos (Inclusi&oacute;n)
<?php 
	}else{
		$Desha = 'readonly class=cTexBoxdisabled';
?>
			Documentos (Modificaci&oacute;n)
<?php 
	} 
?>
		</td>            
...
<form name="frmdocumentos"action="frmdocumentos.php"method="post">
...
<table width="100%" align="center">
	<tr>
   	...
   	<tr>    
		<td><fieldset>
		  	<legend class="legend">Datos del Documento</legend>
        	<table border="0" align="center">
                ...
                <tr>
                    ...
                    <td align="left"><input  name="T_descripcion"type="text"maxlength=100 size="100" onFocus="SelTexto(this);"onKeyPress="fn(this.form,this,event,'')" value="<?=$T_descripcion?>" class="cTexBox"> 
                    </td>
                </tr>
			</table>
         </fieldset>
...
<?php 
	$Arretabla2[0][0]= 'documentos';//Tabla
	$Arretabla2[0][1]= 'T';
	...
	$Arretabla2[4][1]= 'O';
	$Arretabla2[5][0]= 'frmdocumentos'; // Pantalla donde estamos ubicados
	$Arretabla2[5][1]= 'P';
...
?>
            ...
            <script language='javascript'>
        		document.frmdocumentos.T_descripcion.focus();
            </script>
<?php 
	}else{ 
?>
...
?>
            <script language='javascript'>
        		document.frmdocumentos.T_codigo.focus();
            </script>
<?php 
		} 
?>
