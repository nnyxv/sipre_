...
//my_sql
  ...
  return mysql_connect("localhost","sipre_contable","c0nt@b1l1d@d");//return mysql_connect("localhost","oriomka_root","oriomka");Realiza la coneccion al Servidor Remoto
...
$CN ='MY';
		...
		//my_sql
		  $link = mysql_connect("localhost","sipre_contable","c0nt@b1l1d@d");//$link = mysql_connect("localhost","oriomka_root","oriomka");Realiza la coneccion al Servidor Remoto
...
function CargarSaldos($cDesde1,$cHasta1,$cDesde2='',$cHasta2='',$icierre = 0){
	...
	$dfec_proceso1 = obFecha($dfec_proceso,'A')."-".obFecha($dfec_proceso,'M')."-01";
	...
	if($AnoMesDesde < $AnoMesProceso && $AnoMesDesde == $AnoMesHasta){
	 ...
	 // $MesCierr_d = substr($Mesword,0,3)."_cierrd";
	   //	$MesCierr_h = substr($Mesword,0,3)."_cierrh";///cambiado ernesto
	   	$MesCierr_d = substr("junio",0,3)."_cierrd";//cambiado ernesto
		$MesCierr_h = substr("junio",0,3)."_cierrh";//cambiado ernesto
...
//MovimientosH($cDesde1,$cHasta1);
	...
	if ($cDesde2 != ''){
		$SqlStr.= " and codigo between '$cDesde2' and  '$cHasta2' ";
	}//acá a la cuenta en saldo_ant le asigno 0
	$exc = EjecutarExec($con,$SqlStr) or die($SqlStr);
	...
	if($AnoMesDesde == $AnoMesProceso){	
		...
		while ($row = ObtenerFetch($rs3)) { //es aqui justamente donde hace el desbarajuste loc en el saldo anterior
			$iFila++;
...
function auditoria($sentencia, $tabla, $camposClave, $observacion){
