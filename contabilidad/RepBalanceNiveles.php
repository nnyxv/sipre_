	...
	while ($row = ObtenerFetch($exc)){
		...
		if(substr_count(ObtenerResultado($exc,1,$iFila), '.') <= $Nivel){	
			...
			if($Veces == $Nivel){
				...
				//SOLO  PARA LA UTILIDAD NO ESTOY TOMANDO NUnCA EN CUENTA EL ASIENTO DE CIERRE SIEMPRE LO ESTOY APLICANDO DIRECTO 
				if($Nivel ==4 && $codigo==$cuenta1){
						$pdf->SetX(1);   
						$Sal4 = parentesis($SaldoActual);						
						$bordes1 = array ('','',$Bor4,$Bor3,$Bor2,$Bor1,$Bor0); 
...
//auditoria
auditoria('consulta','cuentageneral',$sCampos,'consulta balance general por niveles, fecha desde: '.$cDesde1);
//fin auditoria
