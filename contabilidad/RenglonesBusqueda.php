...
if ($_SESSION["CCSistema"] != ""){
	if ($sTabla == 'sipre_contabilidad.cuenta'){
		$EstadoCondicion = " order by $pClave";
	}
}else{
	$EstadoCondicion = " order by $pClave";
}

if (trim($TexCodigoBus) == '' && trim($TexDescripcionBus) == ''){
	$sql = "Select " .$sCampos. " from ". $sTabla ;
}else{
...
?>
	...
	<tr bgColor='white' id=<?php print($iFila);?> onClick="<?php print("jElegir('$CodigoBuscar','$DesBuscar','$sObjeto1','$sObjeto2','$sPlantillaBus')");?>" style="cursor:pointer" onMouseOut="FueraFila('',this);" onMouseMove="SobreFila('',this);" >        	
    	<td width="20%"  class="RenglonesMov"><?=$CodigoBuscar?></td>
		<td width="80%"  class="RenglonesMov"><?= $DesBuscar?></td>
   </tr>	
<?php   
	}
