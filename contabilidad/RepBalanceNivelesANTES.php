            ...
            while ($row = ObtenerFetch($exc)){
				...
				if(substr_count(ObtenerResultado($exc,1,$iFila), '.') <= $Nivel){	
					...
					if($Veces == $Nivel){
						$pdf->SetX(1);   
						if(substr($cuenta1,0,strlen(trim($codigo)))==trim($codigo)){
						  //$Sal4 = parentesis(bcadd($SaldoActual,$Utilidad,2));echo $Sal4."aqui";
						 						 //$Sal4 = parentesis($SaldoActual);
							$Sal5 = parentesis($SaldoActual);
...
//												 echo "sal5- ".$Sal5."<br>";
						...
						//$campos2 = array ("3.1.01.01.001","UTILIDADES EJERCICIOS ANTERIORES",$Sal5,"","","","");
						$campos2 = array ($codigo,$descripcion,$Sal5,"","","","");
						
					//probando
					if(substr($cuenta1,0,strlen(trim($codigo)))!=trim($codigo)){
							$pdf->enc_detallePre($campos1,$Ancho1,4,$TamañoLetra1,'',$Alinear1,$bordes1,$MaxLon1);// este era el unico que estaba
					}else{//agregado
//						if($codigo == '3.1.01.01.002'){//agregado					
							$pdf->enc_detallePre($campos2,$Ancho1,4,$TamañoLetra1,'',$Alinear1,$bordes1,$MaxLon1);
					}
		...
		//Este calculo es para el Total Pasivo + Capital			
		//if ($cDesde3 == 'NO'){
			 	$SqlStr=" select sum(a.saldo_ant+a.debe-a.haber)";
		/*}else{ 
