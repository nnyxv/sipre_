...
<title>.: SIPRE 2.0 :. Contabilidad - Centro de Costo</title>
...
<!--*****************************************************************************************-->
function Buscar(){
	document.frmcentrocosto.target='mainFrame';
	document.frmcentrocosto.method='post';
	document.frmcentrocosto.action='frmcentrocosto';
	document.frmcentrocosto.StatusOculto.value='BU';
	document.frmcentrocosto.submit();
}// function AbrirBus(sObjeto,oArreglo){
...
function BuscarDescrip(sValor,sCampoBuscar,oArreglo){
	if (Alltrim(sValor) != ''){
		document.frmcentrocosto.TACondicion.value=sCampoBuscar +"= '" + sValor + "'"
		document.frmcentrocosto.TAValores.value=oArreglo;
		document.frmcentrocosto.method='post';
		document.frmcentrocosto.target='topFrame';
		document.frmcentrocosto.action='BusTablaParametros.php';
		document.frmcentrocosto.submit();
 	}// if (Alltrim(sValor) != ''){
}//function BuscarDescrip(sValor,sCampoBuscar,oArreglo){
...
function Ejecutar(sStatus){
	document.frmcentrocosto.target='mainFrame';
    document.frmcentrocosto.method='post';
    document.frmcentrocosto.action='frmcentrocosto.php';
	//alert(sStatus);
    if (sStatus == "LI"){
        document.frmcentrocosto.StatusOculto.value = "LI"
        document.frmcentrocosto.submit();
    }
    if (sStatus == "IN"){
    	if (VerificarFechasJ(document.frmcentrocosto)){
        	return false;
        }
        if (CamposBlancosJ(document.frmcentrocosto)){
        	return false;
        }
        document.frmcentrocosto.StatusOculto.value = "IN"
        document.frmcentrocosto.submit();
	}else if (sStatus == "UP"){
         if (VerificarFechasJ(document.frmcentrocosto)){
            return false;
         ...
         document.frmcentrocosto.StatusOculto.value = "UP"
         document.frmcentrocosto.submit();
	}else if (sStatus == "DE"){
        if (confirm('Desea Eliminar el registro')){
        	document.frmcentrocosto.StatusOculto.value = "DE"
         	document.frmcentrocosto.submit();
        }
	}
    else if (sStatus == 'BU'){
    	if (document.frmcentrocosto.T_codigo.value== '' || document.frmcentrocosto.Desha.value == 'readonly'){
        	return false;
        }
        document.frmcentrocosto.StatusOculto.value = "BU"
        document.frmcentrocosto.submit();
...
//**********************************************************************
    ...
    actualizarCampos('centrocosto', 1, 'descripcion',$T_descripcion);
...
//**********************************************************************
    ...
    $exc = EjecutarExec($con,$SqlStr) or die($SqlStr);
        actualizarCampos('centrocosto', 2, 'codigo',$T_codigo);
...
//**********************************************************************
    ...
    $exc = EjecutarExec($con,$SqlStr) or die($SqlStr);
          actualizarCampos('centrocosto', 3, 'codigo',$T_codigo);
    $StatusOculto ='LI';
    $T_codigo='';
    $T_descripcion='';
}
...
?>
			Centro de Costo (Inclusi&oacute;n)
<?php 
	}else{
   		$Desha = 'readonly class=cTexBoxdisabled';
?>
			Centro de Costo (Modificaci&oacute;n)
<?php 
	} 
?>            
		</td>            
...
<form name="frmcentrocosto"action="frmcentrocosto.php"method="post">
...
<table width="100%" align="center">
    ...
    <tr>
        <td><fieldset>
            <legend class="legend">Datos del Centro de Costo</legend>
...
<?php 
	...
	$Arretabla2[4][1]= 'O';
	$Arretabla2[5][0]= 'frmcentrocosto'; // Pantalla donde estamos ubicados
	$Arretabla2[5][1]= 'P';
	$ArreGeneral = array_envia($Arretabla2); // Serializar Array
?>
			<button name=BtnBuscar type=button value=Buscar onClick="<?php print("PantallaBuscar('T_Codigo','$ArreGeneral')");?>"Class = inputBoton ><table align="center" cellpadding="0" cellspacing="0"><tr><td>&nbsp;</td><td><img src="../img/iconos/find.png"/></td><td>&nbsp;</td><td>Buscar</td></tr></table></button> 
...
<?php
	if (!($StatusOculto =='LI' || $StatusOculto =='' || $StatusOculto =='DE')){ ?>
		...
		<script language='javascript'>
            document.frmcentrocosto.T_descripcion.focus();
        </script>
...
<?php 
            ...
            <script language='javascript'>
        		document.frmcentrocosto.T_codigo.focus();
            </script>
<?php 
		} 
?>
